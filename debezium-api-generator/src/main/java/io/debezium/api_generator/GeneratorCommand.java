...
public class GeneratorCommand implements Runnable {
    ...
    public void run() {
        ...
        outStream.println("Generating API Spec Format: "
                + apiFormat.getDescriptor().getName()
                + ":" + apiFormat.getDescriptor().getVersion());

        outStream.println("Generating connector/fileScanner: " + String.join(", ", connectors.keySet()));
        ...
        if (!errors.isEmpty()) {
            ...
            outStream.println("No errors during parsing. \uD83D\uDE0A");
        }
    }
}
