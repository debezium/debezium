...
cd "${SCRIPT_LOCATION}" || exit 1
{
  ...
  mvn -Dmaven.failsafe.debug="-agentlib:jdwp=transport=dt_socket,server=y,suspend=y,address=*:${DEBUG_PORT}" verify -pl debezium-testing/debezium-testing-system -PsystemITs,oracleITs -Docp.pull.secret.paths="${SECRET_LOCATION}" ${TESTSUITE_ARGUMENTS}
