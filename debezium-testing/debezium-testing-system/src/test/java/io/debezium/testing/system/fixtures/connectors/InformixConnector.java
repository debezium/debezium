...
public class InformixConnector extends ConnectorFixture<SqlDatabaseController> {

    private static final String CONNECTOR_NAME = "inventory-connector-informix";

    public InformixConnector(ExtensionContext.Store store) {
        super(CONNECTOR_NAME, SqlDatabaseController.class, store);
    }

    @Override
    public ConnectorConfigBuilder connectorConfig(String connectorName) {
        return new ConnectorFactories(kafkaController).informix(dbController, connectorName);
    }
