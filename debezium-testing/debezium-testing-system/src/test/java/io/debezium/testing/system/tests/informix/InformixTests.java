...
import static io.debezium.testing.system.tools.ConfigProperties.DATABASE_INFORMIX_PASSWORD;
import static io.debezium.testing.system.tools.ConfigProperties.DATABASE_INFORMIX_USERNAME;
...
public abstract class InformixTests extends ConnectorTest {
    ...
    public void insertCustomer(SqlDatabaseController dbController, String firstName, String lastName, String email)
            throws SQLException {
        SqlDatabaseClient client = dbController.getDatabaseClient(DATABASE_INFORMIX_USERNAME, DATABASE_INFORMIX_PASSWORD);
        String sql = "INSERT INTO informix.customers(first_name,last_name,email) VALUES  ('" + firstName + "', '" + lastName + "', '" + email + "')";
    ...
    public void renameCustomer(SqlDatabaseController dbController, String oldName, String newName) throws SQLException {
        SqlDatabaseClient client = dbController.getDatabaseClient(DATABASE_INFORMIX_USERNAME, DATABASE_INFORMIX_PASSWORD);
        String sql = "UPDATE informix.customers SET first_name = '" + newName + "' WHERE first_name = '" + oldName + "'";
        client.execute("inventory", sql);
    }
    ...
    public void shouldSnapshotChanges() {
        connectController.getMetricsReader().waitForInformixSnapshot(connectorConfig.getDbServerName());

        String topic = connectorConfig.getDbServerName() + ".informix.customers";
    ...
    public void shouldRerouteUpdates(SqlDatabaseController dbController) throws SQLException {
        ...
        awaitAssert(() -> assertions.assertRecordsCount(prefix + ".informix.customers", 5));
