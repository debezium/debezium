...
public class OcpMongoCertGenerator {
    ...
    public static void generateMongoTestCerts(OpenShiftClient ocp) throws Exception {
        ...
        CertUtil.keystoreToConfigMap(ConfigProperties.OCP_PROJECT_DBZ, certificateCreator.generateKeyStore(SERVER_CERT_NAME), TRUSTSTORE_CONFIGMAP, TRUSTSTORE_SUBPATH, ocp);

        LOGGER.info("Creating certificate configmaps for mongo database");
        CertUtil.stringToConfigMap(ConfigProperties.OCP_PROJECT_MONGO, CertUtil.exportCertificateToMongoCompatiblePem(certificateCreator.getLeafCertificateWrapper(SERVER_CERT_NAME), certificateCreator.getCa()), SERVER_CERT_CONFIGMAP, SERVER_CERT_SUBPATH, ocp);
        CertUtil.stringToConfigMap(ConfigProperties.OCP_PROJECT_MONGO, CertUtil.exportToBase64PEMString(certificateCreator.getCa().getHolder()), CA_CERT_CONFIGMAP, CA_CERT_SUBPATH, ocp);
    }
