...
public class CertUtil {
    ...
    /**
     * Creates a configMap in cluster containing single file with given data
    ...
    /**
     * Converts keystore to base64 string and uploads it to ocp cluster in configmap
     * @param project namespace, where to create the configmap
    ...
    public static X509Certificate convertHolderToCert(X509CertificateHolder holder) throws CertificateException {
        JcaX509CertificateConverter converter = new JcaX509CertificateConverter();
        converter.setProvider(new BouncyCastleProvider());
        return converter.getCertificate(holder);
    }

    public static String exportToBase64PEMString(X509Certificate x509Cert) throws IOException {
        StringWriter sw = new StringWriter();
        try (JcaPEMWriter pw = new JcaPEMWriter(sw)) {
