        ...
        private JdbcKafkaSinkRecord buildCreateSinkRecord() {
            ...
            if (!flat) {
                ...
                return new JdbcKafkaSinkRecord(
                        new SinkRecord(topicName, partition, keySchema, key, envelope.schema(), payload, offset),
                        config);
            }
            else {
                return new JdbcKafkaSinkRecord(
                        new SinkRecord(topicName, partition, keySchema, key, recordSchema, after, offset),
                        config);
            }
        }
