...
class AbstractChangeEventSinkTest {
    ...
    void setUp() {
        ...
        when(config.getKeyedMessageBatchMode()).thenReturn(SinkConnectorConfig.KeyedMessageBatchMode.DEDUPLICATION);
    ...
    void keyedPassthroughShouldPreserveAllEvents() {
        when(config.getBatchSize()).thenReturn(500);
        when(config.getPrimaryKeyMode()).thenReturn(SinkConnectorConfig.PrimaryKeyMode.RECORD_KEY);
        when(config.getKeyedMessageBatchMode()).thenReturn(SinkConnectorConfig.KeyedMessageBatchMode.PASSTHROUGH);
        sink = new TestChangeEventSink(config);
