...
public class SqlServerChangeEventSourceFacade implements ChangeEventSourceFacade<SqlServerTaskContext> {
    ...
    public void start() {
        ...
        final Configuration jdbcConfig = config.filter(
                x -> !(x.startsWith(DatabaseHistory.CONFIGURATION_FIELD_PREFIX_STRING) || x.equals(
        ...
        coordinator = new ChangeEventSourceCoordinator(
                ...
                new SqlServerChangeEventSourceFactory(connectorConfig, dataConnection, metadataConnection, dispatcher,
                        clock, schema),
                dispatcher,
                schema);
