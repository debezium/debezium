...
public class MultiTaskWindowHandlerTest {
    ...
    public void initTestState() {
        Assertions.assertFalse(new MultiTaskWindowHandler(10, 3, 0).started);
        Assertions.assertTrue(new MultiTaskWindowHandler(new BsonTimestamp(100, 0), 10, 3, 0).started);
    }
    ...
    private void testScenario(int start, int finalStop, int hop, int tasks, int taskId) {
        ...
        Assertions.assertEquals((int) ceil((double) (finalStop - start) / (double) (hop * tasks)), hops);
    }
