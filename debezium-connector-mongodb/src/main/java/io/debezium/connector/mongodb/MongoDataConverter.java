...
case JAVASCRIPT_WITH_SCOPE: SchemaBuilder jswithscope = SchemaBuilder.struct();
...
case REGULAR_EXPRESSION: SchemaBuilder regexwop = SchemaBuilder.struct();
...
case ARRAY: if (keyValuesforSchema.getValue().asArray().isEmpty()) {
...
case NULL:log.warn("Data type {} not currently supported", valueType);
break;
...
case DOCUMENT: SchemaBuilder documentSchemaBuilder = SchemaBuilder.struct();
