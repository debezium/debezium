...
public class TableSnapshotWorker<P extends Partition, T extends DataCollectionId> {
    ...
    public boolean readOneChunk(JdbcConnection conn) throws SQLException {
        // The connection is borrowed from the pool per round; swap it in for
        // the duration of this call so the existing helpers can use it.
        final JdbcConnection previous = this.connection;
        this.connection = conn;
        try {
            return readOneChunkInternal();
        }
        finally {
    ...
    private boolean readOneChunkInternal() throws SQLException {
        ...
        if (context.maximumKey().isEmpty()) {
            Object[] maximumKey = readMaximumKey();
            if (maximumKey == null) {
        ...
        boolean hasData = retryPolicy.executeWithRetry(
    ...
    private Object[] readMaximumKey() throws SQLException {
        final TableId tableId = (TableId) context.currentDataCollectionId().getId();
        try {
            return retryPolicy.executeWithRetry(
                    () -> connection.queryAndMap(
                            ...
                            rs -> rs.next()
                                    ? keyFromRow(connection.rowToArray(currentTable, rs,
