...
public interface TableMappingStorage<V> extends AutoCloseable {
    ...
    enum Type {
        ...
        Type(final String configPrefix) {
            this.configPrefix = STORAGE_PREFIX + configPrefix;
        }

        /**
         * Returns the configuration prefix for this storage type.
         * Used to construct configuration keys like "memory.management.{prefix}.rocksdb.path".
         *
     ...
     * @param type the type of storage (schemas or tables)
     */
    void configure(RelationalDatabaseConnectorConfig config, boolean tableIdCaseInsensitive, Type type);
