...
class LoggingContextTest {
    ...
    void shouldInheritRootMdcContextInNewThread() {
        ...
        // when
        try (final var rootContext = LoggingContext.initRootContext()) {
    ...
    void shouldCreateNestedRootContext() {
        // given
        MDC.put("A", "1");
        try (final var ctx1 = LoggingContext.initRootContext()) {
            MDC.put("B", "2");
            assertThat(RootLoggingContext.valueCopy()).isEqualTo(Map.of("A", "1"));
            try (final var ctx2 = LoggingContext.initRootContext()) {
                MDC.put("C", "3");
                assertThat(RootLoggingContext.valueCopy()).isEqualTo(Map.of("A", "1", "B", "2"));
                try (final var ctx3 = LoggingContext.initRootContext()) {
