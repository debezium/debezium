...
public class SignalsIT extends AbstractAsyncEngineConnectorTest {
    ...
    public void shouldLogPendingTransactionsViaSignal() throws Exception {
        ...
        // Wait on the per-transaction line rather than the summary line, as they are logged separately and the
        // assertion would otherwise race with the streaming thread between the two log statements.
        Awaitility.await()
                .atMost(Duration.ofSeconds(30))
                .until(() -> logInterceptor.messageMatches(
                        "Active transaction [0-9a-f.]+: startScn=\\d+, changeTime=.+, userName=.+, clientId=.+, redoThread=\\d+, events=1"));

        assertThat(logInterceptor.containsMessage("Pending transactions in Oracle LogMiner buffer as requested by signal 'log-tx-1'"))
                .isTrue();
