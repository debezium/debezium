...
public class DropTransactionAction<P extends Partition> implements SignalAction<P> {
    ...
    private BufferedLogMinerStreamingChangeEventSource getStreamingSource() {
        final Optional<? extends io.debezium.pipeline.source.spi.StreamingChangeEventSource<?, ?>> opt = changeEventSourceCoordinator.getStreamingSource();
        if (opt.isEmpty()) {
            return null;
        }

        final Object streamingSource = opt.get();
        if (!(streamingSource instanceof BufferedLogMinerStreamingChangeEventSource)) {
            throw new DebeziumException(
                    String.format("Signal '%s' is only supported by BufferedLogMinerStreamingChangeEventSource, but found %s",
                            NAME, streamingSource.getClass().getSimpleName()));
        }

        return (BufferedLogMinerStreamingChangeEventSource) streamingSource;
    }
}
