...
public class ExtendedStringBeginEventAdapter extends DmlEventAdapter {
     ...
     * @param oldValues the before state array
     * @param newValues the after state array
     ...
     */
    @ProtoFactory
    public ExtendedStringBeginEvent factory(int eventType, String scn, String tableId, String rowId, String rsId, String changeTime,
                                            String[] oldValues, String[] newValues, String columnName) {
        return new ExtendedStringBeginEvent(EventType.from(eventType), Scn.valueOf(scn), TableId.parse(tableId), rowId, rsId,
                Instant.parse(changeTime), oldValues, newValues, columnName);
    }
