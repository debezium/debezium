...
public class ChronicleLogMinerTransactionCache extends AbstractLogMinerTransactionCache<ChronicleTransaction> {
    ...
    // TODO: Currently a trade-off here. More heap but easier to manage. Maybe should look at Chronicle Map?
    private final Map<String, ChronicleQueueWrapper> chronicleQueueByTx = new ConcurrentHashMap<>();
    private final Map<String, ConcurrentHashMap<Integer, Long>> chronicleIndexByTx = new ConcurrentHashMap<>();
    private final Map<String, ConcurrentSkipListSet<Integer>> spilledEventKeyTombstonesByTx = new ConcurrentHashMap<>();
    private final Map<String, ConcurrentSkipListSet<Integer>> eventIdsByTransactionId = new ConcurrentHashMap<>();
    private final Map<String, ChronicleTransaction> transactions = new ConcurrentHashMap<>();
    ...
    public void addTransaction(ChronicleTransaction transaction) {
        ...
        eventIdsByTransactionId.put(txId, new ConcurrentSkipListSet<>());
    }
    ...
    public void forEachEvent(ChronicleTransaction transaction,
        ...
        ConcurrentSkipListSet<Integer> tombstones = spilledEventKeyTombstonesByTx.get(txId);
        if (tombstones == null) {
    ...
    public void addTransactionEvent(ChronicleTransaction transaction, int eventKey, LogMinerEvent event) {
        String txId = transaction.getTransactionId();
        ConcurrentSkipListSet<Integer> eventSet = eventIdsByTransactionId.get(txId);
        if (eventSet == null) {
    ...
    public boolean removeTransactionEventWithEventKey(ChronicleTransaction transaction, int eventKey) {
        String txId = transaction.getTransactionId();
        ConcurrentSkipListSet<Integer> tombstoneSet = spilledEventKeyTombstonesByTx.computeIfAbsent(txId, id -> new ConcurrentSkipListSet<>());
    ...
    private void spillEventToChronicleQueue(String txId, int eventKey, LogMinerEvent event) {
        ...
        try {
            ...
            chronicleIndexByTx.computeIfAbsent(txId, k -> new ConcurrentHashMap<>()).put(eventKey, chronicleIndex);
        }
        catch (Exception e) {
    ...
    private Integer findLatestSpilledEventKey(String txId, String rowId) {
        ...
        ConcurrentSkipListSet<Integer> tombstones = spilledEventKeyTombstonesByTx.computeIfAbsent(txId, k -> new ConcurrentSkipListSet<>());
        try (ExcerptTailer tailer = queueWrapper.createTailer()) {
