...
public class DmlEventAdapter extends LogMinerEventAdapter {
     ...
     * @param oldValues array of before state
     * @param newValues array of after state
    ...
    public DmlEvent factory(int eventType, String scn, String tableId, String rowId, String rsId, String changeTime, String[] oldValues, String[] newValues) {
        return new DmlEvent(EventType.from(eventType), Scn.valueOf(scn), TableId.parse(tableId), rowId, rsId, Instant.parse(changeTime),
                ObjectArrayMarshaller.stringArrayToObjectArray(oldValues), ObjectArrayMarshaller.stringArrayToObjectArray(newValues));
    }
     ...
     * @return the before state value array
     */
    @ProtoField(number = 7)
     ...
     *
     * @param event the event instance, must not be {@code null}
     * @return the after state value array
     */
    @ProtoField(number = 8)
