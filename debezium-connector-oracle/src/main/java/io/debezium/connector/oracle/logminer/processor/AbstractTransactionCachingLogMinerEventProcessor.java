...
public abstract class AbstractTransactionCachingLogMinerEventProcessor<T extends Transaction> extends AbstractLogMinerEventProcessor<T> implements CacheProvider<T> {
    ...
    private static AbstractTransactionCachingLogMinerEventProcessor instance;
    ...
    protected AbstractTransactionCachingLogMinerEventProcessor(
        ...
        AbstractTransactionCachingLogMinerEventProcessor.instance = this;
    }
    ...
    public static void logCacheStats() {
        if (instance != null) {
            AbstractTransactionCachingLogMinerEventProcessor.instance.displayCacheStatistics();
        }
        else {
    ...
    protected Scn getTransactionCacheMinimumScn() {
        final AtomicReference<Scn> minimumScn = new AtomicReference<>(Scn.NULL);

        getTransactionCache().forEach((k, v) -> {
            final Scn transactionScn = v.getStartScn();
            minimumScn.updateAndGet(o -> o.compareTo(transactionScn) < 0 ? transactionScn : o);
        });
        return minimumScn.get();
    }

    protected T compareOldest(T first, T second) {
    ...
    protected Optional<T> getOldestTransactionInCache() {
        AtomicReference<T> oldestTransaction = new AtomicReference<>(null);

        getTransactionCache().forEach((k, transaction) -> {

            T finalTransaction = transaction;
            oldestTransaction.updateAndGet(t -> t == null ? finalTransaction : compareOldest(t, finalTransaction));

        });

        return Optional.ofNullable(oldestTransaction.get());
    }

    @Override
    protected String getFirstActiveTransactionKey() {
        return getTransactionCache().first().map(LogMinerCache.Entry::getKey).orElse(null);
    }
