...
public class CompositeTransactionCache<T extends Transaction> extends AbstractLogMinerTransactionCache<T> {
    ...
    private static class SpilloverRowIdIndex {
        private final ConcurrentHashMap<String, ConcurrentLinkedDeque<Integer>> rowIndex = new ConcurrentHashMap<>();
            ...
            this.insertionOrder = new LinkedHashMap<IndexKey, Integer>(16, 0.75f, false) {
                ...
                protected boolean removeEldestEntry(Map.Entry<IndexKey, Integer> eldest) {
                    if (size() > maxSize) {
                        ...
                        ConcurrentLinkedDeque<Integer> eventKeys = rowIndex.get(key.rowId);
                        if (eventKeys != null) {
        ...
        ConcurrentHashMap<String, ConcurrentLinkedDeque<Integer>> getRowIndex() {
            return rowIndex;
        }
        ...
        void addEntry(String rowId, Integer eventKey) {
            rowIndex.computeIfAbsent(rowId, k -> new ConcurrentLinkedDeque<>()).addLast(eventKey);
    ...
    private final Map<String, T> transactionRegistry = new ConcurrentHashMap<>();
    private final Map<String, TransactionState> transactionStates = new ConcurrentHashMap<>();
    private final Map<String, SpilloverRowIdIndex> spilloverRowIdIndexes = new ConcurrentHashMap<>();
    ...
    public void forEachEvent(T transaction, InterruptiblePredicate<LogMinerEvent> predicate)
        ...
        // Track if predicate has requested early termination
        ...
        if (shouldContinue.get() && isSpilling(transactionId)) {
    ...
    private boolean tryRemoveFromSpilledIndex(T transaction, SpilloverRowIdIndex index, String rowId) {
        ConcurrentLinkedDeque<Integer> eventKeys = index.getRowIndex().get(rowId);
        if (eventKeys == null) {
            return false;
        }
