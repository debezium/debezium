...
public class UnchangedToastedPlaceholder {
    ...
    public Optional<List<Object>> getArrayValue(Schema elementSchema) {
        switch (elementSchema.type()) {
            ...
            // no BOOLEAN: every placeholder byte is non-zero, so the placeholder would be an all-true list
            // that no placeholder choice can make distinguishable from data; deliberately left on the
            // existing path until debezium/dbz#1601 settles a representation
            case STRUCT:
