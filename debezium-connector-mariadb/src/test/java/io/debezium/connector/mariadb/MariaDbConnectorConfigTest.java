...
public class MariaDbConnectorConfigTest {
    ...
    void shouldDefaultToUsingGtidOnRecovery() {
        ...
        assertThat(new MariaDbConnectorConfig(config).shouldIgnoreGtidOnRecovery()).isFalse();
    }
    ...
    void shouldIgnoreGtidOnRecoveryWhenConfigured() {
        final Configuration config = Configuration.create()
                ...
                .with(MariaDbConnectorConfig.SERVER_ID, 18765)
                .with(MariaDbConnectorConfig.TOPIC_PREFIX, "mariadb-server")
                .with(MariaDbConnectorConfig.IGNORE_GTID_ON_RECOVERY, true)
                .build();

        assertThat(new MariaDbConnectorConfig(config).shouldIgnoreGtidOnRecovery()).isTrue();
    }
    ...
    void validateLogPositionShouldBypassGtidCheckWhenIgnoreGtidOnRecoveryIsEnabled() {
        ...
        final MariaDbConnectorConfig connectorConfig = new MariaDbConnectorConfig(config);
        ...
        when(offsetContext.gtidSet()).thenReturn("0-1-100");
        ...
        final Map<String, ?> offsets = Map.of(
                BinlogOffsetContext.GTID_SET_KEY, "0-1-100",
