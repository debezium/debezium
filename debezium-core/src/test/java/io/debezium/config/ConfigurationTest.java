...
public class ConfigurationTest {
    ...
    public void testMsgKeyColumnsField() {
        ...
        assertThat(errorList.get(0))
                .isEqualTo(Field.validationOutput(MSG_KEY_COLUMNS, "t1,t2 has an invalid format (expecting '^\\s*([^:]+):([^:\\s,]+(,[^:\\s,]+)*)\\s*$')"));
    }
    ...
    public void testMsgKeyColumnsFieldRegexValidation() {
        ...
        errorList = config.validate(Field.setOf(MSG_KEY_COLUMNS)).get(MSG_KEY_COLUMNS.name()).errorMessages();
        assertThat(errorList.get(0))
                .isEqualTo(Field.validationOutput(MSG_KEY_COLUMNS, "t3.C1 has an invalid format (expecting '^\\s*([^:]+):([^:\\s,]+(,[^:\\s,]+)*)\\s*$')"));

        // field : invalid format
        config = Configuration.create().with(MSG_KEY_COLUMNS, "t1:C1;(.*).t2:C1,C2;t3;").build();
        errorList = config.validate(Field.setOf(MSG_KEY_COLUMNS)).get(MSG_KEY_COLUMNS.name()).errorMessages();
        assertThat(errorList.get(0))
                .isEqualTo(Field.validationOutput(MSG_KEY_COLUMNS, "t3 has an invalid format (expecting '^\\s*([^:]+):([^:\\s,]+(,[^:\\s,]+)*)\\s*$')"));

        // field : invalid format
        config = Configuration.create().with(MSG_KEY_COLUMNS, "t1:C1;foobar").build();
        errorList = config.validate(Field.setOf(MSG_KEY_COLUMNS)).get(MSG_KEY_COLUMNS.name()).errorMessages();
        assertThat(errorList.get(0))
                .isEqualTo(Field.validationOutput(MSG_KEY_COLUMNS, "foobar has an invalid format (expecting '^\\s*([^:]+):([^:\\s,]+(,[^:\\s,]+)*)\\s*$')"));
        ...
        config = Configuration.create().with(MSG_KEY_COLUMNS, "inventory.customers:pk1,pk2;dbo.My Table:id;(.*).purchaseorders:pk3,pk4").build();
        assertThat(config.validate(Field.setOf(MSG_KEY_COLUMNS)).get(MSG_KEY_COLUMNS.name()).errorMessages()).isEmpty();
    }
