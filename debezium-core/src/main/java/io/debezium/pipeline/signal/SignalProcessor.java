...
public class SignalProcessor<P extends Partition, O extends OffsetContext> {
    ...
    public SignalProcessor(Class<? extends SourceConnector> connector,
        ...
        this.signalProcessorExecutor = Threads.newSingleThreadScheduledExecutor(connector, config.getLogicalName(), SignalProcessor.class.getSimpleName(), config.connectorName(), config.getConnectorThreadNamePattern(), config.getTaskId(), false);
