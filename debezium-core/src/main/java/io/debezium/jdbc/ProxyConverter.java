...
public enum ProxyConverter {
    ...
    NULL(new int[]{ Types.NULL }, (column, conf) -> {
        LoggerFactory.getLogger(ProxyConverter.class.getName()).warn("Unexpected JDBC type: NULL");
        return null;
    }, null),
    BIT(new int[]{ Types.BIT }, (column, conf) -> column.length() > 1 ? Bits.builder(column.length()) : null,
            (column, field) -> (data, conf) -> ConverterHelper.convertBits(column, field, conf.byteOrder)),
    ...
    BINARY(new int[]{ Types.BLOB, Types.BINARY, Types.VARBINARY, Types.LONGVARBINARY }, (column, conf) -> conf.binaryMode.getSchema(),
            (column, field) -> (data, conf) -> ConverterHelper.convertBinary(column, field, data, conf.binaryMode)),
    ...
    SMALLINT(new int[]{ Types.TINYINT }, (column, conf) -> SchemaBuilder.int16(),
            (column, field) -> (data, conf) -> ConverterHelper.convertSmallInt(column, field, data)),
    ...
    NUMERIC(new int[]{ Types.NUMERIC }, (column, conf) -> SpecialValueDecimal.builder(conf.decimalMode, column.length(), column.scale().get()),
            (column, field) -> (data, conf) -> ConverterHelper.convertNumeric(column, field, data, conf.decimalMode)),
    DECIMAL(new int[]{ Types.DECIMAL }, (column, conf) -> SpecialValueDecimal.builder(conf.decimalMode, column.length(), column.scale().get()),
            (column, field) -> (data, conf) -> ConverterHelper.convertDecimal(column, field, data, conf.decimalMode)),
    ...
    TIMESTAMP(new int[]{ Types.TIMESTAMP }, ColumnBuilderHelper::timestampBuilder,
            (column, field) -> (data, conf) -> ConverterHelper.convertTimestamp(column, field, conf, data)),
    TIME_WITH_TIMEZONE(new int[]{ Types.TIME_WITH_TIMEZONE }, (column, conf) -> ZonedTime.builder(),
            (column, field) -> (data, conf) -> ConverterHelper.convertTimeWithZone(column, field, conf, data)),
    TIMESTAMP_WITH_TIMEZONE(new int[]{ Types.TIMESTAMP_WITH_TIMEZONE }, (column, conf) -> ZonedTimestamp.builder(),
            (column, field) -> (data, conf) -> ConverterHelper.convertTimestampWithZone(column, field, conf, data)),
