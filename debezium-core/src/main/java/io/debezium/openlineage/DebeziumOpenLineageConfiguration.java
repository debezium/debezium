...
public record DebeziumOpenLineageConfiguration(boolean enabled, Config config, Job job) {
    ...
    public static DebeziumOpenLineageConfiguration from(Configuration configuration) {
        ...
        Map<String, String> tags = configuration.getList(OPENLINEAGE_INTEGRATION_JOB_TAGS, LIST_SEPARATOR, pair -> pair.split(KEY_VALUE_SEPARATOR))
                .stream()
                .collect(Collectors.toMap(pair -> pair[0].trim(), pair -> pair[1].trim()));

        Map<String, String> owners = configuration.getList(OPENLINEAGE_INTEGRATION_JOB_OWNERS, LIST_SEPARATOR, pair -> pair.split(KEY_VALUE_SEPARATOR))
                .stream()
                .collect(Collectors.toMap(pair -> pair[0].trim(), pair -> pair[1].trim()));

        return new DebeziumOpenLineageConfiguration(configuration.getBoolean(OPENLINEAGE_INTEGRATION_ENABLED),
                new Config(configuration.getString(OPENLINEAGE_INTEGRATION_CONFIG_FILE_PATH)),
                new Job(
                        configuration.getString(OPENLINEAGE_INTEGRATION_JOB_NAMESPACE, configuration.getString(CommonConnectorConfig.TOPIC_PREFIX)),
                        configuration.getString(OPENLINEAGE_INTEGRATION_JOB_DESCRIPTION),
                        tags,
                        owners));
    }
}
