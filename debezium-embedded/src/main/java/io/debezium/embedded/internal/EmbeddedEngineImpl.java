...
public class EmbeddedEngineImpl implements EmbeddedEngine {
    ...
    private List<SourceRecord> changeRecords;
            ...
            try {
                ...
                try {
                    try {
                        ...
                        while (runningThread.get() != null && handleError == null && keepProcessing) {
                            keepProcessing = !pollBatch();
                        }
                    } finally {
                        if (handleError != null) {
                            // There was an error in the handler so make sure it's always captured...
                            fail("Stopping connector after error in the application's handler method: " + handleError.getMessage(),
                                 handleError);
                        }
                        doCompleteTask(handleError);
                    }
                } catch (Throwable t) {
    ...
    /**
     * Tries to poll a connector for a List of events and process the whole list a single batch.
     *
    ...
    private void readRecordsFromConnector() throws InterruptedException {
        logger.debug("Embedded engine is polling task for records on thread " + runningThread.get());
        changeRecords = task.poll(); // blocks until there are values ...
        logger.debug("Embedded engine returned from polling task for records");
    }
    ...
    public void doCompleteConnector() {
    ...
    public void doCompleteTask(Throwable handlerError) {
        try {
            // First stop the task ...
     ...
     * @return true if processing should be terminated
     */
    public void pollSingle() throws InterruptedException {
        try {
            if (hasRecordsToProcess()) {
                processRecord();
                return;
            }
            readRecordsFromConnector();
            if (hasRecordsToProcess()) {
                logger.debug("Received {} records from the task", changeRecords.size());
                processRecord();
            }
    ...
    private void processRecord() throws InterruptedException {
        final SourceRecord record = changeRecords.remove(0);
        try {
            consumer.accept(record);
            if (handleError != null) {
                throw new ConnectException(handleError);
            }
        }
        catch (StopConnectorException e) {
     ...
     */
    public void commitOneRecord(final SourceRecord record) throws InterruptedException {
        task.commitRecord(record);
        // Record the offset for this record's partition
        offsetWriter.offset(record.sourcePartition(), record.sourceOffset());
        recordsSinceLastCommit += 1;
