...
public interface AsyncEngineConfig extends EmbeddedEngineConfig {
    ...
    // shutdown we have to use bigger timeout. In the past DEFAULT_EXECUTOR_SHUTDOWN_TIMEOUT was 90 seconds, so we wait 3 minutes.
    // As the DEFAULT_EXECUTOR_SHUTDOWN_TIMEOUT was decreased substantially, we may need to increase it in the future if it turns out 8 seconds is too short time
    // to shut down all the tasks gracefully.
    long DEFAULT_TASK_MANAGEMENT_TIMEOUT_MS = 2 * CommonConnectorConfig.DEFAULT_EXECUTOR_SHUTDOWN_TIMEOUT.toMillis();
    ...
    Field TASK_MANAGEMENT_TIMEOUT_MS = Field.createInternal("task.management.timeout.ms")
            .withDescription("Time to wait for task's lifecycle management operations (starting and stopping), given in milliseconds. "
                    ...
                    + "which doubles CommonConnectorConfig#DEFAULT_EXECUTOR_SHUTDOWN_TIMEOUT")
