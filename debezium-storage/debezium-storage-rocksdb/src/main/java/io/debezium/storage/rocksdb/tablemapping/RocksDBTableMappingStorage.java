...
/**
 * RocksDB-based implementation of {@link TableMappingStorage} for persistent, disk-backed storage.
 ...
 */
public class RocksDBTableMappingStorage<V> implements TableMappingStorage<V>, AutoCloseable {
    ...
    public V get(TableId tableId) {
        try {
            byte[] key = serializeTableId(normalizeTableId(tableId));
    ...
    @Override
    public void clear() {
        // Close and reopen to clear all data
        close();
