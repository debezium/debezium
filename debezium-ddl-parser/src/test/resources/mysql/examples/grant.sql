...
TABLE_ENCRYPTION_ADMIN, VERSION_TOKEN_ADMIN, XA_RECOVER_ADMIN, AUDIT_ABORT_EXEMPT, FIREWALL_EXEMPT, SKIP_QUERY_REWRITE, TELEMETRY_LOG_ADMIN, TP_CONNECTION_ADMIN, GROUP_REPLICATION_STREAM ON *.* TO `admin`@`localhost`;
...
GRANT GROUP_REPLICATION_STREAM ON *.* TO 'admin'@'localhost'
...
GRANT GROUP_REPLICATION_STREAM ON *.* TO 'mysqluser'@'%'
