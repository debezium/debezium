...
CREATE TABLE T1 (C POINT SRID 4326 NOT NULL);
CREATE TABLE T1 (C POINT NOT NULL SRID 4326);
CREATE TABLE T1 (C POINT NULL SRID 4326);
...
CREATE TABLE `vendor_contract` (
    ...
    `vendor_id` int DEFAULT NULL COMMENT 'Foreign key to vendor table.',
    `cont_start` date DEFAULT NULL COMMENT 'Contract start date.',
    `cont_end` date DEFAULT NULL COMMENT 'Contract end date.',
    `cont_version` varchar(50) COLLATE utf8mb4_general_ci DEFAULT NULL COMMENT 'Contract version.',
    `exclusive` enum('yes_digital','yes_physical','yes_both','no_both','y','n') COLLATE utf8mb4_general_ci DEFAULT 'no_both' COMMENT 'Yes or No indicates whether there''''s exclusivity on the contract.',
    `orchrep_name` varchar(55) COLLATE utf8mb4_general_ci DEFAULT NULL COMMENT 'Name of the Orchard rep.',
    `carve_out` varchar(255) COLLATE utf8mb4_general_ci DEFAULT NULL COMMENT 'Deprecated',
    `qualification_level` varchar(44) COLLATE utf8mb4_general_ci DEFAULT NULL COMMENT 'Qualification level of the contract.',
    `territory_carve_out` text COLLATE utf8mb4_general_ci COMMENT 'Countries that are restricted from the contract.',
    `dms_carve_out` mediumtext COLLATE utf8mb4_general_ci COMMENT 'DMS customers that are restricted from the contract.',
    `dms_master_carve_out` text COLLATE utf8mb4_general_ci COMMENT 'DMS master master that are restricted from the contract.',
    `encoding_fees_cap_back_ctlg` float DEFAULT NULL COMMENT 'Encoding fees cap for back catalogue.',
    `encoding_fees_cap_new_release` float DEFAULT NULL COMMENT 'Encoding fees cap for new releases.',
    `digital_split` decimal(6,4) DEFAULT NULL COMMENT 'Digital split between label and Orchard.',
    `physical_split` float DEFAULT NULL COMMENT 'Physical split between label and Orchard.',
    `contract_type` enum('vendor_term','per_release_term') COLLATE utf8mb4_general_ci DEFAULT 'vendor_term' COMMENT 'Type of contract. Value can be ''vendor_term'' or ''per_release_term''.',
    `release_term` int DEFAULT NULL COMMENT 'Number of years for the contract if the contract type is per_release_term.',
    `advance_payment` float DEFAULT NULL COMMENT 'Advanced payment amount.',
    `advance_recoupable_percentage` float DEFAULT NULL COMMENT 'Advanced recoupable percentage.',
    `dig_distribution_type` enum('digital_mobile','digital_only','mobile_only') COLLATE utf8mb4_general_ci DEFAULT 'digital_mobile' COMMENT 'Digital distribution type of the track. Value can be ''digital_mobile'', ''digital_only'', or ''mobile_only''.',
    `possible_track_restrictions` enum('Y','N') COLLATE utf8mb4_general_ci DEFAULT 'N' COMMENT 'Yes or No indicates whether there''s possible restrictions on tracks.',
    `oms_type` enum('none','both','orchard','label') COLLATE utf8mb4_general_ci DEFAULT 'none' COMMENT 'Type of OMS. Value can be ''none'', ''both'', ''orchard'', or ''label''.',
    `oms_fee_percentage` float DEFAULT NULL COMMENT 'OMS fee percentage.',
    `negotiated_changes` enum('Y','N') COLLATE utf8mb4_general_ci DEFAULT NULL COMMENT 'Yes or No indicates whether there''s negotiated changes.',
    `negotiated_change_comments` text COLLATE utf8mb4_general_ci COMMENT 'Negotiated change comment text.',
    `contract_complete` enum('Y','N') COLLATE utf8mb4_general_ci DEFAULT 'N' COMMENT 'Indicates if the contract has been reviewed by Legal and marked as completed.',
    `signature_date` date DEFAULT NULL COMMENT 'Date of signature.',
    `marketing_restrictions` enum('Y','N') COLLATE utf8mb4_general_ci DEFAULT 'N' COMMENT 'Indicates whether the agreement has marketing restrictions.',
    `orchard_assignment_right_restriction` enum('Y','N') COLLATE utf8mb4_general_ci DEFAULT 'N' COMMENT 'Indicates whether the agreement has orchard assignment right.',
    `currency_id` smallint unsigned DEFAULT '1' COMMENT 'Foreign key to currency table.',
    `third_party_responsibility` enum('standard','other') COLLATE utf8mb4_general_ci DEFAULT 'standard' COMMENT 'Indicates whether the agreement has third party responsibility.',
    `third_party_responsibility_detail` text COLLATE utf8mb4_general_ci COMMENT 'Detail of third party responsibility.',
    `possible_track_restriction_detail` text COLLATE utf8mb4_general_ci COMMENT 'Detail of possible track_restrictions.',
    `is_amendment` enum('Y','N') COLLATE utf8mb4_general_ci DEFAULT 'N' COMMENT 'Indicates whether the agreement is an amendment.',
    `extend_until_recouped` enum('Y','N') COLLATE utf8mb4_general_ci DEFAULT 'N' COMMENT 'Indicates whether the agreement should extend until recoupment.',
    `sync_admin_territory` text COLLATE utf8mb4_general_ci COMMENT 'Syncronization Admin Territory multi-select dropdown of countries',
    `sync_admin_commission` text COLLATE utf8mb4_general_ci COMMENT 'Sync Admin Split textbox. Enter 0.75 for 75%',
    `sync_admin_type_of_deal` enum('master_admin','master_or_publishing_admin','publishing_admin_only') COLLATE utf8mb4_general_ci DEFAULT NULL COMMENT 'Sync Admin Type of Deal dropdown',
    `royalty_collection_territory` text COLLATE utf8mb4_general_ci COMMENT 'Royalty Collection Territory Multi-select dropdown of countries. ',
    `royalty_collection_commission` float DEFAULT NULL COMMENT 'Royalty Collection Split textbox. Enter 0.75 for 75%',
    `publishing_admin_commission` float DEFAULT NULL COMMENT 'Publishing Administration Split textbox. Enter 0.75 for 75%',
    `publishing_admin_territory` text COLLATE utf8mb4_general_ci COMMENT 'Publishing Admin Territory multi-select dropdown of countries',
    `publishing_admin_limit_grant_of_rights` varchar(50) COLLATE utf8mb4_general_ci DEFAULT NULL COMMENT 'Publishing Administration Limit Grant of Rights field',
    `publishing_admin_misc_provisions` varchar(50) COLLATE utf8mb4_general_ci DEFAULT NULL COMMENT 'Publishing Administration Misc Previsions field',
    `parent_vendor_contract_id` int DEFAULT NULL COMMENT 'Contract id of the previous version of this contract',
    `vendor_type` enum('vendor','oms_client') COLLATE utf8mb4_general_ci DEFAULT 'vendor' COMMENT 'indicate whether this contract is for Label or OMS Client',
    `vendor_proposed_term_id` int DEFAULT NULL COMMENT 'foreign key referencing vendor proposed term table',
    `sync_admin_response_time` int DEFAULT NULL COMMENT 'Sync Admin Response Time textbox',
    `ringtone_publishing_type` enum('both','label','orchard','none') COLLATE utf8mb4_general_ci DEFAULT 'none' COMMENT 'Take Ringtone Publishing dropdown',
    `physical_track_publishing_type` enum('none','both','orchard','label') COLLATE utf8mb4_general_ci DEFAULT 'none' COMMENT 'Field to store physical track publishing ',
    `unlimited_roll` enum('Y','N') COLLATE utf8mb4_general_ci DEFAULT 'N' COMMENT 'Unlimited Roll dropdown',
    `rollover_length_in_months` int DEFAULT NULL COMMENT 'Rollover Length in Months textfield',
    `can_terminate` enum('Y','N') COLLATE utf8mb4_general_ci DEFAULT 'N' COMMENT 'Can Terminate before Rollover Ends dropdown',
    `youtube_composition_clause` enum('N','Y') COLLATE utf8mb4_general_ci DEFAULT 'N',
    `sx_royalty_collection_commission` float DEFAULT '0',
    `topspin_rate` float DEFAULT '0',
    `topspin_rate_territory` text COLLATE utf8mb4_general_ci,
    `notice_required_in_days` int DEFAULT NULL COMMENT 'Notice Required in Days textbox',
    `term_continues_until_recouped` enum('Y','N') COLLATE utf8mb4_general_ci DEFAULT 'N' COMMENT 'Term Continues until Recouped dropdown',
    `brand_split` float DEFAULT NULL COMMENT 'Label''s Split - BRAND textfield in edit vendor contract page',
    `other_rights_option` enum('any_and_all','other','none') COLLATE utf8mb4_general_ci DEFAULT 'any_and_all' COMMENT 'Other rights',
    `other_rights_text` varchar(50) COLLATE utf8mb4_general_ci DEFAULT NULL COMMENT 'Other rights text for ""other"" option',
    `special_product_split` float DEFAULT NULL,
    `special_product_carve_out` text COLLATE utf8mb4_general_ci,
    `payment_interval` enum('month','quarter') COLLATE utf8mb4_general_ci DEFAULT 'quarter' COMMENT 'How often the payment is due',
    `pay_after` enum('30','45','60','90') COLLATE utf8mb4_general_ci DEFAULT '45' COMMENT 'Days before payment is due',
    `show_credit_card` enum('N','Y') COLLATE utf8mb4_general_ci DEFAULT 'N',
    `opt_out` enum('Y','N') COLLATE utf8mb4_general_ci DEFAULT 'N' COMMENT 'Yes or No indicates whether or not deliver content to stores. Default N indicates, opt out is false and content can be delivered',
    `apply_fx_spread` enum('Y','N') CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci DEFAULT 'N' COMMENT 'Yes OR NO indicates whether or not to apply FX Spread calculations during the monthly accounting process',
    `reserve_rate` decimal(4,2) NOT NULL DEFAULT '0.00',
    `number_of_months_before_payout` tinyint unsigned DEFAULT '0',
    `number_of_installments` tinyint unsigned DEFAULT '0',
    `contract_terms` text COLLATE utf8mb4_general_ci,
    `service_type_id` int unsigned DEFAULT NULL COMMENT 'Service type for the contract',
    `is_automatic_rollover` enum('Y','N') COLLATE utf8mb4_general_ci DEFAULT NULL,
    PRIMARY KEY (`id`),
    KEY `vendor_id` (`vendor_id`),
    KEY `FK_service_type` (`service_type_id`),
    CONSTRAINT `FK_service_type` FOREIGN KEY (`service_type_id`) REFERENCES `service_type` (`id`) ON DELETE RESTRICT ON UPDATE RESTRICT
) ENGINE=InnoDB AUTO_INCREMENT=566242 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci COMMENT='Holds all contracts of labels'
#end
