 ...
 */
public abstract class BinlogSchemaMigrationIT<C extends SourceConnector> extends AbstractBinlogConnectorIT<C> {

    private static final Path SCHEMA_HISTORY_PATH = Files.createTestingPath("file-schema-history-json.txt")
            .toAbsolutePath();
    private UniqueDatabase DATABASE = TestHelper.getUniqueDatabase("migration", "empty")
    ...
    public void shouldCorrectlyMigrateTable() throws SQLException, InterruptedException {
        ...
        config = DATABASE.defaultConfig()
                ...
                .with(BinlogConnectorConfig.TABLE_INCLUDE_LIST, DATABASE.qualifiedTableName("monitored") + "," + DATABASE.qualifiedTableName("_monitored_new"))
                .build();

        final BinlogTestConnection connection = getTestDatabaseConnection(DATABASE.getDatabaseName());
    ...
    public void shouldProcessAndWarnOnNonWhitelistedMigrateTable() throws SQLException, InterruptedException {
        ...
        config = DATABASE.defaultConfig()
                .with(BinlogConnectorConfig.SNAPSHOT_MODE, BinlogConnectorConfig.SnapshotMode.INITIAL)
                .with(BinlogConnectorConfig.INCLUDE_SCHEMA_CHANGES, false)
                .with(BinlogConnectorConfig.TABLE_INCLUDE_LIST, DATABASE.qualifiedTableName("monitored"))
                .build();
    ...
    protected abstract Class<?> getRenameTableParserListenerClass();
}
