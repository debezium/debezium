...
public interface Configuration {
    ...
    Pattern PASSWORD_PATTERN = Pattern.compile(
            ".*secret$|.*password$|.*token$|.*sasl\\.jaas\\.config$|.*basic\\.auth\\.user\\.info|.*registry\\.auth\\.client-secret|.*credentials\\.json$|.*connectionstring$|.*connection\\.string$",
    ...
    interface ConfigBuilder<C extends Configuration, B extends ConfigBuilder<C, B>> {
         ...
         * @return this builder object so methods can be chained together; never null
         */
        default B withDefault(Field field, EnumeratedValue value) {
            return withDefault(field.name(), value);
        }
