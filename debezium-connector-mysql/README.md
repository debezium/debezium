...
All of the above commands will start the MySQL Docker container that is built upon the [container-registry.oracle.com/mysql/community-server:8.1](https://container-registry.oracle.com/ords/ocr/ba/mysql/community-server/) Docker image maintained by the MySQL team. This image has an "optimized" MySQL server that includes only a portion of the full installation (e.g., it excludes some tools such as `mysqlbinlog`). However, it starts a little faster and is less verbose in its output.
