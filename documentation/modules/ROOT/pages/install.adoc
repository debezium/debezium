...
* https://repo1.maven.org/maven2/io/debezium/debezium-connector-informix/{debezium-version}/debezium-connector-ingres-{debezium-version}-plugin.tar.gz[Ingres plugin archive] (incubating)
...
Our xref:kc-tutorial.adoc[tutorial] even walks you through using these images, and this is a great way to learn what {prodname} is all about.
...
To install the operator, you can create a dedicated namespace and use the {prodname} Helm chart repository:
...
For detailed instructions on deploying the operator, tracking its progress, and configuring `DebeziumServer` custom resources, see the xref:operations/debezium-operator.adoc[{prodname} Operator documentation].
