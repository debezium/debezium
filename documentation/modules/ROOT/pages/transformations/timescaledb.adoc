...
To configure the replication slot to support logical decoding, specify the following setting in the `postgresql.conf` file:
...
You must restart the database server for the setting to take effect.
...
`"database.hostname": "192.168.99.100",`::
The address of the TimescaleDB server.

`"database.port": "5432",`::
The port number of the TimescaleDB server.

`"database.user": "postgres",`::
The name of the TimescaleDB user.

`"database.password": "postgres",`::
The password for the TimescaleDB.

`"database.dbname" : "postgres",`::
The name of the TimescaleDB database to connect to.

`"topic.prefix": "dbserver1",`::
The topic prefix for the TimescaleDB server or cluster.
...
`"plugin.name": "pgoutput",`::
Indicates use of the `pgoutput` logical decoding plug-in.

`"schema.include.list": "_timescaledb_internal",`::
A list of all schemas that contain TimescaleDB physical tables.

`"transforms": "timescaledb",`::
Enables the SMT to process raw TimescaleDB events.

`"transforms.timescaledb.type": "io.debezium.connector.postgresql.transforms.timescaledb.TimescaleDb",`::
Enables the SMT to process raw TimescaleDB events.

`"transforms.timescaledb.database.hostname": "timescaledb",`::
This line and the lines that follow provide the SMT with TimescaleDB connection information, including the hostname, port number, database user, database password, and database name.
These values must match the values in the preceding `database.hostname`, `database.port`, `database.user`, `database.password`, and `database.dbname` fields.
The
