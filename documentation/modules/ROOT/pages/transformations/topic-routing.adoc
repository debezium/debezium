...
----
transforms=Reroute
transforms.Reroute.type=io.debezium.transforms.ToLogicalTopicRouter
...
To ensure that each event key is unique across the tables whose change event records go to the same topic, the topic routing transformation inserts a field into change event keys. By default, the name of the inserted field is `+__dbz__sourceIdentifier+`. The value of the inserted field is the default destination topic name.
...
|Name of a field to be added to the change event key. The value of this field identifies the source from which the record originated. For the SMT to add this field, `key.enforce.uniqueness` must be `true`, which is the default.
...
|[[by-logical-table-router-logical-destination-cache-size]]xref:by-logical-table-router-logical-destination-cache-size[`logical.destination.cache.size`]
|`16`
|The size used for holding the max entries in LRUCache. The cache will keep the old/new schema for logical destination key and value, also cache the derived key and topic regex result for improving the source record transformation.
|===
