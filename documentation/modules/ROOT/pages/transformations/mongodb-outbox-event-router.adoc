...
The outbox pattern provides a way to safely and reliably exchange data between multiple microservices.
An outbox pattern implementation ensures that the data that a service stores in its database remains consistent with the data that {prodname} publishes for other services to consume.
