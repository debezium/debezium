...
Because the connector discards the increment component that MongoDB uses to order operations that occur during the same second, distinct BSON values might result in the same output value.
...
====
When you set the value of this property to `struct` and set `array.encoding=array`, the SMT cannot process arrays that contain both BSON Timestamp values and BSON date and time values, because the schemas of these types are not compatible.
If an array mixes both types, the SMT returns the following error:

`Array elements with different Bson types are not supported`.

As a workaround, set the following property to enable the connector to capture arrays that contain mixed types: `array.encoding=document`.
====
