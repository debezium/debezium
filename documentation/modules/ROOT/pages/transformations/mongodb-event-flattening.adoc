...
|`false`
|When set to `true` and `delete.tombstone.handling.mode` is `rewrite`, reads the `_id` of the deleted document from the record key and adds it to the payload for delete events.
