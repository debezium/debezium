...
+
When `bson.timestamp.handling.mode=struct` and `array.encoding=array`, the SMT cannot process arrays that contain both BSON Timestamp values and BSON Date values because it represents the two value types with different schemas.
+
To capture arrays that contain these mixed value types, set `array.encoding=document`.
