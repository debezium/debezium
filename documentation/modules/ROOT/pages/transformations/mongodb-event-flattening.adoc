...
To retain the change stream resume token, add the following property:

[source,properties]
----
transforms.unwrap.add.fields=source.resume_token
----

The resulting `__source_resume_token` field contains the token as an Extended JSON string.
Alternatively, use `transforms.unwrap.add.headers=source.resume_token` to retain it in a record header.
Snapshot records have a null token, and tombstone records have no value from which to extract metadata.
When replaying older records whose source schema does not include `resume_token`, the SMT omits the additional field or header.
...
If you specify a field that is not present in the original change event schema, the SMT omits the field from the modified message.
For example, a misspelled `source.resume_tokne` entry is omitted when the requested field does not exist in the change event schema.
If an optional field is present in the schema but its value is null, the SMT adds the field with a null value.
