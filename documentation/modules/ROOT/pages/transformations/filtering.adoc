...
[prodname] offers a community maintained helper PDK(Plugin Development Kit) to run Go, you can get it with:
...
|
|The filesystem location where the `wasm` expression is available to be evaluated for every message. Must evaluate to a Boolean value where a result of `true` keeps the message, and a result of `false` removes it.
