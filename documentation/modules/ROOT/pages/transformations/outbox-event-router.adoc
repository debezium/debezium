...
|`payload`
|The representation of the event itself. The default structure is JSON. By default, the content in this field becomes the sole content of the Kafka message value; if further fields are configured to be included in the outbox event, the Kafka message value will contain an envelope structure containing any additional field(s) and the payload in a field `payload`.
