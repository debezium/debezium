...
The {prodname} MySQL connector tracks the structure of the tables, performs snapshots, transforms binlog events into {prodname} change events as well as where those events are recorded in Kafka, and how the connector behaves when things go wrong.
