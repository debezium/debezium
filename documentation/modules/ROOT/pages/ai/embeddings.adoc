...
You must specify at least one field to use as the input for the embedding, as well as a destination field where the embedding will be placed. For example:
...
[NOTE]
====
Hugging Face supports several link:https://huggingface.co/blog/inference-providers[embedding inference providers] and link:https://huggingface.co/docs/inference-providers/en/index[external providers].
