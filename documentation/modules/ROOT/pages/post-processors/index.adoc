...
|xref:post-processors/s3-large-message.adoc[S3 large message]
|Offloads large `STRING` and `BYTES` field values from change event records to Amazon S3, replacing the in-message value with a compact inline reference.
