...
For workloads in which the same rows are reselected repeatedly (for example, TOAST or LOB columns that {prodname} emits as placeholders on every unrelated row update), you can enable an optional cache of reselected values to reduce the number of database queries.

Enable caching by setting `reselect.cache.enabled` to `true`.
Reselected values are cached per row and per column, using the event's message key as the cache key.
Before querying the database, the post processor checks the cache for each column that requires reselection, and only queries columns that are not already cached.
Eligible columns whose values are present in the event record are also written to the cache, even if no reselection is needed, so a later event that omits the same column can be served from the cache without querying the database again.
...
Because the row identity is the message key, including its schema, a change to the key's schema (for example, a DDL change or a primary-key reordering) yields a different cache key, so the post processor queries the database for the new key rather than returning a stale cached value.
...
Cached values persist after you a connector restarts, so columns that would otherwise need to be reselected continue to be served from the cache after a restart.
...
If you specify a positive `reselect.cache.ttl.ms` value, expired entries are treated as cache misses and removed when they are read.
RocksDB reclaims expired entries from disk during compaction.
...
[NOTE]
====
The time-to-live setting determines how the RocksDB database is opened, and the two modes store entries on disk in different formats.
If you change `reselect.cache.ttl.ms` between `0` and a positive value across a connector restart, entries that were written before the change can become unreadable in the new mode.
Unreadable entries are discarded and treated as cache misses, so the affected columns are transparently reselected from the database and re-cached; no manual cleanup of the storage directory is required.
====
...
ifdef::product[]
{prodname} provides the xref:value-caching-in-memory[in-memory cache].
You can set additional `reselect.cache.*` properties, as described in xref:value-caching-in-memory[In-memory cache].
endif::product[]
ifdef::community[]
{prodname} provides the xref:value-caching-in-memory[in-memory cache] (the default) and the xref:value-caching-rocksdb[RocksDB cache] (`io.debezium.storage.rocksdb.reselect.RocksDbReselectColumnCache`); each caches supports additional `reselect.cache.*` properties, as described in xref:value-caching-rocksdb[RocksDB cache].
endif::community[]
