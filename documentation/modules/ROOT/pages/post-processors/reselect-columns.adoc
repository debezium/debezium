...
|7
|Log or fail the connector in  case reselection fail or give empty record.
...
|`WARN`
|Specifies whether the post processor reselects behaviour if reselection fail or give empty record. +
 +
By default, it only logs the error in case reselection fail or empty. +
 +
You can set this property to `FAIL` to fail the connector in case of reselection fail or empty record.
