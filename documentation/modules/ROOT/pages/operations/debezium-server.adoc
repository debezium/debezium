...
Because the SDK tracks which records are unacknowledged, recovery re-sends only those, and the sink does not retry the batch itself.
An error that recovery cannot resolve still stops the connector, so the offset does not advance past it.
...
The preceding options that default to "The SDK default" are not set unless you configure them, so that the value that applies is the one that the Zerobus SDK version in use defines rather than one that this connector pins.
...
|`@CustomConsumerBuilder`
|Custom configured instance of a `ZerobusSdk` used to open the streams that carry records to the target tables.
By default, the sink builds one from the <<zerobus-endpoint, `endpoint`>> and <<zerobus-workspace-url, `workspace.url`>> options.
