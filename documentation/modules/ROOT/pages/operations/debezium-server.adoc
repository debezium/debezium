...
| using topic name when not set value
...
|[[rabbitmq-autoCreateRoutingKey]]<<rabbitmq-autoCreateRoutingKey, `debezium.sink.rabbitmq.autoCreateRoutingKey`>>
| false
|(Optional) Set auto create routing key when not exist.

|[[rabbitmq-routingKeyDurable]]<<rabbitmq-routingKeyDurable, `debezium.sink.rabbitmq.routingKeyDurable`>>
| true
|(Optional) Set existing routing key data into durable/restored due to Rabbitmq server restart.

|[[rabbitmq-routingKeyFromTopicName]]<<rabbitmq-routingKeyFromTopicName, `debezium.sink.rabbitmq.routingKeyFromTopicName`>>
| false
|(Optional) Set routing key name using topic name from debezium message.

|[[rabbitmq-deliveryMode]]<<rabbitmq-deliveryMode, `debezium.sink.rabbitmq.deliveryMode`>>
| 2
|(Optional) Config when Rabbitmq deliver message no consumer connected. 1 Non-persistent, 2 Persistent.
