...
`zerobusrest`:: Uses the Zerobus REST API.
...
|[[zerobusrest-type]]<<zerobusrest-type, `debezium.sink.type`>>
|No default value.
|Must be set to `zerobusrest`.

|[[zerobusrest-uri]]<<zerobusrest-uri, `debezium.sink.zerobusrest.uri`>>
|No default value.
|The Zerobus REST base URI, for example `https://<workspace-id>.zerobus.<region>.cloud.databricks.com`.

|[[zerobusrest-workspace-url]]<<zerobusrest-workspace-url, `debezium.sink.zerobusrest.workspace.url`>>
|No default value.
|The Databricks workspace URL.
The service uses this value for OAuth and in the `unity-catalog-endpoint` header.

|[[zerobusrest-workspace-id]]<<zerobusrest-workspace-id, `debezium.sink.zerobusrest.workspace.id`>>
|No default value.
|The numeric workspace ID that the service uses to build the Zerobus token audience.

|[[zerobusrest-client-id]]<<zerobusrest-client-id, `debezium.sink.zerobusrest.client.id`>>
|No default value.
|The OAuth client ID (service principal application ID).

|[[zerobusrest-client-secret]]<<zerobusrest-client-secret, `debezium.sink.zerobusrest.client.secret`>>
|No default value.
|The OAuth client secret for the service principal.

|[[zerobusrest-table]]<<zerobusrest-table, `debezium.sink.zerobusrest.table`>>
|No default value.
...
|[[zerobusrest-metrics-log-interval]]<<zerobusrest-metrics-log-interval, `debezium.sink.zerobusrest.metrics.log.interval`>>
|`0`
|Specifies the number of batches that the service processes before it emits a periodic `INFO` log line to summarize sink metrics.
Set the value to `0` to disable periodic logging.
This setting does not affect availability of metrics over JMX.
...
As an alternative to the `zerobus` and `zerobusrest` routes, you can reuse the built-in `kafka` sink to write to the Zerobus Kafka-compatible endpoint.
...
The {prodname} Databricks Zerobus Ingest sink supports the following delivery routes: zerobus (gRPC), zerobusrest (REST), and the <<zerobus-kafka-route, Kafka route>>.
All three routes expose sink metrics over JMX under the object name `debezium.zerobus:type=connector-metrics,context=sink,server=<route>,task=0`, where `<route>` is `grpc` for the `zerobus` route, `rest` for the `zerobusrest` route, or `kafka` for the Kafka route.
...
You can automate periodic metric logging by setting <<zerobus-metrics-log-interval, `debezium.sink.zerobus.metrics.log.interval`>> or <<zerobusrest-metrics-log-interval, `debezium.sink.zerobusrest.metrics.log.interval`>> to a positive value.
