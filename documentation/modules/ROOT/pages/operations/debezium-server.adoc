...
To use the Kafka sink with the Zerobus Kafka-compatible endpoint, configure the Kafka SASL/OAUTHBEARER authentication and set the `sasl.login.callback.handler.class` to `io.debezium.server.databricks.zerobus.ZerobusOAuthBearerLoginCallbackHandler`.
For the complete list of required OAuth properties, see the source code for the `ZerobusOAuthBearerLoginCallbackHandler` for {prodname} Server.
