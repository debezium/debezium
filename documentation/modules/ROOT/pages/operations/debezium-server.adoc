...
|
|Specifies the NKey seed to use when xref:nats-jetstream-auth-jwt[JWT authentication] is enabled.

|[[nats-jetstream-auth-user]]<<nats-jetstream-auth-user, `debezium.sink.nats-jetstream.auth.user`>>
|
|Add this property to the configuration to enable password authentication with NATS. +
