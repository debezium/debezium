...
|`extended`
|The format of the message sent to the Redis stream. Possible values are `extended`(newer format) and `compact`(the until now old format). The `extended` format looks in Redis like this:
[source]
----
1) 1) "1639304527499-0"
   2) 1) "key"
...
while the `compact` format looks like this:
[source]
----
1) 1) "1639304527499-0"
   2) 1) "{\"schema\": ..."
