...
If the recovery process cannot resolve an error, the connector stops, and the offset does not advance further.
...
The behaviour of the gRPC route can be modified by custom logic that provides an alternative implementation for a specific functionality.
When the alternative implementation is not available, the default one is used.
...
|`DECIMAL(p,s)` &mdash; emitted as an exact string, locale-independent
...
You can use JMX metrics and connector log output to monitor the operational status and performance of the Debezium DataBricks Zero sink.

The {prodname} DataBricks Zero sink supports the following delivery routes: zerobus (gRPC), zerobusrest (REST), and the <<zerobus-kafka-route, Kafka route>>.
...
A values near `0` indicates that the sink is catching up to, or is synchronized with, the source.
