...
Milvus is an open-source vector database designed for similarity search and retrieval of high-dimensional data, like embeddings from machine learning models (e.g. text, image, audio).
It is possible to either use to to process vector datatypes captured from a source database or use a transformation to calculate vector out of message field(s) and use it as an embedding.

Milvus sink taks the incoming message and upserts the `after` part into a collection.
The collection cannot contain dots so the sink replaces all dots with underscore characters.
When delete message arrives the appropriate record is removed from a collection.
...
|(Optional) A URL to access Milvus database instance.

|[[milvus-database]]<<milvus-database, `debezium.sink.milvus.database`>>
| _default_
|(Optional) The name of the database containing target collections.
...
The Milvus sink behaviour can be modified by a custom logic providing alternative implementations for specific functionalities.
When the alternative implementations are not available then the default ones are used.
...
|`@CustomConsumerBuilder`
|Custom configured instance of a `MilvusClientV2` used to access target collections.

|[[milvus-ext-stream-name-mapper]]<<milvus-ext-stream-name-mapper, `io.debezium.server.StreamNameMapper`>>
|
|Custom implementation maps the planned destination (topic) name into a Milvus collection.
By default the name with dots replaced with underscores is used.
