...
The sink uses one of the following routes to deliver event records, as specified by the `debezium.sink.type` property:
...
For information about using this option, see the `ZerobusOAuthBearerLoginCallbackHandler` shipped in this module for the required `OAUTHBEARER` login handler.
...
The sink normalizes each Kafka Connect schema value to a format that is compatible with the target Delta column, based on the logical type name, rather than on the serialized JSON representation.
This avoids the "invalid type" class of ingestion errors without requiring source-side `*.handling.mode` tuning.
...
Both routes expose sink metrics over JMX, under the object name `debezium.zerobus:type=connector-metrics,context=sink,server=<route>,task=0`, where `<route>` is `grpc` or `rest`.
This mirrors how the JDBC sink connector and {prodname} source connectors expose their metrics, so the same JMX tooling (for example JConsole, `jmxterm`, or a Prometheus JMX exporter) works out of the box.
...
You can also configure periodic metric logging by setting <<zerobus-metrics-log-interval, `debezium.sink.zerobus.metrics.log.interval`>> or <<zerobusrest-metrics-log-interval, `debezium.sink.zerobusrest.metrics.log.interval`>> to a positive value.
