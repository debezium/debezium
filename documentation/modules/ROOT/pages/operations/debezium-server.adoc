...
https://docs.databricks.com/aws/en/ingestion/zerobus[Databricks Zerobus Ingest] is a serverless, push-based ingestion service that writes records directly into Unity-Catalog-managed Delta tables, without an intermediate message broker.
The sink authenticates with a Databricks service principal using OAuth and streams each change event to the managed Delta table whose name matches the event's destination (topic).

The sink supports two delivery routes, selected by `debezium.sink.type`:

* `zerobus` &mdash; uses the Databricks Zerobus Java SDK over gRPC (recommended);
* `zerobusrest` &mdash; uses the Zerobus REST API, one HTTP request per record, for environments without a persistent connection.

A third option is to reuse the built-in `kafka` sink against the Zerobus Kafka-compatible endpoint; see the `ZerobusOAuthBearerLoginCallbackHandler` shipped in this module for the required `OAUTHBEARER` login handler.

Because Zerobus accepts only JSON objects that match the target table schema, configure {prodname} to emit flat JSON (`ExtractNewRecordState` with `schemas.enable=false`).
Delivery is at-least-once, so deduplicate downstream (for example, using the source LSN carried in the record).
The target table must be a Unity-Catalog-managed Delta table in the same region as the workspace, without Change Data Feed or column mapping, and the service principal needs `USE CATALOG`, `USE SCHEMA`, `SELECT` and `MODIFY` on it.
...
|
|(Optional) A fully-qualified default target table (`catalog.schema.table`). When set, all events are written to this table; otherwise the mapped destination (topic) name is used as the fully-qualified table name.
...
|`0`
|Emit a periodic `INFO` log line summarizing the sink metrics every N processed batches. `0` (the default) disables periodic logging; the metrics remain available over JMX regardless. See <<zerobus-monitoring, Monitoring>>.
...
|
|The Databricks workspace URL, used for OAuth and the `unity-catalog-endpoint` header.

|[[zerobusrest-workspace-id]]<<zerobusrest-workspace-id, `debezium.sink.zerobusrest.workspace.id`>>
|
|The numeric workspace id, used to build the Zerobus token audience.
...
The sink normalizes each value from the Kafka Connect schema to the shape the target Delta column expects, based on the logical type name rather than the already-serialized JSON.
...
In addition, setting `debezium.sink.zerobus[rest].metrics.log.interval` to a positive value emits a periodic `INFO` log line with the same values.
...
|Ingested events split by operation (`c` / `u` / `d`, and snapshot reads `r`).

|`TotalRecordsSkipped`
|Records skipped without being forwarded (tombstones / null payloads, and events whose destination is not a fully-qualified `catalog.schema.table`, such as schema-history or DDL events).

|`TotalErrors`
|Ingestion or flush failures observed.

|`TotalFlushes`
|Stream flushes performed (gRPC route); successful record POSTs (REST route), which are the durability boundary there.

|`MilliSecondsBehindSource`
|Pipeline freshness: `now - source.ts_ms` of the most recently ingested event, in milliseconds. `-1` until the first event with a source timestamp is ingested; approaches `0` when the sink is caught up with the source. This is the primary signal in a CDC-to-lakehouse deployment.

|`LastFlushDurationMillis` / `MaxFlushDurationMillis`
|Duration of the most recent, and the longest, flush (gRPC) or record POST (REST).

|`ActiveStreams`
|Number of currently open Zerobus streams (one per target table); always `0` for the REST route, which opens no persistent streams.
