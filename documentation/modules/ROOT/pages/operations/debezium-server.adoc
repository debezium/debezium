...
// Type: assembly
// Title: {prodname} Server (Technology Preview)
[id="debezium-server"]
= {prodname} Server
...
{prodname} Server is a ready-to-use application that streams change events from a source database directly to messaging and storage systems without requiring Apache Kafka.
You configure a source connector and a sink to define where events come from and where they are delivered.

ifdef::product[]
[IMPORTANT]
====
{prodname} Server is a Technology Preview feature.
...
// Type: procedure
// Title: {prodname} Server installation
// ModuleID: debezium-server-installation
[id="installing-debezium-server"]
== Installation

Download and unpack the {prodname} Server distribution to install the server on your system.
...
ifdef::product[]
. Open the link:{LinkDebeziumDownloads}[*{NameDebeziumDownloads}*] page on the {companyname} Customer Portal.
. From the *Releases* tab, locate *{prodname} {ProductVersion} {prodname} Server* and click *Download Now* to download the ZIP file.
. Extract the contents of the archive to the directory in which you want to install {prodname}.
endif::product[]
...
[source,subs="verbatim,attributes"]
----
debezium-server-dist-{debezium-version}-redhat-{debezium-build-number}/
|-- debezium-server
    |-- config
    |-- debezium-server-dist-{debezium-version}-redhat-{debezium-build-number}-runner.jar
    |-- jmx
    |-- lib
    |-- lib_metrics
    |-- lib_opt
...
The `lib` directory stores dependencies, and the `config` directory contains configuration files.
The `run.bat` or `run.sh` file starts the server.

Before you start the server, specify the source for the change events that you want to stream and the sink destination for those events. 
For details about how to configure {prodname} Server, see the xref:debezium-server-configuration[Configuration] section.


// Type: procedure
// ModuleID: debezium-server-start-the-server
[id="start-the-server"]
=== Start the server

Run the {prodname} Server startup script to begin streaming change events from your configured source connector to the configured sink.

.Prerequisites

* Configure {prodname} Server as described in xref:debezium-server-configuration[{prodname} Server configuration].

.Procedure

. Open a command prompt or terminal window.
. Change to the `_<ServerInstallDirectory>_/debezium-server` directory.
. Start {prodname} Server:
+
...
[NOTE]
====
To use the Oracle connector, copy the Oracle JDBC driver to the `lib` directory before you start the server.

If you use Oracle XStream, also copy the required XStream API files to the `lib` directory.
...
// Type: concept
[id="debezium-server-configuration"]
...
MicroProfile configuration is a standard for supplying configuration values to Java applications from multiple sources, including configuration files, environment variables, and system properties.

[NOTE]
====
Configuration properties that contain the `$` character are normally interpreted by Quarkus for property expansion.
If a property value includes a `$` character as a prefix, such as `++${my.property}++`, the Microprofile configuration parser determines that the value is a https://quarkus.io/guides/config-reference#property-expressions[property expression] and attempts to expand it.

To prevent the parser from attempting to expand values that use this syntax, escape the `$` prefix with a second `$` character; that is, provide the value as `++$${my.property}++`.
The value of the property is then passed to {prodname} as `++${my.property}++`.
====
...
debezium.sink.http.url=https://example.com/events
endif::product[]
...
The configuration file in the preceding example specifies the following settings:

ifdef::community[]
* The sink is set up for AWS Kinesis in region `eu-central-1`
* The source connector is set up for PostgreSQL using the default {prodname} https://github.com/debezium/postgres-decoderbufs[decoderbufs] plugin.
If using PostgreSQL's built-in `pgoutput` plugin, set `debezium.source.plugin.name=pgoutput`
endif::community[]

* The source connector captures events from a schema named `inventory`.
...
* The source offset is stored in a file named `offsets.dat` in the `data` directory.
To prevent startup errors, the directory that you specify must exist before you start the server.
...
__  ____  __  _____   ___  __ ____  ______
 ...
 -/ /_/ / /_/ / __ |/ , _/ ,< / /_/ /\ \
--\___\_\____/_/ |_/_/|_/_/|_|\____/___/
2020-05-15 11:33:12,628 INFO  [io.deb.ser.DebeziumServer] (main) Consumer 'io.debezium.server.http.HttpChangeConsumer' instantiated
2020-05-15 11:33:12,754 INFO  [org.apa.kaf.con.jso.JsonConverterConfig] (main) JsonConverterConfig values:
	converter.type = key
	decimal.format = BASE64
	schemas.cache.size = 1000
...
2020-05-15 11:33:12,838 INFO  [io.deb.con.com.BaseSourceTask] (pool-3-thread-1)    database.hostname = localhost
2020-05-15 11:33:12,838 INFO  [io.deb.con.com.BaseSourceTask] (pool-3-thread-1)    database.password = ********
2020-05-15 11:33:12,839 INFO  [io.deb.con.com.BaseSourceTask] (pool-3-thread-1)    name = http
2020-05-15 11:33:12,839 INFO  [io.deb.con.com.BaseSourceTask] (pool-3-thread-1)    topic.prefix = tutorial
2020-05-15 11:33:12,839 INFO  [io.deb.con.com.BaseSourceTask] (pool-3-thread-1)    database.port = 5432
2020-05-15 11:33:12,839 INFO  [io.deb.con.com.BaseSourceTask] (pool-3-thread-1)    schema.include.list = inventory
2020-05-15 11:33:12,908 INFO  [io.quarkus] (main) debezium-server {debezium-version} (powered by Quarkus 3.33.2) started in 1.198s. Listening on: http://0.0.0.0:8080
2020-05-15 11:33:12,911 INFO  [io.quarkus] (main) Profile prod activated.
2020-05-15 11:33:12,911 INFO  [io.quarkus] (main) Installed features: [cdi, smallrye-health]
----
endif::product[]
...
// Type: reference
// ModuleID: debezium-server-source-configuration-properties
// Title: {prodname} Server source configuration properties
[id="debezium-source-configuration-properties"]
=== Source configuration

Source configuration properties specify which connector {prodname} Server runs and configure offset and schema history storage outside of Kafka Connect.

Most source configuration properties correspond to properties that are documented for individual connectors. 
To use these properties in {prodname} Server, prepend the `debezium.source.` prefix to the connector property name. 
{prodname} Server also provides additional source configuration properties for settings that Kafka Connect normally handles automatically.
For example, `debezium.source.offset.storage` and `debezium.source.offset.storage.file.filename` to configure file-based offset storage.
...
|[[schema-history-internal-azure-storage-account-endpoint]]<<schema-history-internal-azure-storage-account-endpoint,`debezium.source.schema.history.internal.azure.storage.account.blob.endpoint`>>
|
...
Only used when `debezium.source.schema.history.internal.azure.storage.account.connectionstring` is not set.
Should not be set together with `debezium.source.schema.history.internal.azure.storage.account.name`.
...
// Type: reference
// ModuleID: debezium-server-format-configuration-options
// Title: {prodname} Server format configuration options
[id="debezium-format-configuration-options"]
=== Format configuration

Configure how {prodname} Server serializes message keys and values. 
You can configure key and value formats independently by using either the default JSON format or a custom Kafka Connect `Converter` implementation.
...
// Type: reference
// ModuleID: debezium-server-transformation-configuration-options
// Title: {prodname} Server transformation configuration options
[id="debezium-transformations-configuration-options"]
=== Transformation configuration

Configure Kafka Connect Single Message Transformations (SMTs) that {prodname} Server applies to messages before it delivers them to the sink. 
Specify the transformations to apply and the configuration options for each transformation.
...
// Type: reference
// ModuleID: debezium-server-predicates-configuration-options
// Title: {prodname} Server predicates configuration options
[id="debezium-predicates-configuration-options"]
=== Predicates configuration

Configure predicates that evaluate messages and determine when {prodname} Server applies message transformations.
{prodname} Server supports the Kafka Connect https://cwiki.apache.org/confluence/display/KAFKA/KIP-585%3A+Filter+and+Conditional+SMTs[predicate framework for conditional Single Message Transformations (SMTs)].
Predicate properties specify the predicates to use, their implementation classes, and their configuration options.
...
// Type: concept
// Title: Enabling message filtering in {prodname} Server
[id="debezium-server-enabling-message-filtering"]
=== Enabling message filtering

You can enable the filter SMT in {prodname} Server to selectively pass or discard change events based on expression-based conditions that you define.

Because filter expressions are evaluated by a scripting engine running inside the {prodname} Server process, this feature is only safe to enable in environments where access to the server configuration is restricted to authorized operators.
Scripting support is therefore disabled by default and must be explicitly enabled.

To enable message filtering, set the environment variable `ENABLE_DEBEZIUM_SCRIPTING` to `true` before starting {prodname} Server.
When yhou enable this setting, the startup script adds the `debezium-scripting` JAR and the https://jcp.org/en/jsr/detail?id=223[JSR 223] script engine implementations (Groovy and GraalVM JavaScript) from the `opt_lib` directory of the {prodname} Server distribution to the server classpath.
These libraries are kept in `opt_lib` rather than on the default classpath precisely so that scripting is never available unless an operator makes a conscious decision to include them.

.Additional resources
* link:{link-filtering}#message-filtering[{prodname} message filtering SMT].

// Type: reference
// ModuleID: debezium-server-asynchronous-engine-properties
// Title: {prodname} Server asynchronous engine properties
[id="debezium-async-engine-properties"]
=== Asynchronous Engine Properties

By default, {prodname} Server uses the asynchronous embedded engine (`AsyncEmbeddedEngine`) to process change events. 
Use the following properties to configure record-processing threads, task lifecycle management, and engine shutdown behavior.
...
// Type: reference
// ModuleID: debezium-server-quarkus-runtime-configuration-properties
// Title: {prodname} Server Quarkus runtime configuration properties
[id="debezium-additional-configuration-options"]
=== Additional configuration

{prodname} Server runs on the Quarkus framework.
All configuration properties that Quarkus exposes are available in {prodname} Server.
The following table lists commonly used Quarkus properties for configuring HTTP endpoint access and logging behavior.
...
You can disable JSON logging by setting `quarkus.log.console.json=false` in the `config/application.properties` file.
For details, see the example in the file.

// Type: assembly
// Title: {prodname} Server sink configuration
[id="debezium-server-sink-configuration"]
=== Sink configuration

{prodname} Server supports implementations for delivering change events to different sink destinations.
Configure the destination to which {prodname} Server delivers change events.
Set the `debezium.sink.type` property to specify the sink implementation, and then configure the properties that apply to the selected sink.


ifdef::community[]
// Type: reference
// Title: Amazon Kinesis sink configuration
[id="debezium-server-amazon-kinesis-sink-configuration"]
==== Amazon Kinesis
...
// Type: reference
// Title: Amazon Kinesis sink injection points
[id="debezium-server-amazon-kinesis-injection-points"]
===== Injection points
...
ifdef::community[]
// Type: reference
// Title: Google Cloud Pub/Sub sink configuration
[id="debezium-server-google-cloud-pubsub-sink-configuration"]
...
// Type: reference
// Title: Google Cloud Pub/Sub sink injection points
[id="debezium-server-google-cloud-pubsub-injection-points"]
===== Injection points
...
ifdef::community[]
// Type: reference
// Title: Pub/Sub Lite sink configuration
[id="debezium-server-pubsub-lite-sink-configuration"]
==== Pub/Sub Lite
...
// Type: reference
// Title: Pub/Sub Lite sink injection points
[id="debezium-server-pubsub-lite-injection-points"]
===== Injection points
...
// Type: reference
// Title: HTTP Client sink configuration
[id="debezium-server-http-client-sink-configuration"]
==== HTTP Client

Configure the HTTP Client sink to deliver change events from {prodname} Server to an HTTP endpoint for downstream processing.
The HTTP Client sink supports optional JWT and OAuth 2.0 client credentials authentication, and provides options for configuring batched event delivery.

You can use the HTTP Client sink to integrate {prodname} Server with Knative and enable it to act as a Knative Event Source.
...
|
|The HTTP Server URL to stream events to. 
This can also be set by defining the `K_SINK` environment variable, which is used by the Knative source framework.
...
|[[httpclient-batch-enabled]]<<httpclient-batch-enabled, `debezium.sink.http.batch.enabled` >>
|false
...
|[[httpclient-batch-max-size]]<<httpclient-batch-max-size, `debezium.sink.http.batch.max-size` >>
|200
...
`oauth2`:: OAuth2 client credentials grant (https://datatracker.ietf.org/doc/html/rfc6749#section-4.4[RFC 6749 Section 4.4]).
...
|
|Specifies the base URL (e.g., `http://myserver:8000/`) for JWT authentication. 
The paths `auth/authenticate` and `auth/refreshToken` are appended for the JWT initial and authentication REST requests.
...
|[[httpclient-authentication-oauth2-client-id]]<<httpclient-authentication-oauth2-client-id, `debezium.sink.http.authentication.oauth2.client_id` >>
|
|Specifies the OAuth2 client ID for the client credentials grant.

|[[httpclient-authentication-oauth2-client-secret]]<<httpclient-authentication-oauth2-client-secret, `debezium.sink.http.authentication.oauth2.client_secret` >>
|
|Specifies the OAuth2 client secret for the client credentials grant.

|[[httpclient-authentication-oauth2-token-url]]<<httpclient-authentication-oauth2-token-url, `debezium.sink.http.authentication.oauth2.token_url` >>
|
|The URL of the OAuth2 token endpoint (e.g., `\https://auth.example.com/oauth/token`).

|[[httpclient-authentication-oauth2-scope]]<<httpclient-authentication-oauth2-scope, `debezium.sink.http.authentication.oauth2.scope` >>
|
|Optional space-separated list of OAuth2 scopes to request (e.g., `data:read data:write`).

|[[httpclient-authentication-oauth2-client-auth-method]]<<httpclient-authentication-oauth2-client-auth-method, `debezium.sink.http.authentication.oauth2.client_auth_method` >>
|client_secret_basic
|Specifies how client credentials are sent to the token endpoint.
Supports one of the following options:

`client_secret_basic`:: Sends credentials as an HTTP Basic Authorization header (https://datatracker.ietf.org/doc/html/rfc6749#section-2.3.1[RFC 6749 Section 2.3.1]).
`client_secret_post`:: Sends `client_id` and `client_secret` as form fields in the POST request body.

|[[httpclient-authentication-oauth2-http-method]]<<httpclient-authentication-oauth2-http-method, `debezium.sink.http.authentication.oauth2.token_url.http_method` >>
|POST
|The HTTP method to use for token requests. 
The OAuth2 specification requires `POST`, but some providers accept `GET` with query parameters.
Set to `GET` only for providers that do not support the standard `POST` method.

|[[httpclient-authentication-oauth2-params]]<<httpclient-authentication-oauth2-params, `debezium.sink.http.authentication.oauth2.params.*` >>
|
|Optional additional parameters to include in the token request. 
For example, some providers require an `audience` or `resource` parameter. 
Set these using the `params.` prefix, for example, `debezium.sink.http.authentication.oauth2.params.audience=\https://my-api.example.com`.
...
ifdef::community[]
// Type: reference
// Title: Apache Pulsar sink configuration
[id="debezium-server-apache-pulsar-sink-configuration"]
==== Apache Pulsar
...
// Type: reference
// Title: Apache Pulsar sink injection points
[id="debezium-server-apache-pulsar-injection-points"]
===== Injection points
...
ifdef::community[]
// Type: reference
// Title: Azure Event Hubs sink configuration
[id="debezium-server-azure-event-hubs-sink-configuration"]
==== Azure Event Hubs
...
|[[dynamic-partition-routing]]<<dynamic-partition-routing, `debezium.sink.eventhubs.dynamicpartitionrouting`>>
|
| An optional setting that controls the behavior of dynamic partition routing. There are three possible values:

- `default`: If the record key is not null, it is used as the batch partition key. Else, if the record's partition ID is not null, events are batched and routed to the specified partition ID. Otherwise, round-robin routing is applied. This is the default when no value is provided.
- `key`: If the record key is not null, it is used as the batch partition key. Otherwise, round-robin routing is applied.
- `partitionid`: If the record's partition ID is not null, events are batched and routed to the specified partition ID. Otherwise, round-robin routing is applied.
...
// Type: concept
// Title: Using partitions in Azure Event Hubs
[id="debezium-server-azure-event-hubs-using-partitions"]
===== Using partitions in EventHubs

Azure Event Hubs supports multiple partition routing strategies that control how {prodname} distributes change events across partitions.
You can specify fixed partition IDs, partition keys, or the Partition Routing transformer to match the throughput and ordering requirements in your deployment.
...
// Type: reference
// Title: Azure Event Hubs sink injection points
[id="debezium-server-azure-event-hubs-injection-points"]
===== Injection points
...
ifdef::community[]
// Type: reference
// Title: Redis (Stream) sink configuration
[id="debezium-server-redis-stream-sink-configuration"]
==== Redis (Stream)

The Redis sink stores change events as entries in a Redis stream, supporting both single-instance and cluster deployments.
You configure the sink by setting the `debezium.sink.type` property to `redis` and specifying connection, authentication, and storage options.
...
{prodname} can use either a single Redis instance or use Redis Cluster mode for sink operations:
...
|`0`
|A number in the range 0--15 used that {prodname} uses to select the database to work with.
This setting is only available to standalone Redis connections; Redis Clusters use only database 0.
...
// Type: reference
// ModuleID: debezium-server-redis-stream-message-format
// Title: Redis (Stream) sink message format
...
// Type: reference
// Title: Redis (Stream) sink injection points
[id="debezium-server-redis-stream-injection-points"]
===== Injection points
...
// Type: reference
// Title: Redis sink configuration examples
[id="debezium-server-redis-configuration-examples"]
==== Redis Configuration Examples

The following examples show complete `application.properties` configurations for connecting {prodname} Server to a Redis sink in both single-instance and cluster deployment modes.

// Type: reference
// Title: Redis sink single instance mode configuration
[id="debezium-server-redis-single-instance-mode"]
...
// Type: reference
// Title: Redis sink cluster mode configuration
[id="debezium-server-redis-cluster-mode"]
===== Cluster Mode

Use the following configuration when connecting {prodname} Server to a Redis Cluster, specifying multiple node addresses for sink, offset storage, and schema history.
...
ifdef::community[]
// Type: reference
// Title: NATS Streaming sink configuration
[id="debezium-server-nats-streaming-sink-configuration"]
==== NATS Streaming
...
// Type: reference
// Title: NATS Streaming sink injection points
[id="debezium-server-nats-streaming-injection-points"]
===== Injection points
...
ifdef::community[]
// Type: reference
// Title: NATS JetStream sink configuration
[id="debezium-server-nats-jetstream-sink-configuration"]
==== NATS JetStream
...
|[[nats-jetstream-stream-name]]<<nats-jetstream-stream-name, `debezium.sink.nats-jetstream.stream-name`>>
|DebeziumStream
|Optional custom name for the stream that {prodname} creates.
...
// Type: reference
// Title: NATS JetStream sink injection points
[id="debezium-server-nats-jetstream-injection-points"]
===== Injection points
...
ifdef::community[]
// Type: reference
// Title: Apache Fluss sink configuration
[id="debezium-server-apache-fluss-sink-configuration"]
==== Apache Fluss

https://fluss.apache.org/[Apache Fluss] is a streaming storage built for real-time analytics, providing unified streaming and batch processing capabilities with high performance, low latency, and strong consistency guarantees.
{prodname} Server can publish change events to Apache Fluss, supporting both primary key tables for upsert/delete operations and log tables for append-only streaming.
...
|[[fluss-type]]<<fluss-type, `debezium.sink.type`>>
|
|Must be set to `fluss`.

|[[fluss-bootstrap-servers]]<<fluss-bootstrap-servers, `debezium.sink.fluss.bootstrap.servers`>>
|
|Comma-separated list of Fluss coordinator server addresses in `host:port` format.

|[[fluss-default-database]]<<fluss-default-database, `debezium.sink.fluss.default.database`>>
|
|Default Fluss database used when resolving table paths from event destinations.

|[[fluss-primary-key-mode]]<<fluss-primary-key-mode, `debezium.sink.fluss.primary.key.mode`>>
|`auto`
|Determines the write mode selection based on primary key presence. Valid values:

`auto`:: Automatically selects between append and upsert mode based on whether the table has a primary key.
`upsert`:: Requires tables to have a primary key and performs insert/update/delete operations.
`append`:: Writes all records in append-only mode, regardless of primary key existence.

|[[fluss-table-auto-create]]<<fluss-table-auto-create, `debezium.sink.fluss.table.auto.create`>>
|`false`
|When `true`, automatically creates Fluss tables if they do not exist, deriving the schema from the Debezium event schema.

|[[fluss-retries-max]]<<fluss-retries-max, `debezium.sink.fluss.retries.max`>>
|`5`
|Maximum number of retry attempts on transient write failures.

|[[fluss-retries-interval-ms]]<<fluss-retries-interval-ms, `debezium.sink.fluss.retries.interval.ms`>>
|`1000`
|The initial retry interval in milliseconds.

|[[fluss-retries-max-interval-ms]]<<fluss-retries-max-interval-ms, `debezium.sink.fluss.retries.max.interval.ms`>>
|`60000`
|The maximum retry interval in milliseconds.

|[[fluss-retries-backoff-multiplier]]<<fluss-retries-backoff-multiplier, `debezium.sink.fluss.retries.backoff.multiplier`>>
|`2.0`
...
// Type: reference
// Title: Apache Fluss sink injection points
[id="debezium-server-apache-fluss-injection-points"]
===== Injection points

The Fluss sink behavior can be modified by a custom logic providing alternative implementations for specific functionalities.
When the alternative implementations are not available then the default ones are used.
...
|[[fluss-ext-stream-name-mapper]]<<fluss-ext-stream-name-mapper, `io.debezium.server.StreamNameMapper`>>
|
|Custom implementation maps the planned destination (topic) name into a physical Fluss table name.
By default the same name is used.
...
// Type: reference
// Title: Apache Kafka sink configuration
[id="debezium-server-apache-kafka-sink-configuration"]
==== Apache Kafka

Configure the Apache Kafka sink to publish change events from {prodname} Server to Kafka topics.
Use the sink properties to specify broker connection details and other Kafka-specific settings.
...
// Type: reference
// Title: Apache Kafka sink injection points
[id="debezium-server-apache-kafka-injection-points"]
===== Injection points

Customize Kafka sink behavior by providing implementations of supported extension points.
For example, you can implement a custom `StreamNameMapper` to control how {prodname} Server maps source streams to Kafka topics.
If you do not provide a custom implementation, {prodname} Server uses the default implementation.
...
ifdef::community[]
// Type: reference
// Title: Pravega sink configuration
[id="debezium-server-pravega-sink-configuration"]
==== Pravega
...
// Type: reference
// Title: Pravega sink injection points
[id="debezium-server-pravega-injection-points"]
===== Injection points
...
ifdef::community[]
// Type: reference
// Title: Infinispan sink configuration
[id="debezium-server-infinispan-sink-configuration"]
==== Infinispan
...
// Type: reference
// Title: Infinispan sink injection points
[id="debezium-server-infinispan-injection-points"]
===== Injection points
...
ifdef::community[]
// Type: reference
// Title: Apache RocketMQ sink configuration
[id="debezium-server-apache-rocketmq-sink-configuration"]
==== Apache RocketMQ
...
// Type: reference
// Title: Apache RocketMQ sink injection points
[id="debezium-server-apache-rocketmq-injection-points"]
===== Injection points
...
ifdef::community[]
// Type: reference
// Title: RabbitMQ Stream sink configuration
[id="debezium-server-rabbitmq-stream-sink-configuration"]
==== RabbitMQ Stream
...
// Type: reference
// Title: RabbitMQ Stream sink injection points
[id="debezium-server-rabbitmq-stream-injection-points"]
===== Injection points
...
// Type: reference
// Title: RabbitMQ Native Stream sink configuration
[id="debezium-server-rabbitmq-native-stream-sink-configuration"]
...
ifdef::community[]
// Type: reference
// Title: Milvus sink configuration
[id="debezium-server-milvus-sink-configuration"]
==== Milvus
...
// Type: reference
// Title: Milvus sink injection points
[id="debezium-server-milvus-injection-points"]
===== Injection points
...
ifdef::community[]
// Type: reference
// Title: Qdrant sink configuration
[id="debezium-server-qdrant-sink-configuration"]
==== Qdrant Sink
...
ifdef::community[]
// Type: reference
// Title: InstructLab sink configuration
[id="debezium-server-instructlab-sink-configuration"]
==== InstructLab
...
// Type: reference
// ModuleID: debezium-server-instructlab-mapping-definition
// Title: InstructLab sink mapping definition
...
ifdef::community[]
// Type: reference
// Title: JDBC sink configuration
[id="debezium-server-jdbc-sink-configuration"]
==== JDBC
...
|[[jdbc-type]]<<jdbc-type, `debezium.sink.type`>>
|
|Must be set to `jdbc`.

|[[jdbc-props]]<<jdbc-props, `debezium.sink.jdbc.*`>>
|
|All JDBC connector configuration properties can be set using the `debezium.sink.jdbc.` prefix.
For the full list of available properties, including connection, runtime, and dialect-specific options, see the xref:connectors/jdbc.adoc#jdbc-connector-properties[{prodname} JDBC connector configuration properties].

|===

[NOTE]
====
The JDBC sink requires the appropriate JDBC driver for the target database to be available on the classpath.
The {prodname} Server distribution does not include any JDBC drivers for target databases, so you must add the driver for your destination database manually to the `lib/` directory.
====

[IMPORTANT]
====
Although the JDBC sink reuses the {prodname} JDBC sink connector, {prodname} Server does not currently provide the same delivery guarantees that are available when running the connector in other runtimes such as Kafka Connect.
In particular, features that depend on the runtime for offset management, exactly-once semantics, or automatic error handling and retries may behave differently or may not be available.
====
...
ifdef::community[]
// Type: reference
// Title: Apache Iceberg sink configuration
[id="debezium-server-apache-iceberg-sink-configuration"]
==== Apache Iceberg

The {prodname} Apache Iceberg connector is a sink that runs in {prodname} Server. It consumes database change events from a source {prodname} connector and writes them to Apache Iceberg tables.
This allows for direct replication of a Database (Change Data Capture events) to Iceberg tables on cloud storage or HDFS, eliminating the need for intermediate systems like ETL Tool, Spark or other streaming platforms.

The {prodname} Iceberg connector supports both `upsert` and `append` modes for data replication. When event and key schema information is enabled, the destination Iceberg tables are automatically created upon the initial startup.
...
[NOTE]
====
- It is recommended to use `ExtractNewRecordState` transformation (event flattening). Key features like `upsert` mode and automatic primary key detection depend on the flattened event structure produced by this SMT. Without it, the connector can only operate in a more limited append-only mode.

- The Debezium Server sink for Apache Iceberg is a community-maintained, open-source project. Please note that it is maintained in a separate repository, which you can find here: link:https://github.com/memiiso/debezium-server-iceberg[Debezium Server Iceberg GitHub]
====
...
|[[iceberg-type]]<<iceberg-type, `debezium.sink.type`>>
|
|Must be set to `iceberg`.

|[[iceberg-catalog-name]]<<iceberg-catalog-name, `debezium.sink.iceberg.catalog-name`>>
|`default`
|User-specified Iceberg catalog name.

|[[iceberg-warehouse]]<<iceberg-warehouse, `debezium.sink.iceberg.warehouse`>>
|
|Required. Root path of the Iceberg data warehouse (e.g., `s3a://my-bucket/warehouse`).

|[[iceberg-io-impl]]<<iceberg-io-impl, `debezium.sink.iceberg.io-impl`>>
|`org.apache.iceberg.io.ResolvingFileIO`
|Iceberg FileIO implementation used to write data files.

|[[iceberg-table-namespace]]<<iceberg-table-namespace, `debezium.sink.iceberg.table-namespace`>>
|`default`
|The namespace within the catalog to use for tables.

|[[iceberg-table-prefix]]<<iceberg-table-prefix, `debezium.sink.iceberg.table-prefix`>>
|``
|An optional prefix to add to the names of all created Iceberg tables.

|[[iceberg-destination-regexp]]<<iceberg-destination-regexp, `debezium.sink.iceberg.destination-regexp`>>
|``
|A regular expression used to rewrite the destination table name.

|[[iceberg-destination-regexp-replace]]<<iceberg-destination-regexp-replace, `debezium.sink.iceberg.destination-regexp-replace`>>
|``
|The replacement string for the `destination-regexp` expression.

|[[iceberg-destination-uppercase-table-names]]<<iceberg-destination-uppercase-table-names, `debezium.sink.iceberg.destination-uppercase-table-names`>>
|`false`
|If `true`, creates uppercase Iceberg table names.

|[[iceberg-destination-lowercase-table-names]]<<iceberg-destination-lowercase-table-names, `debezium.sink.iceberg.destination-lowercase-table-names`>>
|`false`
|If `true`, creates lowercase Iceberg table names.

|[[iceberg-table-mapper]]<<iceberg-table-mapper, `debezium.sink.iceberg.table-mapper`>>
|`default-mapper`
|The mapping strategy for source to destination table names. Only `default-mapper` is currently provided.

|[[iceberg-upsert]]<<iceberg-upsert, `debezium.sink.iceberg.upsert`>>
|`false`
|If `true`, enables upsert mode. If `false`, uses append-only mode.

|[[iceberg-upsert-keep-deletes]]<<iceberg-upsert-keep-deletes, `debezium.sink.iceberg.upsert-keep-deletes`>>
|`true`
|In upsert mode, if `true`, deleted rows are kept and marked with `__deleted=true` (soft delete). If `false`, they are physically deleted.

|[[iceberg-upsert-dedup-column]]<<iceberg-upsert-dedup-column, `debezium.sink.iceberg.upsert-dedup-column`>>
|`__source_ts_ns`
|The column used to deduplicate records in upsert mode. The record with the highest value in this column is kept.

|[[iceberg-upsert-op-field]]<<iceberg-upsert-op-field, `debezium.sink.iceberg.upsert-op-field`>>
|`__op`
|The field name that contains the operation type (`c`, `u`, `d`, `r`) for deduplication logic.

|[[iceberg-write-format-default]]<<iceberg-write-format-default, `debezium.sink.iceberg.write.format.default`>>
|`parquet`
|Default file format for Iceberg tables. Can be `parquet`, `avro`, or `orc`.

|[[iceberg-nested-as-variant]]<<iceberg-nested-as-variant, `debezium.sink.iceberg.nested-as-variant`>>
|`false`
|If `true`, all nested data is stored in Iceberg `variant` fields, allowing schema changes to be absorbed without schema evolution.

|[[iceberg-allow-field-addition]]<<iceberg-allow-field-addition, `debezium.sink.iceberg.allow-field-addition`>>
|`true`
|If `true`, allows the connector to automatically add new columns to Iceberg tables when new fields appear in source events.

|[[iceberg-create-identifier-fields]]<<iceberg-create-identifier-fields, `debezium.sink.iceberg.create-identifier-fields`>>
|`true`
|If `false`, the connector will not create identifier fields for primary keys in the Iceberg table. Required to be `false` for append-only consumption of nested events.

|[[iceberg-preserve-required-property]]<<iceberg-preserve-required-property, `debezium.sink.iceberg.preserve-required-property`>>
|`false`
|If `true`, columns in the Iceberg table will preserve their original `required`/`optional` property from the source. By default, only primary key columns are marked as required.

|[[iceberg-batch-size-wait]]<<iceberg-batch-size-wait, `debezium.sink.batch.batch-size-wait`>>
|`NoBatchSizeWait`
|The batch size wait strategy to control commit frequency.

|[[iceberg-concurrent-uploads]]<<iceberg-concurrent-uploads, `debezium.sink.batch.concurrent-uploads`>>
|`1`
|The number of parallel threads to use for uploading data to Iceberg.

|[[iceberg-concurrent-uploads-timeout-minutes]]<<iceberg-concurrent-uploads-timeout-minutes, `debezium.sink.batch.concurrent-uploads.timeout-minutes`>>
|`60`
|The timeout in minutes to wait for all parallel uploads to complete.

|[[iceberg-offset-storage]]<<iceberg-offset-storage, `debezium.source.offset.storage`>>
|`io.debezium.server.iceberg.offset.IcebergOffsetBackingStore`
|Sets the offset backing store to use an Iceberg table.

|[[iceberg-offset-storage-table-name]]<<iceberg-offset-storage-table-name, `debezium.source.offset.storage.iceberg.table-name`>>
|`_debezium_offset_storage`
|The name of the Iceberg table used to store connector offsets.

|[[iceberg-schema-history-internal]]<<iceberg-schema-history-internal, `debezium.source.schema.history.internal`>>
|`io.debezium.server.iceberg.history.IcebergSchemaHistory`
|Sets the schema history to use an Iceberg table.

|[[iceberg-schema-history-internal-table-name]]<<iceberg-schema-history-internal-table-name, `debezium.source.schema.history.internal.iceberg.table-name`>>
|`_debezium_database_history_storage`
|The name of the Iceberg table used to store database schema history.

|[[iceberg-passthrough-properties]]<<iceberg-passthrough-properties, `debezium.sink.iceberg.{iceberg.prop.name}`>>
|
|Passes any property prefixed with `debezium.sink.iceberg.` directly to the Iceberg library configuration, removing the prefix. This can be used to configure underlying details like S3 access credentials, catalog properties, etc. See the link:https://iceberg.apache.org/docs/latest/configuration/[Iceberg documentation] for available properties.
...
ifdef::community[]
// Type: assembly
// Title: Databricks Zerobus Ingest sink configuration
[id="debezium-server-databricks-zerobus-ingest-sink-configuration"]
==== Databricks Zerobus Ingest

The {prodname} https://docs.databricks.com/aws/en/ingestion/zerobus[Databricks Zerobus Ingest] is a serverless, push-based ingestion service that writes change event records directly into Unity Catalog Delta tables.
The service does not require an intermediate message broker.
The sink uses OAuth to authenticate with a Databricks service principal and streams each event record to the Delta table that corresponds to the event destination.

The sink uses one of the following routes to deliver event records, as specified by the xref:zerobus-type[`debezium.sink.type`] property:

`zerobus`:: (recommended) Uses the Databricks Zerobus Java SDK over gRPC.
`zerobus-rest`:: Uses the Zerobus REST API.
This method accepts only one HTTP request per record.
Select this option for environments that lack a persistent connection.

`kafka`:: Reuses the built-in Kafka sink against the Zerobus Kafka-compatible endpoint.
For more information, see <<zerobus-kafka-route, Kafka route>>.

Because Zerobus accepts only JSON objects that match the target table schema, configure {prodname} to emit flat JSON by applying the {link-prefix}:{link-event-flattening}#new-record-state-extraction[`ExtractNewRecordState` SMT] with `schemas.enable=false`.
Delivery uses an at-least-once guarantee.
As a result, the sink might deliver the same record more than once.
Configure downstream consumers to deduplicate records, for example, by using the source LSN that the record carries.
To enable the service to write data to the target table, the following requirements apply:

* The target table must be a Unity Catalog-managed Delta table.
* The table must be in the same Databricks region as the workspace.
* Change Data Feed and column mapping must be disabled.
...
// Type: reference
// Title: Databricks Zerobus Ingest gRPC route configuration properties
[id="debezium-server-zerobus-grpc-route-configuration"]
===== gRPC route (`zerobus`)

The following table describes the properties that control how Zerobus Ingest behaves when it uses gRPC streams to maintain connections between {prodname} and Zerobus endpoints.
...
|[[zerobus-type]]<<zerobus-type, `debezium.sink.type`>>
|No default value.
|Must be set to `zerobus`.

|[[zerobus-endpoint]]<<zerobus-endpoint, `debezium.sink.zerobus.endpoint`>>
|No default value.
|The Zerobus gRPC endpoint, for example `<workspace-id>.zerobus.<region>.cloud.databricks.com` (AWS) or `<workspace-id>.zerobus.<region>.azuredatabricks.net` (Azure).

|[[zerobus-workspace-url]]<<zerobus-workspace-url, `debezium.sink.zerobus.workspace.url`>>
|No default value.
|The Databricks workspace URL, for example `https://dbc-xxxx.cloud.databricks.com`.

|[[zerobus-client-id]]<<zerobus-client-id, `debezium.sink.zerobus.client.id`>>
|No default value.
|The OAuth client id (service principal application id).

|[[zerobus-client-secret]]<<zerobus-client-secret, `debezium.sink.zerobus.client.secret`>>
|No default value.
|The OAuth client secret for the service principal.

|[[zerobus-table]]<<zerobus-table, `debezium.sink.zerobus.table`>>
|No default value.
|(Optional) The fully qualified name of the default target table (`catalog.schema.table`).
When this option is set, all events are written to this table; otherwise the mapped destination (topic) name is used as the fully qualified table name.

|[[zerobus-payload-mode]]<<zerobus-payload-mode, `debezium.sink.zerobus.payload.mode`>>
|`typed`
|Specifies the structure of rows that the sink writes to the target table.

Set one of the following values:

`typed`::
Writes the fields of each change event to columns by using the data types of the target table.
For details about how source types map to target column types, see <<zerobus-data-type-mapping, Data type mapping>>.

`envelope`::
Writes each change event as a raw CDC envelope.
Use this mode when a downstream process handles type casting and materialization.

The two modes write different row schemas.
The target table schema must match the selected mode; changing the mode without changing the target table schema causes ingestion to fail.

[IMPORTANT]
====
If you set the value to `envelope`, do not apply the `ExtractNewRecordState` SMT.
This transformation removes the `op` field of the change event, so the sink cannot determine the operation type and records every row with `operation=change`.
Apply only routing transformations, such as `RegexRouter`, in this mode.
====

|[[zerobus-record-format]]<<zerobus-record-format, `debezium.sink.zerobus.record.format`>>
|`json`
|Specifies the wire encoding for records that use the <<zerobus-payload-mode, configured payload mode>>.

Set one of the following values:

`json`::
Writes JSON records.
This format supports the `typed` and `envelope` payload modes.

`protobuf`::
Writes schema-bound Protobuf records by using a descriptor that the sink supplies to Zerobus.
Choose this format for envelope mode when you want Zerobus to use an explicit Protobuf schema instead of JSON encoding.
This format supports only the `envelope` payload mode.
The sink reports an error at start-up if you combine `protobuf` with the `typed` payload mode.

|[[zerobus-max-record-bytes]]<<zerobus-max-record-bytes, `debezium.sink.zerobus.max.record.bytes`>>
|`10000000`
|Specifies the maximum encoded size, in bytes, of a JSON or Protobuf envelope record.
The sink validates every record in a target-table group before it ingests any record in that group.
If a record exceeds the limit, ingestion fails and the source offsets for the batch are not committed.

|[[zerobus-json-flexible-fields-encoding]]<<zerobus-json-flexible-fields-encoding, `debezium.sink.zerobus.json.flexible.fields.encoding`>>
|`string`
|Specifies how JSON envelope mode writes the `key`, `value`, `source_position`, and `headers` fields.
Set the value to `string` to write canonical JSON text, or to `object` to write nested JSON values.

|[[zerobus-idempotency-mode]]<<zerobus-idempotency-mode, `debezium.sink.zerobus.idempotency.mode`>>
|`source`
|Specifies whether an envelope contains a deterministic `idempotency_key`.
Set the value to `source` to write a `sha256:<64 hexadecimal characters>` identity derived from the source event, or to `none` to omit the identity.
The readable source partition and offset remain available in `source_position` in both modes.

|[[zerobus-tombstone-handling-mode]]<<zerobus-tombstone-handling-mode, `debezium.sink.zerobus.tombstone.handling.mode`>>
|`event`
|Specifies how envelope mode handles tombstone records.
Set the value to `event` to write an envelope with `operation=tombstone`, the original key, and a null value.
Set the value to `drop` to skip tombstones intentionally.

|[[zerobus-filter-destination-regex]]<<zerobus-filter-destination-regex, `debezium.sink.zerobus.filter.destination.regex`>>
|No default value.
|An optional regular expression that the complete Debezium destination must match for the envelope to be ingested.

|[[zerobus-filter-operations]]<<zerobus-filter-operations, `debezium.sink.zerobus.filter.operations`>>
|No default value.
|An optional comma-separated operation allowlist.
The canonical Debezium codes are `c`, `r`, `u`, `d`, `t`, and `m`.
The aliases `create`, `read`, `update`, `delete`, `truncate`, `message`, `change`, and `tombstone` are also accepted.

|[[zerobus-filter-header-name]]<<zerobus-filter-header-name, `debezium.sink.zerobus.filter.header.name`>>
|No default value.
|The name of the record header to evaluate.
Configure this property together with <<zerobus-filter-header-value-regex, `filter.header.value.regex`>>.

|[[zerobus-filter-header-value-regex]]<<zerobus-filter-header-value-regex, `debezium.sink.zerobus.filter.header.value.regex`>>
|No default value.
|An optional regular expression that the complete value of the configured header must match.
Configure this property together with <<zerobus-filter-header-name, `filter.header.name`>>.

|[[zerobus-filter-value-json-pointer]]<<zerobus-filter-value-json-pointer, `debezium.sink.zerobus.filter.value.json.pointer`>>
|No default value.
|A JSON Pointer that selects a field from the serialized event value for filtering.
The pointer must begin with `/`.
Configure this property together with <<zerobus-filter-value-regex, `filter.value.regex`>>.

|[[zerobus-filter-value-regex]]<<zerobus-filter-value-regex, `debezium.sink.zerobus.filter.value.regex`>>
|No default value.
|An optional regular expression that the complete value selected by the configured JSON Pointer must match.
Configure this property together with <<zerobus-filter-value-json-pointer, `filter.value.json.pointer`>>.

|[[zerobus-filter-malformed-mode]]<<zerobus-filter-malformed-mode, `debezium.sink.zerobus.filter.malformed.mode`>>
|`fail`
|Specifies how the sink handles an event value that it cannot parse or evaluate for value filtering.
Set the value to `fail` to stop the batch, or to `drop` to skip the event.

|[[zerobus-max-inflight]]<<zerobus-max-inflight, `debezium.sink.zerobus.max.inflight.records`>>
|`50000`
|Maximum number of un-acknowledged records per stream.

|[[zerobus-max-open-streams]]<<zerobus-max-open-streams, `debezium.sink.zerobus.max.open.streams`>>
|`100`
|Specifies the maximum number of Zerobus streams that the sink keeps open at one time.
Zerobus binds each stream to a single target table, and each stream holds a connection.
Set this property to limit how many connections a source that captures many tables can hold at once.

When the number of open streams exceeds the specified value, the sink flushes and closes the streams that have gone longest without receiving a record, and reopens them on demand when a record for that table arrives.
Set the value to `0` to keep every stream open, so that the sink never closes a stream to stay within a limit.

|[[zerobus-recovery]]<<zerobus-recovery, `debezium.sink.zerobus.recovery`>>
|The SDK default.
|Specifies whether the Zerobus SDK recovers a stream that fails with a retriable error by re-sending the records that the service has not acknowledged.

Because the SDK tracks which records are unacknowledged, recovery re-sends only those records.
The sink does not retry the entire batch.
If the recovery process cannot resolve an error, the connector stops, and the offset does not advance.

|[[zerobus-recovery-retries]]<<zerobus-recovery-retries, `debezium.sink.zerobus.recovery.retries`>>
|The SDK default.
|Specifies the maximum number of times that the SDK attempts to recover a failed stream.

|[[zerobus-recovery-backoff-ms]]<<zerobus-recovery-backoff-ms, `debezium.sink.zerobus.recovery.backoff.ms`>>
|The SDK default.
|Specifies the delay, in milliseconds, between stream recovery attempts.

|[[zerobus-recovery-timeout-ms]]<<zerobus-recovery-timeout-ms, `debezium.sink.zerobus.recovery.timeout.ms`>>
|The SDK default.
|Specifies the time, in milliseconds, after which the SDK abandons a stream recovery attempt.

|[[zerobus-flush-timeout-ms]]<<zerobus-flush-timeout-ms, `debezium.sink.zerobus.flush.timeout.ms`>>
|The SDK default.
|Specifies the time, in milliseconds, that a flush waits for the records in the stream to be acknowledged.

|[[zerobus-metrics-log-interval]]<<zerobus-metrics-log-interval, `debezium.sink.zerobus.metrics.log.interval`>>
|`0`
|Specifies the number of batches that the service processes before it emits a periodic `INFO` log line to summarize sink metrics.
Set the value to `0` to disable periodic logging.
This setting does not affect availability of metrics over JMX.
For more information, see <<zerobus-monitoring, Monitoring>>.

|===

In the preceding table, several Zerobus sink properties list the default value `The SDK default`.
The SDK default is the value defined by the version of the Zerobus SDK that is used by the current {prodname} Server instance.

The SDK default applies only when the property is not specified in the {prodname} Server configuration.
If the property is omitted, the sink uses the SDK-defined value, including any changes introduced by later SDK upgrades.
If you specify a value for the property, the configured value takes precedence.

// Type: concept
// Title: Databricks Zerobus Ingest envelope row contract
[[zerobus-envelope-row-contract]]
====== Envelope row contract

Set <<zerobus-payload-mode, `debezium.sink.zerobus.payload.mode`>> to `envelope` to preserve the change event for downstream materialization instead of mapping it directly onto the target table columns.
The `max.record.bytes`, `json.flexible.fields.encoding`, `idempotency.mode`, `tombstone.handling.mode`, and `filter.*` properties apply only to this payload mode.

JSON and Protobuf envelope records expose the same logical fields:

[cols="25%a,75%a",options="header"]
|===
|Field
|Description

|`target_table`
|The fully qualified Unity Catalog table to which the sink routes the record.

|`destination`
|The original Debezium destination before the sink resolves the target table.

|`partition`
|The event partition, when one is present.

|`operation`
|One of `create`, `read`, `update`, `delete`, `truncate`, `message`, `change`, or `tombstone`.
The sink uses `change` when the source value does not contain a recognized Debezium operation.

|`idempotency_key`
|A deterministic `sha256:<64 hexadecimal characters>` event identity when <<zerobus-idempotency-mode, `idempotency.mode`>> is `source`.
Use this field to deduplicate records after an at-least-once replay.

|`key`
|The original serialized record key.

|`value`
|The original serialized Debezium change-event value.
The value is null for a tombstone.

|`source_position`
|A map containing source partition and offset entries.
Entry names use the `partition.` and `offset.` prefixes.

|`headers`
|The record headers, ordered by header name.
|===

For JSON records, <<zerobus-json-flexible-fields-encoding, `json.flexible.fields.encoding`>> controls whether `key`, `value`, `source_position`, and `headers` are canonical JSON strings or nested JSON values.
For Protobuf records, the sink supplies the descriptor to Zerobus and encodes these flexible fields as canonical JSON strings.

The sink applies configured include filters in the following order: destination, operation, header, and value.
Every configured regular expression must match the complete selected value.
An event that does not match is skipped and is considered processed when the rest of the batch succeeds.
If <<zerobus-filter-operations, `filter.operations`>> is configured, a tombstone is ingested only when the allowlist includes `tombstone`, even if <<zerobus-tombstone-handling-mode, `tombstone.handling.mode`>> is `event`.

The sink provides an at-least-once delivery guarantee.
For each contiguous target-table group, the sink serializes and validates every record before ingestion, ingests the group, and waits for Zerobus to acknowledge the final record.
The sink commits source offsets only after every target-table group in the batch succeeds.
If a later group fails, an earlier acknowledged group can be replayed because the source batch remains uncommitted.
Use `idempotency_key` to deduplicate such replays downstream.

The following example configures the JSON envelope contract, deterministic identities, explicit tombstones, and an operation allowlist:

[source,properties]
----
debezium.sink.type=zerobus
debezium.sink.zerobus.endpoint=<workspace-id>.zerobus.<region>.cloud.databricks.com
debezium.sink.zerobus.workspace.url=https://<workspace-host>
debezium.sink.zerobus.client.id=<client-id>
debezium.sink.zerobus.client.secret=<client-secret>

debezium.sink.zerobus.payload.mode=envelope
debezium.sink.zerobus.record.format=json
debezium.sink.zerobus.max.record.bytes=10000000
debezium.sink.zerobus.json.flexible.fields.encoding=string
debezium.sink.zerobus.idempotency.mode=source
debezium.sink.zerobus.tombstone.handling.mode=event
debezium.sink.zerobus.filter.operations=c,r,u,d,t,m,tombstone
----

// Type: reference
// Title: Databricks Zerobus Ingest gRPC route injection points
[id="debezium-server-zerobus-grpc-injection-points"]
====== Injection points

You can modify the behavior of the gRPC route by injecting custom logic that replaces a specific default implementation.
When no alternative implementation is available, the sink uses the default.
...
|[[zerobus-ext-sdk]]<<zerobus-ext-sdk, `com.databricks.zerobus.ZerobusSdk`>>
|`@CustomConsumerBuilder`
|A customized instance of a `ZerobusSdk` that is configured for use in opening the streams that carry records to target tables.
By default, the sink builds the instance from the <<zerobus-endpoint, `endpoint`>> and <<zerobus-workspace-url, `workspace.url`>> options.

|===

// Type: reference
// Title: Databricks Zerobus Ingest REST route configuration properties
[id="debezium-server-zerobus-rest-route-configuration"]
===== REST route (`zerobus-rest`)

The `zerobus-rest` route delivers change event records to Databricks Zerobus Ingest by using the Zerobus REST API, sending one HTTP request per record.
In environments that do not have persistent gRPC connections, set the following properties to specify how {prodname} Server uses the REST route to delivers event records to a target table.
...
|[[zerobus-rest-type]]<<zerobus-rest-type, `debezium.sink.type`>>
|No default value.
|Must be set to `zerobus-rest`.

|[[zerobus-rest-uri]]<<zerobus-rest-uri, `debezium.sink.zerobus-rest.uri`>>
|No default value.
|The Zerobus REST base URI, for example `https://<workspace-id>.zerobus.<region>.cloud.databricks.com`.

|[[zerobus-rest-workspace-url]]<<zerobus-rest-workspace-url, `debezium.sink.zerobus-rest.workspace.url`>>
|No default value.
|The Databricks workspace URL.
The service uses this value for OAuth and in the `unity-catalog-endpoint` header.

|[[zerobus-rest-workspace-id]]<<zerobus-rest-workspace-id, `debezium.sink.zerobus-rest.workspace.id`>>
|No default value.
|The numeric workspace ID that the service uses to build the Zerobus token audience.

|[[zerobus-rest-client-id]]<<zerobus-rest-client-id, `debezium.sink.zerobus-rest.client.id`>>
|No default value.
|The OAuth client ID (service principal application ID).

|[[zerobus-rest-client-secret]]<<zerobus-rest-client-secret, `debezium.sink.zerobus-rest.client.secret`>>
|No default value.
|The OAuth client secret for the service principal.

|[[zerobus-rest-table]]<<zerobus-rest-table, `debezium.sink.zerobus-rest.table`>>
|No default value.
|(Optional) A fully qualified default target table (`catalog.schema.table`). When not set, the mapped destination (topic) name is used.

|[[zerobus-rest-metrics-log-interval]]<<zerobus-rest-metrics-log-interval, `debezium.sink.zerobus-rest.metrics.log.interval`>>
|`0`
|Specifies the number of batches that the service processes before it emits a periodic `INFO` log line to summarize sink metrics.
Set the value to `0` to disable periodic logging.
This setting does not affect availability of metrics over JMX.
...
// Type: assembly
// Title: Databricks Zerobus Ingest Kafka route
[[zerobus-kafka-route]]
===== Kafka route

As an alternative to the `zerobus` and `zerobus-rest` routes, you can reuse the built-in `kafka` sink to write to the Zerobus Kafka-compatible endpoint.
This route uses the standard Kafka producer with `SASL_SSL` and the `OAUTHBEARER` mechanism.
Authentication is handled by `io.debezium.server.databricks.zerobus.ZerobusOAuthBearerLoginCallbackHandler`, which performs the OAuth token exchange and attaches the Unity Catalog authorization details that Zerobus requires.

Because this route uses the generic `kafka` sink rather than the Zerobus sink, the data type normalization that the <<zerobus-data-type-mapping, Data type mapping>> section describes does not apply.
Instead, the emitted JSON must match the target Delta table schema, which requires the source-side and format settings that the following sections describe.
Zerobus is partitionless, and it ingests only the record value.
It does not persist record keys, headers, partition assignments, or timestamps.

The Kafka-compatible endpoint must be enabled for the workspace.
If it is not enabled, authentication fails with `Access denied: feature "Zerobus Ingest Kafka Endpoint" is not enabled for workspace "<workspace-id>". Error Code: 4062`.

To use this route, set `debezium.sink.type` to `kafka` and configure the following producer properties:

[cols="55%a,45%a",options="header"]
|===
|Property
|Description

|`debezium.sink.kafka.producer.bootstrap.servers`
|The Zerobus Kafka endpoint, for example `<workspace-id>.zerobus.<region>.cloud.databricks.com:9092`.

|`debezium.sink.kafka.producer.security.protocol`
|Must be set to `SASL_SSL`.

|`debezium.sink.kafka.producer.sasl.mechanism`
|Must be set to `OAUTHBEARER`.

|`debezium.sink.kafka.producer.sasl.login.callback.handler.class`
|Must be set to `io.debezium.server.databricks.zerobus.ZerobusOAuthBearerLoginCallbackHandler`.

|`debezium.sink.kafka.producer.sasl.jaas.config`
|The JAAS configuration for the `OAuthBearerLoginModule`, which supplies the OAuth options that the login callback handler reads.

|`debezium.sink.kafka.producer.acks`
|Must be set to `all`.

|`debezium.sink.kafka.producer.enable.idempotence`
|Must be set to `false`.
Zerobus does not implement the transactional APIs, so it rejects the `InitProducerId` request that the Kafka producer sends when idempotence is enabled.
The producer then fails with `UnsupportedVersionException: The node does not support INIT_PRODUCER_ID`.
Because Kafka clients enable idempotence by default, you must set this property explicitly.

|`debezium.sink.kafka.producer.compression.type`
|Must be set to `none`.
Zerobus rejects compressed record batches.
|===

The `sasl.jaas.config` entry accepts the following options:
...
|`workspaceUrl`
|Yes, unless `tokenEndpoint` is set.
|The Databricks workspace URL.
The token endpoint is derived as `<workspaceUrl>/oidc/v1/token`.

|`workspaceId`
|Yes, unless `audience` is set.
|The numeric workspace id.
The token audience is derived as `api://databricks/workspaces/<workspaceId>/zerobusDirectWriteApi`.

|`tables`
|Yes
|A comma-separated list of fully qualified target tables (`catalog.schema.table`).
The handler uses this list to build the OAuth Rich Authorization Requests (RFC 9396) authorization details, which grant the token the Unity Catalog privileges that Zerobus requires for the target tables.

|`tokenEndpoint`
|No
|Overrides the OIDC token endpoint that is otherwise derived from `workspaceUrl`.

|`audience`
|No
|Overrides the token audience that is otherwise derived from `workspaceId`.

|`token`
|No
|A bearer token, such as a personal access token, to use without modification.
If you set this option, the handler skips the OAuth exchange and uses this token directly.
|===

// Type: reference
// Title: Record format for the Databricks Zerobus Ingest Kafka route
[id="debezium-server-zerobus-kafka-route-record-format"]
====== Record format for the Kafka route

Zerobus accepts a record value only if it is a JSON object whose fields match the target Delta table schema.
A record that does not match is rejected with `InvalidRecordException: This record has failed the validation on broker and hence will be rejected`.
To satisfy the requirement, configure the following options in addition to the producer properties:
...
|`debezium.transforms`
|Apply the {link-prefix}:{link-event-flattening}#new-record-state-extraction[`ExtractNewRecordState` SMT] so that the emitted record is a flat row rather than a change event envelope.
Optionally apply the `RegexRouter` SMT to map the topic name to the fully qualified name of the target table.

|`debezium.format.value.schemas.enable`
|Must be set to `false`.
Otherwise, the JSON converter wraps each value in a `{"schema": ..., "payload": ...}` envelope, which does not match the table schema.

|`debezium.source.decimal.handling.mode`
|Must be set to `string`.
Zerobus rejects a JSON number for a `DECIMAL` column, and the default `precise` mode emits a Base64-encoded byte array.
The `string` mode emits the exact decimal value as a JSON string, which Zerobus accepts.

|`debezium.source.time.precision.mode`
|Use the default `adaptive` mode, which emits a `TIMESTAMP` column as the number of microseconds since the epoch.
Zerobus rejects an ISO-8601 string for a `TIMESTAMP` column, and it interprets a numeric value as microseconds.
Because of the latter, the `connect` mode, which emits milliseconds, silently stores an incorrect instant.
|===

// Type: reference
// Title: Tuning throughput on the Databricks Zerobus Ingest Kafka route
[id="debezium-server-zerobus-kafka-route-tuning-throughput"]
====== Tuning throughput on the Kafka route

Throughput on the Kafka route is bound by network round trips rather than by local processing, because Zerobus acknowledges each request from the region in which the workspace runs.
When the default producer settings are active, the number of records that the service ingests per second is approximately equal to the number of in-flight requests, multiplied by the number of records per request, and divided by the request latency.
As a result, a pipeline that produces one record at a time spends most of its time waiting.

You can modify the default behavior by configuring the producer properties in the following table.
Optimal values for these settings depend on the latency between the service and the workspace.
Use the values in the table as starting points and adjust them based on latency data.
To find the request latency of a running pipeline, read the `request_latency_avg` attribute of the standard `kafka.producer:type=producer-metrics` MBean, and to confirm that batching takes effect, read `records_per_request_avg`.

[cols="40%a,15%a,45%a",options="header"]
|===
|Property
|Suggested value
|Description

|`debezium.sink.kafka.producer.linger.ms`
|`100`
|Specifies how long the producer waits to send requests.
Requests accumulate until the configured interval passes.

Set the value to `0` to remove the delay interval so that the producer sends each request as soon as possible.
When the property is set to `0`, the producer sends requests even if they contain a small number of records.
Because each request incurs the overhead of a full network round trip, when the producer has a high number of records to process, a `0` value can result in lower throughput.

Set a value greater than `0` if you want the producer to wait between sending requests.
Setting a value greater than `0` can improve throughput, because the producer batches more records per request.

In a change data capture pipeline, the end-to-end latency is typically much greater than the delay introduced by this setting, so the additional wait is usually negligible.

|`debezium.sink.kafka.producer.batch.size`
|`1048576`
|Specifies the maximum size, in bytes, of a single batch.
If `linger.ms` is set, set this option if you want the producer to send batches after their size reaches the configured limit.
This setting overrides the `linger.ms` setting to allow the producer to send batches before the end of the configured wait time.
Zerobus rejects records larger than 10 MB, so a batch size of 1 MB leaves ample headroom.

|`debezium.sink.kafka.producer.max.in.flight.requests.per.connection`
|`10`
|Specifies the maximum number of unacknowledged requests that the producer can send before it receives a response.
Increasing the value can improve throughput by allowing the producer to send additional requests before it receives acknowledgements from earlier requests.
However, increasing the value can also result in records being delivered out of order if requests are retried.

Zerobus is partitionless and provides an at-least-once delivery guarantee, so strict record ordering is not guaranteed regardless of how you configure this property.
|===

[NOTE]
====
In standard Kafka environments where idempotence is enabled, `max.in.flight.requests.per.connection` must not exceed `5`, because the idempotent producer only guarantees ordering and exactly-once semantics for up to five in-flight requests.
The Kafka route for the Databricks Zerobus Ingest sink requires `enable.idempotence=false`, so you can set this property to a value greater than `5`.
====

The following example shows a complete configuration for the Kafka route:
...
debezium.sink.kafka.producer.compression.type=none
debezium.sink.kafka.producer.linger.ms=100
debezium.sink.kafka.producer.batch.size=1048576
debezium.sink.kafka.producer.max.in.flight.requests.per.connection=10
...
debezium.transforms.unwrap.add.headers=op,source.ts_ms
debezium.transforms.route.type=org.apache.kafka.connect.transforms.RegexRouter
debezium.transforms.route.regex=<topic-prefix>\\.<schema>\\.<table>
debezium.transforms.route.replacement=<catalog>.<schema>.<table>
----

// Type: reference
// Title: Databricks Zerobus Ingest data type mapping
[[zerobus-data-type-mapping]]
===== Data type mapping

When the sink maps data types, rather than converting values based on their serialized JSON representation, it uses Kafka Connect logical type information to normalize values to a format that is compatible with the target Delta column.
By basing conversions on logical types rather than on the JSON encoding of values, the sink can accommodate different source-side serialization settings and avoid invalid-type ingestion errors without requiring users to tune source-side *.handling.mode options.

The following table shows mappings between {prodname} logical types and Delta column types:

[cols="45%a,55%a",options="header"]
|===
|Source / {prodname} logical type
|Delta column type

|Integer, floating point, boolean, string
|`INT`/`BIGINT`, `DOUBLE`/`FLOAT`, `BOOLEAN`, `STRING`

|`org.apache.kafka.connect.data.Decimal` (decimal / numeric / money)
|`DECIMAL(p,s)` Emitted as an exact locale-independent string.

|`io.debezium.time.Date` / Connect `Date`
|`DATE` (epoch days)

|`io.debezium.time.Timestamp` / `MicroTimestamp` / `NanoTimestamp` / Connect `Timestamp`
|`TIMESTAMP` or `TIMESTAMP_NTZ` (normalized to epoch micros)

|`io.debezium.time.ZonedTimestamp` (timestamp with time zone)
|`STRING` (ISO-8601 with offset)

|`io.debezium.time.Time` and other time-of-day types
|`BIGINT` (Zerobus does not currently ingest the Delta `TIME` type)

|Bytes / `io.debezium.data.Bits` (binary, blob, varbinary)
|BINARY (encoded as Base64)

|`io.debezium.data.Json` (JSON / JSONB)
|`VARIANT`
...
// Type: reference
// Title: Monitoring the Databricks Zerobus Ingest sink
[[zerobus-monitoring]]
===== Monitoring

You can use JMX metrics and connector log output to monitor the operational status and performance of the Debezium Databricks Zerobus Ingest sink.

The {prodname} Databricks Zerobus Ingest sink supports the following delivery routes: zerobus (gRPC), zerobus-rest (REST), and the <<zerobus-kafka-route, Kafka route>>.
All three routes expose sink metrics over JMX under the object name `debezium.zerobus:type=connector-metrics,context=sink,server=<route>,task=0`, where `<route>` is `grpc` for the `zerobus` route, `rest` for the `zerobus-rest` route, or `kafka` for the Kafka route.
The metrics use the same JMX exposure model as the JDBC sink connector and {prodname} source connectors, so existing JMX monitoring tools such as JConsole, jmxterm, and Prometheus JMX Exporter can read them without any Zerobus-specific configuration.

The sink registers the metrics when it starts, so no option is required to begin collecting them.
Reading them from outside the process is a separate step: the service exposes JMX only after you set the `JMX_HOST` and `JMX_PORT` environment variables, and it starts the bundled Prometheus JMX Exporter only after you set `JMX_EXPORTER_PORT`.
The {link-prefix}:{link-debezium-monitoring}#monitoring-debezium[{prodname} monitoring documentation] provides details about how to expose metrics by using JMX.
You can automate periodic metric logging by setting <<zerobus-metrics-log-interval, `debezium.sink.zerobus.metrics.log.interval`>> or <<zerobus-rest-metrics-log-interval, `debezium.sink.zerobus-rest.metrics.log.interval`>> to a positive value.
After you enable either of these options, at the specified interval the connector writes an `INFO` log message that contains the same metric values that are available through JMX.

The Kafka route is the exception to metrics being collected automatically: because it writes through the generic `kafka` sink, the Zerobus sink is never invoked and cannot instrument itself.
On that route the metrics are not collected until you register the following producer interceptor, which reports the same attributes under `server=kafka`:
...
The interceptor reads the operation and the source timestamp from the record headers, and falls back to scanning the serialized value when the headers are absent.
Because the interceptor runs for every produced record, prefer the headers on a high-volume pipeline: configure the `ExtractNewRecordState` SMT with `add.headers=op,source.ts_ms` in addition to `add.fields`, as shown in <<zerobus-kafka-route, Kafka route>>.
Zerobus does not persist record headers, so the metadata does not reach the target table, and reading it from a header rather than scanning the value makes the interceptor several times cheaper per record.
To enable periodic metric logging on this route, set `debezium.sink.kafka.producer.zerobus.metrics.log.interval` to a positive value.
The option is the counterpart of `debezium.sink.zerobus[rest].metrics.log.interval`, except that it counts records rather than batches, because a producer interceptor observes individual sends.
Two attributes carry a route-specific meaning: `TotalFlushes` counts broker-acknowledged records, which is the durability boundary on this route, and `TotalRecordsSkipped` remains `0`, because the generic sink forwards every record that it receives.

[NOTE]
====
On the Kafka route the interceptor times one in every 128 records rather than every record, so that `LastFlushDurationMillis` and `MaxFlushDurationMillis` describe a representative sample.
Reading the system clock is significantly more expensive when the producer's network thread reads it concurrently, and on a high-volume pipeline that cost outweighs everything else the interceptor does.
The first record is always timed, so the gauges report a value even on a pipeline that produces few records.
All counters remain exact.
====
The standard `kafka.producer:type=producer-metrics` and `kafka.producer:type=producer-topic-metrics` MBeans remain available, and provide per-topic byte counts and request latencies that complement the sink metrics.
...
|Total number of change events that Zerobus durably accepted, across all tables.
On the gRPC route the sink counts a record only after the flush that covers it succeeds, so a batch that fails to flush never advances this metric.

|`TotalInserts` / `TotalUpdates` / `TotalDeletes` / `TotalReads`
|These metrics report the number of ingested events sorted by operation type (`c` / `u` / `d`, and snapshot reads `r`).

|`TotalRecordsSkipped`
|Number of records skipped without being forwarded (tombstones / null payloads, and events whose destination is not a fully qualified `catalog.schema.table`, such as schema-history or DDL events).
The value remains `0` on the Kafka route, which forwards every record that it receives.

|`TotalErrors`
|Number of ingestion or flush failures observed.

|`TotalFlushes`
|The metric counts one of the following operations, depending on the route configuration:

* Stream flush operations (gRPC route).
* Successful record POST requests (REST route).
* Broker-acknowledged records (Kafka route).

For the REST and Kafka routes, a successful POST or acknowledgement indicates that the record has been durably accepted and is no longer at risk of loss due to a service failure.

|`MilliSecondsBehindSource`
|Indicates the age, in milliseconds, of the most recently processed event.
The value is calculated as `current time - source.ts_ms`.
The metric reports the value `-1` until after the Delta table ingests an event that contains a source timestamp.
Values near `0` indicates that the sink is catching up to, or is synchronized with, the source.

This metric is a primary indicator of pipeline freshness in a CDC-to-lakehouse deployment.

|`MilliSecondsSinceLastEvent`
|Indicates the time that elapsed since the sink last forwarded an event.
The metric reports the value `-1` until the sink forwards the first event.

Use this metric together with `MilliSecondsBehindSource`.
Because `MilliSecondsBehindSource` reports the age of the last event that the sink processed, its value stops changing if the pipeline stops, and, as a result, an idle source cannot be distinguished from a stalled sink.
By contrast, if the value of `MilliSecondsSinceLastEvent` grows while the source database is known to be receiving writes, the pipeline is stalled.

|`LastFlushDurationMillis`
|Duration of the most recent flush (gRPC), record POST (REST), or broker acknowledgement (Kafka) operation.
On the Kafka route the value describes a sampled subset of records rather than every record, as the following note describes.

|`MaxFlushDurationMillis`
|Longest duration of any flush (gRPC), record POST (REST), or broker acknowledgement (Kafka) operation since the task started.
On the Kafka route the value describes a sampled subset of records rather than every record, as the following note describes.

|`ActiveStreams`
|Number of currently open Zerobus streams (one per target table).
For the REST and Kafka routes, which do not open persistent streams, this value is always `0`.

|`Connected`
|Indicates whether the sink currently holds a usable connection to Zerobus.
On the gRPC route the value reflects the state of the SDK client, and on the REST route the state of the HTTP client.
On the Kafka route the value is always `true` while metrics are reported, because a producer interceptor observes records rather than connection state.
To monitor the connection on the Kafka route, use the standard `kafka.producer:type=producer-metrics` attributes.

|`Route`
|The delivery route these metrics belong to (`grpc`, `rest`, or `kafka`).
...
ifdef::community[]
// Type: assembly
// Title: Cassandra connector configuration notes
[id="debezium-server-cassandra-connector"]
=== Cassandra connector configuration notes

The {prodname} Cassandra connector requires additional JVM options at startup and a specific `application.properties` configuration when run with {prodname} Server.

// Type: procedure
// Title: Running {prodname} Server with the Cassandra connector
[id="debezium-server-running-with-cassandra-connector"]
==== Running {prodname} Server with Cassandra connector

{prodname} Server requires specific JVM options to run the Cassandra connector on Java 11 and later.
Set these options through the `JDK_JAVA_OPTIONS` environment variable before starting the server.

Set the `JDK_JAVA_OPTIONS` environment variable to the following options before starting {prodname} Server:
...
// Type: reference
// Title: Sample `application.properties` for running the Cassandra connector with Redis sink
[id="debezium-server-cassandra-connector-sample-application-properties"]
==== Sample of basic `application.properties` for running Cassandra connector with Redis sink

The following `application.properties` example shows the minimum configuration required to run the {prodname} Cassandra connector with a Redis sink.
...
// Type: reference
// Title: Cassandra connector transformation for operation code
[id="debezium-server-cassandra-connector-transformation-for-operation-code"]
...
// Type: concept
// Title: {prodname} Server extensions
[id="debezium-server-extensions"]
== Extensions
...
// Type: concept
// Title: Implementing a new {prodname} Server sink
[id="debezium-server-implementing-a-new-sink"]
=== Implementation of a new sink
...
// Type: concept
// Title: Customizing an existing {prodname} Server sink
[id="debezium-server-customizing-an-existing-sink"]
