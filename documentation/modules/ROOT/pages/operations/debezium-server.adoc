...
|`typed`
|Specifies the shape of the row that the sink writes, which is the data contract of the target table.
Set the value to `typed` to map each change event onto the column types of the target table, as <<zerobus-data-type-mapping, Data type mapping>> describes.
Set the value to `envelope` to land the change event as a raw CDC envelope, so that typed materialization occurs downstream of the table.

The two modes are mutually exclusive, because a table is created for one of them.

[IMPORTANT]
====
Do not apply the `ExtractNewRecordState` SMT when you set the value to `envelope`.
The transformation removes the `op` field of the change event, so the sink cannot determine the operation and records every row with `operation=change`.
...
|`json`
|Specifies the encoding of a record on the wire.
The `typed` payload mode writes `json`.
The sink reports an error at start-up if you combine the `typed` mode with the `protobuf` format.
