...
Source configuration properties control which connector {prodname} Server runs and how it stores offsets and schema history outside of Kafka Connect.
...
// Type: reference
// ModuleID: debezium-server-additional-configuration-options
// Title: {prodname} Server additional configuration options
...
https://oauth.net/2/client-credentials/[OAuth2 client credentials] authentication, and batch mode for sending multiple
events in a single HTTP request.
...
|
|Optional additional parameters to include in the token request. For example, some providers require an `audience` or `resource` parameter. Set these using the `params.` prefix, e.g. `debezium.sink.http.authentication.oauth2.params.audience=\https://my-api.example.com`.
...
ifdef::community[]
// Type: assembly
// Title: Running the {prodname} Cassandra connector with {prodname} Server
