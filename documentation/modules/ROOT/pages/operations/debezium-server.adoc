...
A third option is to reuse the built-in `kafka` sink against the Zerobus Kafka-compatible endpoint.
...
Throughput on this route is bound by network round trips rather than by local processing, because Zerobus acknowledges each request from the region in which the workspace runs.
With the default producer settings, the number of records that the service ingests per second is approximately the number of in-flight requests, multiplied by the number of records per request, and divided by the request latency.
As a result, a pipeline that produces one record at a time spends most of its time waiting.

The following producer properties control that behavior.
Because the optimal values depend on the latency between the service and the workspace, treat the values in the following table as a starting point, and then measure.
...
|`100`
|Specifies how long the producer waits to accumulate records before it sends a request.
The default value, `0`, sends each record as soon as possible, so every request pays a full round trip to carry very few records.

Because this setting delays a record by up to the configured duration, it trades latency for throughput.
On a change data capture pipeline, in which a round trip to the workspace already dominates, the added delay is typically negligible.

|`debezium.sink.kafka.producer.batch.size`
|`1048576`
|Specifies the maximum size, in bytes, of a single batch.
Set this option together with `linger.ms`: at the default value of `16384`, a batch of even moderately sized change events fills, and is therefore sent, before the configured wait elapses, which limits the effect of the preceding setting.
Zerobus rejects a record that is larger than 10 MB, so a batch size of 1 MB leaves ample headroom.

|`debezium.sink.kafka.producer.max.in.flight.requests.per.connection`
|`10`
|Specifies how many requests can be awaiting a response at the same time, which sets how much of the round-trip latency the producer can overlap.

A value greater than `5` is permitted here only because this route requires `enable.idempotence=false`.
Increasing the value relaxes the ordering of records that are retried.
Zerobus is partitionless and provides an at-least-once guarantee, so a pipeline on this route cannot depend on strict ordering in any case.
|===
...
|`TotalFlushes`
|The metric counts stream flush operations when using the gRPC route, successful record POST requests when using the REST route, or broker-acknowledged records when using the Kafka route.
