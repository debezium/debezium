...
|`100`
|Maximum number of Zerobus streams that the sink keeps open at one time.
Zerobus binds one stream to one table, so a source that captures many tables would otherwise hold an unbounded number of connections.
When the number of tables exceeds this limit, the sink flushes and closes the least recently used streams, and reopens them on demand.
Set the value to `0` to keep every stream open.
...
Several properties in the preceding table list "The SDK default" as their default value.
If you do not add such a property to the configuration, the sink does not pass a value for it to the Zerobus SDK, and the default of the SDK version in use applies.
If you add the property and set a value, the sink passes your value instead.
The sink deliberately avoids pinning these values so that they cannot drift from the SDK across upgrades.
