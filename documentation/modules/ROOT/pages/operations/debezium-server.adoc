...
|`token`
|No
|A bearer token (for example, a personal access token) to use verbatim.
When set, the handler skips the OAuth exchange and uses this token directly.
|===
...
Recommended Delta column types:

[cols="45%a,55%a",options="header"]
|===
|Source / Debezium logical type
|Delta column type
