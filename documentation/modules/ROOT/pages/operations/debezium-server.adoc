...
It's designed for high-load applications and allows you to efficiently store, manage, and search vector embeddings. You can use Qdrant to process vector datatypes that are captured from a source database, or use it with a transformation to calculate vectors from message fields and then use them as embeddings.

Qdrant sink ingests incoming messages and upserts the `after` part of each message into a collection.
When a delete message arrives, the matching record is removed from the collection.

The sink follows these rules

* Each Debezium collection/table is mapped to a Qdrant collection
* Primary keys are required and used as Qdrant point IDs (only `INT64` and `UUID` are supported)
* `FloatVector` and `DoubleVector` can be used as source for Qdrant vector
* Non-primary key and non-vector fields are mapped to Qdrant payload
...
|
|(Optional) The API key used to authenticate to access the Qdrant database instance.

|[[qdrant-vector-field-names]]<<qdrant-vector-field-names, `debezium.sink.qdrant.vector.field.names`>>
|
|(Optional) Comma-separated list of `collection-name:field-name` pairs to explicitly define which vector field should be used for given collection. +
This is mandatory for source tables/collections with more than one vector field.

|[[qdrant-field-include-list]]<<qdrant-field-include-list, `debezium.sink.qdrant.field.include.list.<collection-name>`>>
|
|(Optional) Comma-separated list of a subset of field names for given collection that will form the Qdrant collection payload.
...
You can modify the behavior of the Qdrant sink connector by applying custom logic that provides alternative implementations for specific functions.
If the alternative implementations are not available, the connector uses the default implementations.
...
InstructLab is a community-driven approach to build truly open-source large language models (LLMs).
InstructLab augments models using **taxonomies**, a collection of skills and knowledge to help to identify gaps in desired capabilities and used to tune base models efficiently.

Debezium Server supports automating the addition of skills and knowledge directly into taxonomies by creating or updating InstructLab's question and answer (`qna.yml`) files.
...
The mappings can source values directly from fields within the event payload, headers, or statically configured constants.
...
|
|The absolute path to the root directory where taxonomy skills and knowledge are stored.
This will be used in conjunction with taxonomy domain properties to construct the full path to the `qna.yml`.

|[[instructlab-taxonomies]]<<instructlab-taxonomies, `debezium.sink.instructlab.taxonomies`>>
|
|The comma separated list of symbolic names of taxonomy mappings.

|[[instructlab-taxonomy-topic]]<<instructlab-taxonomy-topic, `debezium.sink.instructlab.taxonomy._<name>_.topic`>>
|`.*`
|A regular expression used to match topics to determine if the _<name>_ taxonomy mapping is applied.

|[[instructlab-taxonomy-question]]<<instructlab-taxonomy-question, `debezium.sink.instructlab.taxonomy._<name>_.question`>>
|
|Specifies a xref:#instructlab-mapping-definition[] that will be used as the seed example's `question` attribute in the `qna.yml` file.
This is required.

|[[instructlab-taxonomy-answer]]<<instructlab-taxonomy-answer, `debezium.sink.instructlab.taxonomy._<name>_.answer`>>
|
|Specifies a xref:#instructlab-mapping-definition[] that will be used as the seed example's `answer` attribute in the `qna.yml` file.
This is required.

|[[instructlab-taxonomy-context]]<<instructlab-taxonomy-context, `debezium.sink.instructlab.taxonomy._<name>_.context`>>
|
|Specifies a xref:#instructlab-mapping-definition[] that will be used as the seed example's `context` attribute in the `qna.yml` file.
This is optional.
...
For example, a value of `a/b` and a base path of `/taxonomy` will represent `/taxonomy/a/b/qna.yml`.
...
A mapping definition represents one of three options:

Value::
When the mapping is prefixed with `value:`, it is interpreted as a specific field in the event's payload that supplies the value used.
For example, given a mapping of `value:abc`, the sink expects a field named `abc` to source the value from.
If the event is a {prodname}-structured payload, the field will be sourced from the `after` section of the payload.
If the event is flattened, the field will be sourced directly from the event's value.

Header::
When the mapping is prefixed with `header:`, it is interpreted as a specific header in the event that supplies the value used.
For example, given a mapping of `header:h1`, the sink expects a header named `h1` to source the value from.

Constant::
Any value that is not prefixed with `header:` or `value:` will be treated as a constant value, and will be used as-is.
This permits setting a specific mapping to a static, constant value via configuration.
