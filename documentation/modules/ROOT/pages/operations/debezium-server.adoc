...
|[[redis-trim-maxlen]]<<redis-trim-maxlen, `debezium.sink.redis.trim.maxlen`>>
|
|(Optional) Evicts entries as long as the stream's length exceeds the specified threshold.

*Note: when set, consumers may not receive all change events, and
events might be evicted from the stream before they are processed.*

|[[redis-trim-relative-minid-ms]]<<redis-trim-relative-minid-ms, `debezium.sink.redis.trim.relative.minid.ms`>>
|
|(Optional) Duration in which to keep the events
for trimming the stream. For example, if you want to continuously
evict all items older than 1 hour on each; use `3600000`.

*Note: when set, consumers may not receive all change events, and
events might be evicted from the stream before they are processed.*

|[[redis-trim-exact]]<<redis-trim-exact, `debezium.sink.redis.trim.exact`>>
| `false`
|Whether to trim exactly the amount specified (performance
may be slower with this turned on).

|[[redis-trim-limit]]<<redis-trim-limit, `debezium.sink.redis.trim.limit`>>
|
|When `redis.trim.maxlen` or `redis.trim.relative.minid` is set and
`redis.trim.exact` is set to `false`, this is the maximum
count of events that will be evicted in one operation.

Read more here: https://redis.io/commands/xtrim#nearly-exact-trimming[Redis: XTRIM Exact]
|===
