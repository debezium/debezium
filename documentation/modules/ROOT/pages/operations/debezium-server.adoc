...
- `default-azure-credential`: Authenticate using Azure Entra ID via `DefaultAzureCredential`. When this mode is used, `debezium.sink.eventhubs.fullyqualifiednamespace` and `debezium.sink.eventhubs.hubname` must be set. The identity running {prodname} must have been granted the **Azure Event Hubs Data Sender** role on the namespace or the target event hub.
