...
The {prodname} DataBricks Zero sink supports the following delivery routes: zerobus (gRPC) and zerobusrest (REST).
Both routes expose sink metrics over JMX under the object name debezium.zerobus:type=connector-metrics,context=sink,server=<route>,task=0, where <route> is `grpc` for the `zerobus` route or `rest` for the `zerobusrest` route.
The metrics use the same JMX exposure model as the JDBC sink connector and {prodname} source connectors, enabling you to use existing JMX monitoring tools such as JConsole, jmxterm, and Prometheus JMX Exporter without additional configuration.
...
|`TotalFlushes`
|The metric counts stream flush operations when using the gRPC route, or successful record POST requests when using the REST route.
For the REST route, a successful POST indicates that the record has been durably accepted and is no longer at risk of loss due to a service failure.
...
|Duration of the most recent flush (gRPC) or record POST (REST) operation.

|MaxFlushDurationMillis
|Longest duration of any flush (gRPC) or record POST (REST) operation since the task started.

|`ActiveStreams`
|Number of currently open Zerobus streams (one per target table).
For the REST route, which does not open persistent streams, this value is always `0`.

|`Route`
|The delivery route these metrics belong to (`grpc` or `rest`).
