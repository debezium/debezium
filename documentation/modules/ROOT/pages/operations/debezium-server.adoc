...
{prodname} provides a ready-to-use application that streams change events from a source database to messaging infrastructure like Amazon Kinesis, Amazon SNS, Google Cloud Pub/Sub, Apache Pulsar, Redis (Stream), or NATS JetStream.
...
https://aws.amazon.com/sns/[Amazon Simple Notification Service (SNS)] is a managed pub/sub messaging system with support for standard and FIFO topics, fan-out delivery to multiple subscribers, and message filtering.
SNS exposes a set of REST APIs and provides a Java SDK that is used to implement the sink.
...
|
|The AWS region in which the SNS topics are provisioned (e.g. `us-east-1`, `eu-central-1`).

|[[sns-endpoint]]<<sns-endpoint, `debezium.sink.sns.endpoint`>>
|_endpoint determined by AWS SDK_
|(Optional) A custom endpoint URL for the SNS service.
Useful for local development with tools such as https://localstack.cloud/[LocalStack] (e.g. `\http://localhost:4566`).
...
If not present, the https://docs.aws.amazon.com/sdk-for-java/latest/developer-guide/credentials-chain.html[default credentials provider chain] is used.
...
|
|(Optional) The default SNS topic ARN to which all messages are published when the resolved destination is neither a full ARN nor combined with a prefix.
...
If not configured or the header is absent, no deduplication ID is set and SNS relies on content-based deduplication (if enabled on the topic).
...
The SNS sink resolves the target topic ARN for each event using the following strategy:

First, the destination name is determined.
By default it is derived from the source topic (e.g. `myapp-inventory-products`).
When using the xref:transformations/outbox-event-router.adoc[Outbox Event Router], the destination name is the value of the `aggregatetype` column (e.g. `user`, `notification`).

Then, the ARN is resolved from the destination name using the first matching rule:
...
This allows flexible routing strategies, from publishing all events to a single topic, to dynamically routing to per-table topics using a prefix.
If you need more advanced topic name mapping, you can provide a custom implementation via the injection points described below.
...
The SNS sink automatically detects FIFO topics when the resolved topic ARN ends with `.fifo` and populates the required `MessageGroupId` field.

For ordering, the `MessageGroupId` is resolved with the following precedence:

1. The value of the header specified by `debezium.sink.sns.fifo.message.group.id.header` (defaults to `aggregateId`).
2. The event key.
3. The value of `debezium.sink.sns.fifo.default.group.id` (defaults to `default`).

For deduplication, if `debezium.sink.sns.fifo.message.dedup.id.header` is configured and the corresponding header is present, its value is used as the `MessageDeduplicationId`.
...
The SNS sink groups events by destination topic ARN and publishes them using the https://docs.aws.amazon.com/sns/latest/api/API_PublishBatch.html[SNS PublishBatch API] in batches of up to 10 messages (the SNS API limit).
Each individual message must not exceed 256 KB; the sink validates the payload size before publishing and raises an error if the limit is exceeded.

===== Injection points

The SNS sink behaviour can be modified by a custom logic providing alternative implementations for specific functionalities.
When the alternative implementations are not available then the default ones are used.
...
|`@CustomConsumerBuilder`
|Custom configured instance of a `SnsClient` used to publish messages to target topics.

|[[sns-ext-stream-name-mapper]]<<sns-ext-stream-name-mapper, `io.debezium.server.StreamNameMapper`>>
|
|Custom implementation maps the planned destination (topic) name into a physical SNS topic name or ARN.
By default the same name is used.
...
All change events are published to a single standard SNS topic:
...
Each source table receives its own SNS topic.
The topic ARN is composed from the prefix and the destination name:
...
Use the xref:transformations/topic-routing.adoc[Topic Routing] transformation to replace dots with hyphens (or another valid character) before the destination name is resolved to an ARN.
...
This example configures {prodname} Server to capture changes from a MySQL outbox table and publish them to SNS.
