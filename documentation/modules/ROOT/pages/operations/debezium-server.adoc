...
If a property value contains a `++${...}++` syntax and should not be expanded, the `$` character must be escaped using `++$$++` so that the value is written as `++$${name}++`.
This syntax guarantees that the property value is passed to {prodname} as `++${name}++` without expansion.
====
