...
For testing purposes, you can use the following, simplified and unsecured, approach.

* First, https://strimzi.io/downloads/[download] the latest Strimzi release and unpack it into a local directory
* Create a dedicated namespace for the Strimzi Kafka Cluster Operator:
...
* Modify the downloaded source files to use the `kafka` namespace for the installation of the Strimzi Kafka Cluster Operator:
...
* Set the `STRIMZI_NAMESPACE` variable in `install/cluster-operator/060-Deployment-strimzi-cluster-operator.yaml` to `debezium-example`:
...
[source,subs="attributes",options="nowrap"]
----
$ kubectl run -n debezium-example -it --rm --image=quay.io/debezium/kafka:{debezium-docker-label}  --restart=Never watcher -- watch-topic -a -k mysql.inventory.customers
----
