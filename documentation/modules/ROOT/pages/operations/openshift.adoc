...
This procedure is for setting up {prodname} connectors on Red Hat's https://www.openshift.com/[OpenShift] container platform. These instructions have been tested by using the https://github.com/minishift/minishift[Minishift tool], which allows you to run a single node OpenShift instance locally on your machine.

A complete example of this set-up using Minishift is in the https://github.com/debezium/debezium-examples/tree/master/openshift[{prodname} examples repository].

To get started even more quickly, try out the https://learn.openshift.com/middleware/debezium-getting-started/[{prodname} online learning scenario].
It starts an OpenShift cluster just for you, which lets you start using {prodname} in your browser in just a few minutes.

== {prodname} Deployment

To set up Kafka and Kafka Connect on OpenShift, use the set of images that are provided by the https://strimzi.io/[Strimzi] project. These images offer "Kafka as a Service" by providing enterprise grade configuration files and images that bring Kafka to OpenShift.
...
. Deploy a Kafka broker cluster and a Kafka Connect cluster and then create a Kafka Connect image with the {prodname} connectors installed:
+
[source,shell,subs="attributes",options="nowrap"]
----
...
for PLUGIN in {mongodb,mysql,postgres,sqlserver}; do \
