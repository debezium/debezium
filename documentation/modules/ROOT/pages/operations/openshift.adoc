...
For the {prodname} deployment we will use https://strimzi.io/[Strimzi] project, which manages the Kafka deployment on OpenShift cluster.
The simplest way for installing Strimzi is to install Strimzi operator.
You can install it from OpenShift UI.
In OperatoHub tab select Strimzi and click on install button.
If you prefer command line tools, you can install Strimzi operator this way as well:
...
Here we took the advantage of the OpenShift build-in registry, already running as a service on the OpenShift cluster.
