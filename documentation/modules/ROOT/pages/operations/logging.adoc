...
.Procedure
. Dynamically changing logging level

Kafka Connect allows to change logging level dynamically without restarting your server. For doing that, you can send a PUT request to the admin/loggers endpoint providing the package name for which you want to change log level. For example, if by any reason you wanted to get debug information for the {prodname} Oracle connector you could send this request to your Kafka Connect:

[source,shell,options="nowrap"]
----
curl -s -X PUT -H "Content-Type:application/json" http://localhost:8083/admin/loggers/io.debezium.connector.oracle.logminer -d '{"level": "DEBUG"}'
----

.Changing logging level permanently
