...
{prodname} uses Log4j loggers, log levels, and appenders to control which messages appear in logs and where they are written.
Logging configuration specifies the level for each logger and the appenders where {prodname} writes log messages.
Because loggers are hierarchical, the root logger configuration serves as a default for all loggers in the hierarchy, but you can override individual child or descendant loggers.
...
Descendants in the logging hierarchy inherit the configuration of the root logger by default.
