...
.Dinamically changing logging level

Kafka Connect allows to change logging level dinamically without restarting your server. For doing that, you can send a PUT request to the admin/loggers endpoint providing the package name for which you want to change log level. For example, if by any reason you wanted to get debug information for the Oracle connector you could send this request to your Debezium server:
