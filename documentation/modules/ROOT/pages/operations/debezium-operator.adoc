  ...
  runtime:
    ...
    metrics:
      ...
      openTelemetry:
        ...
        collector:
          endpoint: String
          jmxIntervalMs: int  # defaults to 10000
          metricExportIntervalMs: int  # defaults to 60000
    templates:
      container:
...
| [[debezium-operator-schema-reference-metrics-opentelemetry]]<<debezium-operator-schema-reference-metrics-opentelemetry, `+openTelemetry+`>>
| <<debezium-operator-schema-reference-opentelemetry, `+OpenTelemetry+`>>
| No default value
...
[id="debezium-operator-schema-reference-opentelemetry"]
=== OpenTelemetry schema reference
Used in: <<debezium-operator-schema-reference-metrics, `+Metrics+`>>

When OpenTelemetry is enabled, the operator automatically configures the {prodname} Server container with the following environment variables:

* `OTEL_ENABLED` -- Activates the OpenTelemetry Java Agent bundled with {prodname} Server.
...
* `OTEL_JMX_INTERVAL_MILLISECONDS` -- JMX metrics scrape interval in milliseconds (defaults to `10000`).
* `OTEL_METRIC_EXPORT_INTERVAL` -- Metrics export interval in milliseconds (defaults to `60000`).

You can override any of these environment variables by specifying them in `spec.runtime.environment.vars`.
User-provided environment variables take precedence over operator-managed defaults.
...
| [[debezium-operator-schema-reference-opentelemetry-enabled]]<<debezium-operator-schema-reference-opentelemetry-enabled, `+enabled+`>>
| boolean
| false
| Enables OpenTelemetry

| [[debezium-operator-schema-reference-opentelemetry-collector]]<<debezium-operator-schema-reference-opentelemetry-collector, `+collector+`>>
| <<debezium-operator-schema-reference-otelcollector, `+OtelCollector+`>>
| No default value
| OpenTelemetry collector configuration
|===

[id="debezium-operator-schema-reference-otelcollector"]
=== OtelCollector schema reference
Used in: <<debezium-operator-schema-reference-opentelemetry, `+OpenTelemetry+`>>


.OpenTelemetry collector properties
[cols="20%a,25%s,15%a,40%a",options="header"]
|===
| Property | Type | Default | Description

| [[debezium-operator-schema-reference-otelcollector-endpoint]]<<debezium-operator-schema-reference-otelcollector-endpoint, `+endpoint+`>>
| String
| http://localhost:4318
| OTLP collector endpoint

| [[debezium-operator-schema-reference-otelcollector-jmxintervalms]]<<debezium-operator-schema-reference-otelcollector-jmxintervalms, `+jmxIntervalMs+`>>
| int
| 10000
| JMX metrics scrape interval in milliseconds

| [[debezium-operator-schema-reference-otelcollector-metricexportintervalms]]<<debezium-operator-schema-reference-otelcollector-metricexportintervalms, `+metricExportIntervalMs+`>>
| int
| 60000
...
| [[debezium-operator-schema-reference-containertemplate-imagepullpolicy]]<<debezium-operator-schema-reference-containertemplate-imagepullpolicy, `+imagePullPolicy+`>>
| Always,IfNotPresent,Never
| No default value
| https://kubernetes.io/docs/concepts/containers/images/#image-pull-policy[Image pull policy] for the container.
|===
