...
Connection:: Defines the connection for your source and destination.
...
Use the **Connection** section of the UI to define the connection for both the database that hosts your data and your data sink.
You can configure any connection for your database that {prodname} supports as a source or any {prodname} Server sinks are that available as a destination.
The connection that you create can be shared among multiple sources and destinations.

[id="debezium-platform-creating-a-connection"]
==== Creating a connection

You can create any source or destination connection provided in the connection catalog list.
The catalog lists all the databases that {prodname} supports as a source and {prodname} Server sinks that are available as destinations.

You can filter the catalog list based on type i.e **Source** or **Destination**.
Once you select a connection type you can configure the connection in the next step. 
In the configuration form you can specify the connection name and predefined properties. 
There is another section called **Additional properties** where you can add any custom properties.

[.responsive]
video::CVY4Y4kAs_E[youtube, title="Create, edit and remove a connection"]


After you have filled the configured properties, you have to **Verify** the connection configuration it validate that a 
a proper connections is established with database source or sink.
Once the connection is successfully verified you can click on **Create connection** .
...
To edit a connection, from the platform UI, open to the **Connection** menu, click the **Action** menu of the connection that you want to edit, and then click **Edit**.
...
For connection you can select the predefined source connection or can create a new by one clicking **Create connection** 
which will open the **Create connection** modal. For more information, see xref:debezium-platform-creating-a-connection[Creating a connection]. +
...
In {prodname} management platform to use signal to perform a specified action we use the **SourceSignalChannel** configuration in the source connector.

To enable the signalling capability for the data pipeline you need to **Setup signalling** in the source connector configuration. 

image::Debezium-platform-signalling.png[Edit source confirmation modal]

Here you issue a SQL command to add a signal message to a specialized signaling data collection.
The signaling data collection, which you create on the source database, is designated exclusively for communicating with Debezium. 
...
In the modal you have to provide the **Signaling collection name** and run the provided DDL query to create the signaling data collection on the source database. 
Before saving the **signalling collection name** in the source configuration you can verify it in the UI. Once verification is successful click *Done* to save the signalling.
...
For connection you can select the predefined destination connection or can create a new by one clicking **Create connection** 
which will open the **Create connection** modal. For more information, see xref:debezium-platform-creating-a-connection[Creating a connection]. +
...
In the pipeline logs you can download the logs, toggle to full screen view, search the log for a relevant section or look for the notifications i.e [Notification Service] using **Grep notifications** button 

==== Pipeline actions

The {prodname} platform provides an easy way to perform any action on the data pipeline.
To use signals to trigger a connector to perform a specified action, 
From the UI, go to the pipeline overview page, then click **Pipeline actions** tab.

image::Debezium-platform-action-list.png[{prodname} platform - Pipeline action list]

In the UI, select any action from the dropdown, once you select an action relevant form with required fields will appear.
**Action Id** field will be prefilled with UUID string, but its editable field so that if needed you can update the **Action Id** 
just to keep in mind that **Action Id** need to be unique for each action. 
After filling all the required fields click **Submit** to trigger an action.

[NOTE]
====
In order to be able to perform action on the pipeline, you need to enable the Signaling for the source connector. 
For more information see xref:debezium-platform-enable-signalling[Enable signalling].
====
