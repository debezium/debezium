...
You can configure a connection for any database that {prodname} supports as a data source, or as a destination sink.
...
. Click **Add connection** to get to **Connection catalog** page.
. Click **Source** or **Destination** to filter the list by connection type then select the required connection card.
. In the configuration form, specify the connection name and predefined properties.
In the **Additional properties** section, specify any custom properties.
...
.Prerequisites
	...
	* Signaling is enabled for the source connector. For more information see xref:debezium-platform-enable-signalling[Enabling signaling].

The {prodname} platform provides an easy way to perform any action on the data pipeline.
You can use signals to trigger a connector to perform specified actions. 
