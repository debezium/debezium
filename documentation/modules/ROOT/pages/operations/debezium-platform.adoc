...
Source:: Specifies from which you want {prodname} to read data.
...
For a connection, you can select the predefined source connection, or click **Create connection** 
...
For more information on signaling see {link-prefix}:{link-signalling}#debezium-signaling-enabling-source-signaling-channel[Enabling source signaling channel]
...
. Click *Verify* to check if signaling data collection is correctly setup.
...
====
. Complete the required fields, and then click **Submit** to trigger an action.
