...
**Smart Editor**:: Enables you to define the source configuration in JSON. It also provides the ability to directly use the configuration of a {prodname} connector on Kafka Connect or in {prodname} Server by automatically formatting it into the required JSON format for the platform.

[NOTE]
====
image::create-source-toolbar.png[{prodname} platform - Create source toolbar]
If you want to directly use the configuration of a {prodname} connector on Kafka Connect or {prodname} Server configuration for creating source then select the **Create using smart editor** button in the toolbar in **Source catalog** page.
====
...
With a few small differences, the JSON that you use to configure a data source in the {prodname} management Platform is nearly identical to the JSON in the `config` section that defines the configuration for a {prodname} connector on Kafka Connect or in the {prodname} Server.

Now stage UI provides you the ability to directly use the configuration of a {prodname} connector on Kafka Connect or in {prodname} Server configuration by either uploading or pasting into the **Smart Editor**.
Stage UI will intelligently recognize the configuration format and will provide the appropriate prompt in the UI along with **Auto format** action button in the **Smart Editor** toolbar to automatically convert provided JSON into the required JSON format for the platform.  
...
When you add this JSON to the **Smart Editor**, it intelligently recognizes that the provided JSON is in Kafka Connect format and will inform via displaying an alert.

image::Debezium-platform-smart-editor-kafka.png[{prodname} UI smart editor - Kafka connect]

The Smart Editor toolbar will then display an **Auto Format** option (highlighted in the image above). 
You can use this option to automatically convert the provided JSON into the required {prodname} Platform JSON format.

The following image shows the auto formatted JSON that you would use to define a MySQL data source where optionally you can update the name or add some appropriate description in the JSON.
...
Similarly you can also use the {prodname} Server configuration in the **Smart Editor** where it will automatically extract the source configuration and provide you {prodname} platform supported JSON format.
This example is covered in the destination xref:debezium-platform-configuring-a-destination-with-the-smart-editor[see here].
...
[NOTE]
====
Editing a source will affect all pipelines that use it. A confirmation modal will also appear informing this before changes are being submitted from the UI.
...
**Smart Editor**:: Enables you to define the sink configuration in JSON. It also provides the ability to directly use the {prodname} Server configuration by automatically extracting the sink configuration from it and formatting it into the required JSON format for the platform.
...
In the pipeline designer click on **DBZ server config** which will open a modal where you can upload or drag and drop your {prodname} server properties file. Once you upload the file click on **Create** button.
