...
* Connector table shows each connector with it's type (MongoDB, MySQL, Oracle, PostgreSQL and SQL Server), connector status and connector tasks.
...
* *Transformations* - enable single message at a time modification. This can use to to modify records before they are sent to Apache Kafka by applying them to the {prodname} connectors. Read more about the transformation here xref:{link-transformations}[transformations]
* *Topic creation* - define automatic topic creation default properties and groups.
...
* *Custom properties* - enable pass-thru of additional properties that are not covered by the previous sections can be entered here e.g database.* and database.history.* options. The user can enter any valid property name and value.

==== Create Connector - Review
The *Review* step provides a summary of the configuration that will be used to create the connector.  If happy with the selections, click 'Finish' to create the connector.  If the properties need adjustment, navigate back to the earlier steps. The user can also copy or download the JSON configuration payload for the connector.
