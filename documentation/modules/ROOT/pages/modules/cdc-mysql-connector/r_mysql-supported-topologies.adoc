...
IMPORTANT: Because these hosted options do not allow a *global read lock*, table-level locks are used to create the _consisten snapshot_. See xref:no-global-read-lock-mysql-connect_{context}[What to do with no global read lock] for more details.
====
