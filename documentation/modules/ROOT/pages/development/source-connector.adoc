...
This guide maps those contracts to the concrete classes that you provide, using a fictional source named AcmeDB (`AcmeDbConnector` and its companion classes) for the code snippets and the connectors in the main {prodname} repository as production references, in particular:
...
Every subsection states the contract, shows a short AcmeDB snippet, and points at the SQL Server and PostgreSQL implementations for the production-grade details.

[NOTE]
====
{prodname} connectors prefix the classes that implement a framework contract with the name of the source, for example `SqlServerConnector`, `SqlServerConnectorTask`, and `SqlServerOffsetContext`, or `PostgresConnector` and `PostgresOffsetContext`.
The snippets in this guide follow the same convention for a fictional source named AcmeDB: `AcmeDbConnector`, `AcmeDbConnectorTask`, `AcmeDbOffsetContext`, and so on, in the package `io.debezium.connector.acmedb`.
A few helper classes, such as `SourceInfo` and `Module`, are conventionally left unprefixed.
Replace `AcmeDb` with the name of your source throughout.
====
...
image::source-connector-dispatch-flow.svg[Change event dispatch flow from the streaming source through the EventDispatcher and ChangeEventQueue to the runtime]
...
services:
  ...
  connect:
    ...
    volumes:
      - ./target/acmedb-connector:/kafka/connect/acmedb-connector
----
...
----
{
    "name": "acmedb-connector",
    "config": {
        "connector.class": "io.debezium.connector.acmedb.AcmeDbConnector",
        "tasks.max": "1",
        "topic.prefix": "acmedb",
        "acmedb.property": "value"
    }
}
----
+
...
curl -i -X POST -H "Accept:application/json" -H "Content-Type:application/json" \
  http://localhost:8083/connectors/ -d @register-acmedb-connector.json
----
...
[source,properties]
----
debezium.source.connector.class=io.debezium.connector.acmedb.AcmeDbConnector
debezium.source.topic.prefix=acmedb
debezium.source.acmedb.property=value
