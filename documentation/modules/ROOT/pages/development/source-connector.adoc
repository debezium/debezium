...
| Standard {prodname} envelope (`op`, `before`, `after`, `source`, `ts_ms`)
...
Kafka Connect runs any source connector, and xref:{link-debezium-server}[{prodname} Server] and the xref:{link-engine}[{prodname} Engine] embed the Kafka Connect API, so they can also run source connectors.
...
. When the snapshot completes, the coordinator starts the `StreamingChangeEventSource` at the offset where the snapshot finished, and streaming emits `CREATE` (`c`), `UPDATE` (`u`), and `DELETE` (`d`) events from that point on.
. Emitted events flow through the `ChangeEventQueue` into the Kafka Connect polling loop, and Kafka Connect periodically persists the offsets that your `OffsetContext.getOffset()` returns.
...
public SnapshotResult<MyOffsetContext> execute(ChangeEventSourceContext context, MyPartition partition,
    ...
    for (SourceRow row : readCurrentState()) {                  // your source access
...
Relational connectors extend `JdbcValueConverters`.
| `SqlServerValueConverters`, `PostgresValueConverter`
