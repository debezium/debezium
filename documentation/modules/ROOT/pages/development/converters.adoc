...
By default, {prodname} connectors map source data types to Kafka Connect schema types using built-in type mappings.
You can override these defaults by developing and deploying a custom converter when your downstream application requires a different data format.
...
The following list describes the purpose of select elements in the preceding Java converter class example:
...
The following example shows how to implement a converter that maps source columns of type `isbn` to Kafka Connect `STRING` schema values.
Use this example as a model for writing converters that handle custom or non-standard column types in your source database.

The converter in the example performs the following operations:
...
The following list describes the purpose of the compile dependency versions in the preceding `pom.xml`example:

`${version.debezium}`::
Represents the version of the {prodname} connector.

`${version.kafka}`::
Represents the version of Apache Kafka in your environment.
...
To use a custom converter with a {prodname} connector, export the converter as a JAR file and place it in the connector's plug-in directory.
...
[subs="+quotes"]
----
converters: _<converterSymbolicName>_
_<converterSymbolicName>_.type: _<fullyQualifiedConverterClassName>_
----
+
The following list describes the properties in the preceding example:
