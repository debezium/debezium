...
Basically, you create the `DebeziumEngine` with a configuration (perhaps loaded from a properties file) that defines the environment for both the engine and the connector.
You also provide parametrize the engine with the format int which you want to receive the messages (currently only `Connect` is supported) and with a function that it will call for every data change event produced by the connector.
...
A fluent-style builder API is used to create an engine that uses our `Properties` object and that sends all data change records to the `handleEvent(SourceRecord)` method, which can be any method that matches the signature of the `java.util.function.Consumer<SourceRecord>` functional interface, where `SourceRecord` is the `org.apache.kafka.connect.source.SourceRecord` class.
This example speaks specifically about `SourceRecord` but the engine implementation is parametrized but different types like `JSON` or `Avro` could be returned by the engine.
...
The engine's connector will stop reading information from the source system, forward all remaining `SourceRecord` objects to your handler function, and flush the latest offets to offset storage.
...
* `Connect.class` - the output value is Kafka Connect's `SourceRecord`
...
Internally the engine uses apropriate implementations of Connect converter to wich the conversion is delegated.
The convertor can be parametrized using engine properties to modify its behaviour.
...
Where the `Change` datatype is the key/value pair.
