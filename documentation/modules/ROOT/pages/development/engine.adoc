...
|`schema.history.file.filename`
|`""`
|Path to a file where the database schema history is stored. +
Required when `schema.history` is set to the `<...>.FileDatabaseHistory`.

|`schema.history.kafka.topic`
|`""`
|The Kafka topic where the database schema history is stored. +
Required when `schema.history` is set to the `<...>.KafkaDatabaseHistory`.
