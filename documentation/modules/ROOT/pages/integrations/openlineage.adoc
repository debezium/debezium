...
Data lineage tracks where data originates, how it moves between systems, and what transformations it undergoes.
OpenLineage is the open standard that {prodname} uses to emit and share that lineage metadata across your data pipeline.
...
{prodname} maps its internal objects and events to the OpenLineage data model, representing connectors as Jobs and database tables or Kafka topics as Datasets.
The following sections describe how those mappings work for source connectors, sink connectors, and the connector lifecycle.

// Type: concept
[id="debezium-openlineage-job-mapping"]
=== OpenLineage job mapping

Each {prodname} connector instance is represented as an OpenLineage Job, which captures the connector's identity, version, and full configuration to support lineage tracing and pipeline debugging.
...
For source connectors, {prodname} maps the database tables it monitors to OpenLineage input datasets and the Kafka topics it produces to output datasets, capturing schema information for both.
...
Before you can use the {prodname} OpenLineage integration, install the required dependencies for your deployment type — either Kafka Connect or {prodname} Server.
...
// Type: procedure
[id="debezium-openlineage-deploy-the-integration-on-streams"]
=== Deploy the {prodname} OpenLineage integration on {StreamsName}

In a {StreamsName} environment, you can deploy the integration by adding the Maven URL for the OpenLineage artifact to the Kafka Connect custom resource YAML file.

.Procedure

* Add the OpenLineage artifact by inserting te following lines in the `spec.build` section of the YAML that defines your Kafka Connect CR.
...
...
    ...
    plugins:
      - name: debezium-connector
        artifacts:
          - type: zip
            url: {red-hat-maven-repository}debezium/debezium-openlineage-core/{debezium-version}.Final/debezium-openlineage-core-{debezium-version}.Final-libs.zip
----

.Additional resources
* xref:debezium-openlineage-configuring-the-integration[]
* xrefdebezium-openlineage-configuring-the-openlineage-client[]
endif::product[]
...
You can verify that the {prodname} OpenLineage integration is working correctly and diagnose problems by inspecting connector logs, reviewing OpenLineage events, and resolving common configuration issues.
...
The following reference describes common problems that can prevent the OpenLineage integration from working correctly, along with steps to resolve them.
