...
1. When the record is processed by other pipeline jobs before it reaches the converter, implement a mechanism for recording the metadata in the message headers, for example, by using the `HeaderFrom` SMT.
...
By default, the CloudEvents converter automatically generates `id` and `type` fields for an event. 
You can customize the way that the converter populates these fields by specifying values for the fields in headers. 
For example:
