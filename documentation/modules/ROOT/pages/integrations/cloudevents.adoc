...
// Title: Emitting {prodname} change event records in CloudEvents format (Technology Preview)
...
The following list describes select fields in the preceding CloudEvents change event record:
...
+
Valid values for `_CONNECTOR_TYPE_` are `db2`, `mariadb,`
ifdef::community[]
`informix`,
endif::community[]
...
The following list describes selected fields in a CloudEvents change event record that uses Avro format:
...
The following list describes selected fields in the preceding CloudEvents converter configuration:

`value.converter.serializer.type`::
Specifying the `serializer.type` is optional, because `json` is the default.

The CloudEvents converter converts Kafka record values. 
In the same connector configuration, you can specify `key.converter` if you want to operate on record keys.
