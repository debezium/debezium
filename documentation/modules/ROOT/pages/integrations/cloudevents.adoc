...
|null
|Specifies CloudEvents schema name under which the schema is registered in a Schema Registry. The setting is meaningless when `serializer.type` is `json` in
which case the value is schemaless. The default value is `null` which means the default algorithm will be used to generate the
schema name: `${serverName}.${databaseName}.CloudEvents.Envelope`.
