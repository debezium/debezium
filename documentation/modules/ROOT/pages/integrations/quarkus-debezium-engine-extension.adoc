[id="debezium-quarkus-engine-extension"]
...
==== By custom filter strategy

For advanced filtering, you can provide a custom `CapturingFilterStrategy` that receives the full event and decides whether to capture it:
...
[NOTE]
====
The `filter` attribute is mutually exclusive with `destination` and `engine`, validated at build time.
====

==== By field changes

A built-in `CapturingFieldsFilterStrategy` can filter events based on the state of specific watched fields, where that state depends on the event type:

- *Create / Read*: captured if any watched field exists in the record schema
- *Update*: captured only if at least one watched field value changed between before and after
- *Delete*: captured if any watched field exists in the before schema
- *Truncate / Message*: not captured (no field-level data)

Extend it and pass the field names to watch:
...
The filter strategy also works with batch event processing (`CapturingEvents<BatchEvent>`):
...
For batch events, each record in the batch is individually evaluated by `shouldCapture` and only matching records are delivered to the filter handler.

==== Fallback chain

When multiple `@Capturing` methods are defined, events are dispatched through a fallback chain:

1. Methods with `filter` are checked first. The filter's `shouldCapture` decides.
2. Methods with a registered deserializer for the destination (single event only).
3. Methods with `destination` (exact match, then wildcard `*` fallback).

First match wins.
A `@Capturing()` with no destination acts as a wildcard and catches any event not matched by the previous levels.
If no wildcard is defined, unmatched events are skipped.

The same fallback chain applies to both single event and batch event processing.
