...
|debezium-offsets
|The name of the NATS Object Store bucket where {prodname} stores offsets. The entire offset map is stored as a single object.
...
|0
|Specifies the maximum age of messages in the stream, in milliseconds. Set to `0` for unlimited.

|[[schema-history-internal-nats-max-bytes]]<<schema-history-internal-nats-max-bytes, `schema.history.internal.nats.max.bytes`>>
|-1
|Specifies the maximum size of the stream, in bytes. Set to `-1` for unlimited.
...
|[[schema-history-internal-nats-recovery-attempts]]<<schema-history-internal-nats-recovery-attempts, `schema.history.internal.nats.recovery.attempts`>>
|100
|Specifies the number of consecutive unsuccessful attempts to retrieve schema history data before recovery stops.
