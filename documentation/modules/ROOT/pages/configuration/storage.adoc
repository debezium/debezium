...
|debezium-offsets
|The name of the NATS Object Store bucket where {prodname} stores offsets. Each offset is stored as an individual object, named by the encoded offset key.
...
|60000
|Specifies the maximum time, in milliseconds, that {prodname} spends recovering the schema history before recovery stops.
