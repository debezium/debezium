...
ifdef::community[]
. Install the Avro converter from link:https://repo1.maven.org/maven2/io/apicurio/apicurio-registry-distro-connect-converter/{apicurio-version}/apicurio-registry-distro-connect-converter-{apicurio-version}.tar.gz[the installation package] into a plug-in directory. This is not needed when using the link:https://quay.io/repository/debezium/connect[Debezium Connect container image], see details in <<deploying-with-debezium-containers>>.
endif::community[]
ifdef::product[]
...
To install standalone Confluent Schema Registry, please see Confluent's https://docs.confluent.io/platform/current/installation/installing_cp/overview.html[official deployment guide].

[NOTE]
====
There's also possibility to https://docs.confluent.io/platform/current/installation/docker/installation.html[install] it as a container.
====
...
As of {prodname} 2.0.0, Confluent Schema Registry support is on part of the {prodname} containers.
To make it working, you need to install into Connect plugin directory appropriate jar files which contain Confluent Avro converters.
Namely, you would need following jar file:
...
[NOTE]
====
You can download the required jar files https://packages.confluent.io/maven/[Confluent's Maven repository].
====
