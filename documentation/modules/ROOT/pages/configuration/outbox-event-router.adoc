...
[WARNING]
====
If the connector is configured to emit heartbeat events, an additional configuration option `value.converter.delegate.converter.type` will be required.
This option defines the fully qualified converter class name that will be delegated events that the `ByteBufferConverter` does not natively support.
====
