...
It is recommended to use either xref:filter-topic-regex[topic.regex] configuration option or a link:https://cwiki.apache.org/confluence/display/KAFKA/KIP-585%3A+Filter+and+Conditional+SMTs[an SMT predicate] to ensure that the expression is applied only to a valid messages.
====
