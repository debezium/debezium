...
----
The following list describes select fields in the preceding initial snapshot notification example:
...
----
The `total_rows`, `total_chunks`, and `chunk_index` fields let you report the progress of the scan for the current data collection:

`total_rows`:: A best-effort estimate of the total number of rows in the data collection that is currently being snapshotted.
{prodname} uses the database's own metadata statistics when they are available, and otherwise falls back to a bounded count.
Because the value is an estimate, it might not exactly match the number of rows that are ultimately scanned.
`total_chunks`:: The number of chunks that are required to complete the scan, derived from `total_rows` and the configured `incremental.snapshot.chunk.size`.
`chunk_index`:: The number of chunks that have been completed so far for the current data collection, from `1` up to `total_chunks`.
The reported progress never exceeds 100 percent.

These fields are present only when {prodname} can establish an estimate for the data collection; if it cannot, the fields are omitted.
...
----
The `total_rows` field reports the best-effort estimate of the total number of rows in the data collection.
It is present only on a successful scan completion (`status` is `SUCCEEDED`) and only when {prodname} can establish an estimate for the data collection.
Because the value is an estimate, it might differ slightly from `total_rows_scanned`, which reports the exact number of rows that were scanned.
...
1. xref:debezium-configuring-custom-notification-channels[Create a Java project for the channel] to implement the channel, and xref:debezium-connector-common-module-dependency[add `{prodname} Connector Core` as a dependency].
...
----
The following list describes the purpose of select lines in the preceding `NotificationChannel` interface example:
...
//   Type: concept
[id="debezium-connector-common-module-dependency"]
...
----
The following list describes select fields in the preceding `pom.xml` dependency example:

`${version.debezium}`::
Represents the version of the {prodname} connector.
