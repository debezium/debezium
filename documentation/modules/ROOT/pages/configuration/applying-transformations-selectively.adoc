...
By default, when you configure a single message transformation (SMT) for a Kafka Connect connector, the transformation is applied to every record that the connector processes.
You can define SMT predicates to modify the default behavior so that Kafka Connect applies transformations to some records, and not to others.
Predicates can be assigned to transformations for source connectors, such as {prodname}, as well as sink connectors.
