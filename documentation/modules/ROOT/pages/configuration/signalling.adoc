...
=== Provide Custom action
As for xref:debezium-custom-signaling-channel[custom signal channel], you can also provide your custom actions implementing the
following interface:
...
The `io.debezium.pipeline.signal.actions.SignalAction` exposes a single method where you will receive the payload sent through the signal channel.

After that you need to provide all your custom actions through the SPI interface `io.debezium.pipeline.signal.actions.SignalActionProvider`
...
You need to provide your implementation to just return a map of signal action where the key of the map is the name of the action that you will use as xref:debezium-signaling-description-of-required-structure-of-a-signaling-data-collection[`type`] in the signal.
