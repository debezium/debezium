...
The default file name is `file-signals.txt`.
If you want to use a different file, you can specify the file name by setting the `signal.file` property in the connector configuration.

=== Message format

File contains a JSON object with `id`, `type` and `data` fields.
