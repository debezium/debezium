...
Stopping of an ad hoc snapshot is available for the following {prodname} connectors:
...
The signal requires that the `type` be specified in the payload; however, the `data-collections` is optional.
By not specifying `data-collections`, the currently in-progress snapshot will be stopped.
By specifying `data-collections`, you can selectively stop or prevent snapshotting specific tables associated to the current in-progress incremental snapshot.
