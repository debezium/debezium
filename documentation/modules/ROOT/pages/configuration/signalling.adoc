...
The values that you assign to the `filter` property is specific to the type of ad hoc snapshot.

For incremental snapshots the value is a fragment that is appended to the condition clause of a query used to select records from {data-collection}.
For blocking snapshots the value is the same `SELECT` statement you would use for the connector `snapshot.select.statement.overrides` property for an initial snapshot.
|===
