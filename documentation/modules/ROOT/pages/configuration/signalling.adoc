...
Enabling the source channel is not required when using any of others available signaling channels to trigger an incremental snapshot of a read-only MySQL database with {link-prefix}:link-mysql-connector}#enable-mysql-gtids[GTIDs enabled]. 
