...
8. After the {prodname} connector starts streaming from the new capture table, drop the old capture table by running the stored procedure `sys.sp_cdc_disable_table` with the parameter `@capture_instance` set to the old capture instance name. The connector emits a notification when it has completed reading from the old capture instance, see xref:sqlserver-notifications[Notifications].
...
3. When {prodname} starts streaming from the new capture table, you can drop the old capture table by running the `sys.sp_cdc_disable_table` stored procedure with the parameter `@capture_instance` set to the old capture instance name. The connector emits a notification when it has completed reading from the old capture instance, see xref:sqlserver-notifications[Notifications].
...
// Type: assembly
// ModuleID: notifications-debezium-sql-server-connector-performance
// Title: Notifications for {prodname} SQL Server connector
[[sqlserver-notifications]]
== Notifications

Apart form the snapshot related notifications, {prodname} SQL Server connector provides notifications related to schema evolution.
For information about notifications and how to enable them, see the {link-prefix}:{link-notification}#debezium-notification[{prodname} notification documentation].
