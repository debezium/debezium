...
The {prodname} SQL Server connector performs an initial snapshot to capture the current state of the database before it begins streaming change events.
You can customize the snapshot workflow by changing the value of the `snapshot.mode` configuration property.
...
|`when_needed_no_data`
|After the connector starts, it performs a snapshot like `when_needed` options. Like `no_data` options, it does not create `READ` events to represent the data set at the point of the connector's start-up (Step 7.b).
...
By default, the initial snapshot captures schema information for every table in the database, not only tables that are designated for capture.
Understanding this behavior helps you control snapshot scope and avoid schema history errors when you add tables to the capture set later.
...
6. (Optional) If the connector performed a `no_data` snapshot, after the snapshot completes, initiate an xref:sqlserver-incremental-snapshots[incremental snapshot] to capture data from the tables that you added.
...
// Type: concept
[id="debezium-sqlserver-chunk-based-parallel-snapshots"]
...
// Type: concept
[id="sqlserver-collation-considerations"]
==== SQL Server collation considerations

Each SQL Server server or database is configured to use a specific collation, which determines how the database stores, sorts, compares, and displays character data.
The sorting rules for some SQL Server collation sets, such as (SQL_*), are not compatible with the Unicode sorting algorithm.
...
* https://learn.microsoft.com/en-us/sql/relational-databases/collations/collation-and-unicode-support?view=sql-server-ver16#Collation_Defn[Collations].
* https://learn.microsoft.com/en-us/sql/connect/jdbc/setting-the-connection-properties?view=sql-server-ver16[SQL Server connection properties documentation].
...
The {prodname} SQL Server connector cannot begin streaming if no maximum LSN is recorded in the database.
The most likely reason for a missing LSN is that the SQL Server Agent is not running.

The maximum LSN can be omitted due to any of the following reasons:

* The SQL Server Agent is not running.
* No changes are recorded in the change table yet.
* The database has low activity, and the CDC cleanup job has periodically cleared entries from the CDC tables.

Of these reasons, a stopped SQL Server Agent is the only true error condition, because a running SQL Server Agent is a prerequisite for the connector to function.
The other two conditions are normal.
To differentiate a stopped agent from normal conditions, the connector queries the SQL Server Agent status.
If the SQL Server Agent is not running, the connector writes an ERROR to the log: "No maximum LSN recorded in the database; SQL Server Agent is not running".

To help diagnose the issue, run the following query to check the status of the SQL Server Agent:
...
[IMPORTANT]
====
Running the preceding query to check the running status of the SQL Server Agent requires `VIEW SERVER STATE` permission.
`VIEW SERVER STATE` is an instance-wide permission, and granting it provides a user with visibility into every database hosted on the instance.
As an alternative to granting this permission, you can configure the `database.sqlserver.agent.status.query` property to run a custom query.

For example, `database.sqlserver.agent.status.query=SELECT [#db].func_is_sql_server_agent_running()`
`[#db]` represents a placeholder for the database name.

Define a function that returns `true` or `1` if the SQL Server Agent is running, or `false` or `0` otherwise.
By configuring a property to run the function, you can safely use high-level permissions without granting `VIEW SERVER STATE` permission to any user.
====

.Additional resources

* https://dba.stackexchange.com/questions/62230/what-minimum-permissions-do-i-need-to-provide-to-a-user-so-that-it-can-check-the/103275#103275[Minimum permissions required to check the status of the SQL Server Agent Service (Stack Exchange)^].
* https://sqlquantumleap.com/2018/02/15/safely-and-easily-use-high-level-permissions-without-granting-them-to-anyone-server-level/[Safely and easily using high-level permissions without granting them to anyone: server-level (Sql Quantum Leap)^].
...
The {prodname} SQL Server connector does not support change capture for all SQL Server database object types.
Indexed views, also known as materialized views, are not supported because SQL Server CDC requires the base object of a capture instance to be a table.
...
The {prodname} SQL Server connector uses an internal schema history topic to store a record of schema changes, enabling it to apply the correct schema to change events regardless of when they were captured.
The schema history topic is for internal connector use only and is not intended for consuming applications.

Because the database schema can change at any time, the connector must be able to identify the schema that was in effect when each change event was captured.
...
If you update the schema of a SQL Server table that is in capture mode, you must also update the schema of the corresponding change table.
...
By comparing the value for `payload.source.ts_ms` with the value for `payload.ts_ms`, you can determine the lag between the source database update and Debezium.
...
+
It is possible to override the table's primary key by setting the xref:sqlserver-property-message-key-columns[`message.key.columns` connector configuration property].
...
In this example, there is one required field named `id` of type `int32`.
...
Key schema names have the format _connector-name_._database-schema-name_._table-name_.`Key`.
In this example:
+
`server1`:::
...
`dbo`:::
The database schema for the table that was changed.
...
Every {prodname} SQL Server change event value contains a schema section and a payload section.
The payload uses an envelope structure that captures the state of a row before and after each `INSERT`, `UPDATE`, or `DELETE` operation.

The `schema` section describes the `Envelope` structure of the `payload` section, including its nested fields.
...
The connector emits a _create_ event whenever a row is inserted into a captured table.
The event payload contains the full state of the new row in the `after` field, and the `before` field is null.
...
+
`server1.dbo.testDB.customers.Value` is the schema for the payload's `before` and `after` fields.
...
`name` (`io.debezium.connector.sqlserver.Source`)::
`io.debezium.connector.sqlserver.Source` is the schema for the payload's `source` field.
This schema is specific to the SQL Server connector.
...
In this example, the `email` value is `john.doe@example.org`.

`after`::
An optional field that specifies the state of the row after the event occurred.
You can compare the `before` and `after` structures to determine what the update to this row was.
In the example, the `email` value is now `noreply@example.org`.

`source`::
Mandatory field that describes the source metadata for the event.
The `source` field structure has the same fields as in a _create_ event, but some values are different, for example, the sample _update_ event has a different offset.
The source metadata includes:
+
* {prodname} version
* Connector type and name
...
`ts_ms`, `ts_us`, `ts_ns`::
Optional fields that display the time at which the connector processed the event, in millisecond, microsecond, and nanosecond formats.
In the event message envelope, the time is based on the system clock in the JVM that runs the Kafka Connect task.
+
...
[WARNING]
====
When the {prodname} SQL Server connector captures changes from an Always On read-only replica, it does not reliably emit an `END` transaction marker immediately after a transaction completes.
In other configurations, the connector can query link:https://learn.microsoft.com/en-us/sql/relational-databases/system-dynamic-management-views/change-data-capture-sys-dm-cdc-log-scan-sessions[sys.dm_cdc_log_scan_sessions] to confirm that the most recent log scan session has concluded, but this dynamic management view (DMV) is not available for Always On read-only replicas.
When {prodname} captures changes from an Always On read-only replica, the connector emits a transaction `END` event only after it detects the arrival of the first event from a subsequent transaction in the change tables.
This behavior can cause {prodname} to delay delivery of the `END` markers in environments with low or infrequent traffic.
====
...
Values outside this range — for example a `9999-12-31` end-of-time sentinel stored in a `DATETIME2(7)` column — cannot be represented and cause the connector to throw an `IllegalArgumentException` from the value converter.
...
If a column may legitimately contain values outside the representable range, declare it as `DATETIME2(0 - 6)` so that Debezium emits `Timestamp` (millisecond) or `MicroTimestamp` (microsecond) instead, or set `time.precision.mode=isostring` to capture the value as an ISO 8601 string.
====
...
|`DATE`
|`STRUCT`
|`io.debezium.time.StructuredDate` +
 +
...
By default, JDBC connections to Microsoft SQL Server use SSL encryption.
Configure SSL encryption settings using JDBC pass-through properties to enforce certificate validation, or to disable encryption in environments where SQL Server does not use SSL.

If SSL is not enabled for a SQL Server database, or if you want to connect to the database without using SSL, you can disable SSL by setting the value of the `driver.encrypt` property in connector configuration to `false`.

.Procedure

* To enforce SSL with a valid certificate, set the following JDBC pass-through properties to configure the connection:
+
[source,json]
----
{
...
* In non-production environments, if SQL Server is configured to use a known self-signed certificate that you trust, you can enable encryption and bypass certificate path validation by setting the following properties:
+
[source,json]
----
{
...
----
+
WARNING: Do not enable automatic trust of server certificates in production environments.
If you disable validation, the connector skips certificate chain and host name validation, which can leave connections vulnerable to man‑in‑the‑middle attacks.
Use an explicit trust configuration only as a convenience in testing and development environments
...
. In the template, replace the database name in the `USE` statement with the name of the database that you want to capture.
. Replace the `source_schema`, the `source_name`, the `role_name` and the `filegroup_name` parameters with the table's schema name, the table name, the database user or role name used by {prodname}, and the filegroup name. You can use `CTRL+SHIFT+M` to open a parameters' replacement dialog box.
...
You can use the {prodname} SQL Server connector with SQL Server on Azure.
Enable CDC on the Azure SQL database and configure the connector to connect to the Azure SQL endpoint.
...
The SQL Server capture job agent processes change event records from the transaction log, and its polling frequency directly affects both database CPU load and the latency of change events available to {prodname}.
Tuning agent parameters can help you balance performance and responsiveness for your workload.
...
The SQL Server `msdb.dbo.cdc_jobs` table defines the parameters that control capture job agent behavior.
Use these parameters together with the `sys.sp_cdc_change_job` stored procedure to tune polling frequency and transaction batch sizes to balance database server load and event latency.

If you experience performance issues while running the capture job agent, adjust capture job settings to reduce CPU load by running the `sys.sp_cdc_change_job` stored procedure and supplying new values.
...
* https://docs.microsoft.com/en-us/sql/relational-databases/system-tables/dbo-cdc-jobs-transact-sql?view=latest[`msdb.dbo.cdc_jobs`]
* https://docs.microsoft.com/en-us/sql/relational-databases/system-stored-procedures/sys-sp-cdc-change-job-transact-sql?view=latest[`sys.sp_cdc_change_job`]
...
ifdef::product[]
To deploy a {prodname} SQL Server connector, use {StreamsName} to automatically create a Kafka Connect image that includes the connector plug-in.
...
Configure a {prodname} SQL Server connector by setting properties in a JSON file that specifies the database connection, topic prefix, table capture list, and schema history topic.
You can optionally filter, mask, or truncate column values to control what data the connector captures and emits.
...
The name of the database schema history topic where the connector will write and recover DDL statements.
...
This property is required unless database encryption is disabled (`driver.encrypt=false`).
...
The required configuration properties establish the database connection and define the basic identity and scope of the connector.
You must set these properties before the connector can start.
...
Optional when using Microsoft Entra authentication.
...
|`adaptive`
| Time, date, and timestamps can be represented with different kinds of precision, including: `adaptive` (the default) captures the time and timestamp values exactly as in the database using either millisecond, microsecond, or nanosecond precision values based on the database column's type; `connect` always represents time and timestamp values using Kafka Connect's built-in representations for Time, Date, and Timestamp, which uses millisecond precision regardless of the database columns' precision; `isostring` represents temporal values as ISO-8601 formatted strings at the UTC time zone; or `structured` represents temporal values as Kafka Connect structs with separate calendar and clock components.
...
`when_needed_no_data`:: After the connector starts, it performs a snapshot like `when_needed` options. Like `no_data` options, it does not create `READ` events to represent the data set at the point of the connector's start-up (Step 7.b).
...
|[[sqlserver-property-poll-dispatch-interval-ms]]<<sqlserver-property-poll-dispatch-interval-ms, `+poll.dispatch.interval.ms+`>>
|`0`
...
For more information, see xref:sqlserver-streaming-metrics[Streaming metrics].
...
+
For example, to specify a SELECT statement for the `mydb.customer.orders` table, add the following property:
+
...
The following example shows how to configure the `snapshot.select.statement.overrides` property to perform a snapshot of the `mydb.customers.orders` table that includes only records that are not soft-deleted; that is, the value of the soft-delete field, `delete_flag`, is set to `0`.
====
----
...
For multi-database deployments, if `tasks.max > 1`, you can configure one signal table per database by providing a comma-separated list of fully-qualified signal table names. For example:
...
[IMPORTANT]
====
Support for using multiple signaling tables with the {prodname} SQL Server connector is a Technology Preview feature.
...
|[[sqlserver-property-incremental-snapshot-pool-release-delay-ms]]<<sqlserver-property-incremental-snapshot-pool-release-delay-ms, `+incremental.snapshot.pool.release.delay.ms+`>>
|`60000` (1 minute)
...
|[[sqlserver-property-incremental-snapshot-retry-max-attempts]]<<sqlserver-property-incremental-snapshot-retry-max-attempts, `+incremental.snapshot.retry.max.attempts+`>>
|`5`
...
|[[sqlserver-property-incremental-snapshot-retry-initial-delay-ms]]<<sqlserver-property-incremental-snapshot-retry-initial-delay-ms, `+incremental.snapshot.retry.initial.delay.ms+`>>
|`1000`
...
|[[sqlserver-property-incremental-snapshot-retry-max-delay-ms]]<<sqlserver-property-incremental-snapshot-retry-max-delay-ms, `+incremental.snapshot.retry.max.delay.ms+`>>
|`60000` (1 minute)
...
|[[sqlserver-property-incremental-snapshot-retry-backoff-multiplier]]<<sqlserver-property-incremental-snapshot-retry-backoff-multiplier, `+incremental.snapshot.retry.backoff.multiplier+`>>
|`2.0`
...
|[[sqlserver-property-snapshot-max-threads-multiplier]]<<sqlserver-property-snapshot-max-threads-multiplier, `+snapshot.max.threads.multiplier+`>>
|`1`
...
|[[sqlserver-property-legacy-snapshot-max-threads]]<<sqlserver-property-legacy-snapshot-max-threads, `+legacy.snapshot.max.threads+`>>
|`false`
...
|[[sqlserver-property-custom-sanitize-pattern]]<<sqlserver-property-custom-sanitize-pattern, `custom.sanitize.pattern`>>
...
* `function`: The data is queried by calling `cdc.[fn_cdc_get_all_changes_#]` function.
* `direct`: Makes the connector to query change tables directly. This is the default mode.
...
|[[sqlserver-property-extended-headers-enabled]]<<sqlserver-property-extended-headers-enabled, `extended.headers.enabled`>>
|true
...
When you change the schema of a SQL Server table that has CDC enabled, you must update the corresponding capture table before the connector can continue to emit change events with the correct structure.
The procedures in this section describe how to refresh capture tables after a schema change.

For SQL Server tables that have change data capture enabled, event records are persisted to a capture table on the server as changes occur.
If you introduce a change in the structure of the source table, for example, by adding a new column, that change is not dynamically reflected in the change table.
...
The connector emits a notification after it finishes reading from the old capture instance.
For more information, see xref:sqlserver-notifications[Notifications].
...
3. After {prodname} starts streaming from the new capture table, you can drop the old capture table by running the `sys.sp_cdc_disable_table` stored procedure with the parameter `@capture_instance` set to the old capture instance name.
The connector emits a notification after it finishes reading from the old capture instance.
For more information, see xref:sqlserver-notifications[Notifications].
...
// Type: concept
// ModuleID: debezium-sql-server-connector-status-notification-after-a-schema-change
// Title: Status notifications that the {prodname} SQL Server connector emits after a schema change.
[[sqlserver-notifications]]
== Notifications

The {prodname} SQL Server connector can emit notification events to report on significant connector state changes, such as the completion of a schema evolution change.
You can configure notifications to monitor connector activity and trigger downstream workflows.

If you enable notifications for the connector, after you make a schema change, the connector emits a notification to report the status of the change.
For more information about notifications and how to enable them, see the {link-prefix}:{link-notification}#debezium-notification[{prodname} notifications documentation].

The following example shows a status notification that {prodname} might emit after a schema evolution change.
