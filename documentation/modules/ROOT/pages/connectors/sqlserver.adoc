...
[WARNING]
====
When capturing changes from an Always On read-only replica, there is no way for {prodname} to reliably identify if change tables contain all data for a given transaction.
The transaction `END` marker is thus emitted only after the first event of another transaction arrives under these circumstances.
This can lead to the delayed delivery of `END` markers in case of a low-traffic system.
====
