...
Debezium emits a new message to this topic whenever a new table is streamed from or when the structure of the table is altered (link:#sqlserver-schema-evolution[schema evolution procedure must be followed]).
...
All data change events produced by the SQL Server connector have a key and a value, although the structure of the key and value depend on the table from which the change events originated (see link:#sqlserver-topic-names[Topic names]).
