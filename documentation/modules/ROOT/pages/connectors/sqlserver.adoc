...
Conversely, to enforce SSL with a valid certificate, you configure the connection using JDBC pass-through properties:
...
If the SQL Server is using a self-signed certificate, you can set `driver.trustServerCertificate` to bypass certificate path validation:
