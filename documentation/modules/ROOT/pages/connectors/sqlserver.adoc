...
|[[sqlserver-property-query-all-tables]]<<sqlserver-property-query-all-tables, `+query.all.tables+`>>
|`false`
|Attempt to retrieve changes for all whitelisted tables rather than checking beforehand to see which tables actually have changes to be retrieved. This should be turned on only when there is confidence that a very high percentage of whitelisted tables will have changes in each iteration.
