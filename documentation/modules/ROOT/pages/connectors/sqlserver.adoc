...
Also field `event_serial_no` is present in case of streaming.
This is used to differentiate among events that have the same commit and change LSN.
There are mostly two sitations when you can see it present with value different from `1`
** update event wil have the value set to `2`, this is beacuse the update generates two events in the CDC change table (https://docs.microsoft.com/en-us/sql/relational-databases/system-tables/cdc-capture-instance-ct-transact-sql?view=sql-server-2017[source documentation]).
...
** when primary key is updated then SQL Server emits two records - `delete` to remove the record with old primary key value and `insert` to create the record with new primary key.
