...
Optionally, the Client ID of a Managed Identity can be set in the `database.user` property without database.password when using a managed identity mode.
If not set, the driver default behavior applies.
