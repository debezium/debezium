...
* `+start -->+` `_<COMMAND>_`
+
In this case, enter the specified `_<COMMAND>_` in the terminal window as shown in the foll example:
+
[source,shell]
----
...
The following added schema parameters propagate the original type name and also the original length for variable-width types: +
 +
