...
The Db2 connector has been tested with Db2/Linux 11.5.0.0. It is expected that the connector would also work on Windows, AIX and zOS.
...
Unlike the {prodname} SQL Server connector, it is possible for a Db2 connector to stream changes from only one Db2 database.
...
A database administrator must put tables into capture mode before you can run a {prodname} Db2 connector to capture changes that are committed to a Db2 database. To put tables into capture mode, {prodname} provides user-defined functions (UDFs). The procedure here shows how to install and run these UDFs. 

Alternatively, you can run `db2` control commands.  

.Prerequisites

* Database administration privileges. 
* You are logged in as the `db2instl` user. 
* On the machine on which Db2 is running, the content in link:https://github.com/debezium/debezium-connector-db2/src/test/docker/db2-cdc-docker[] is available in the `$HOME/asncdctools/src` directory. 

.Procedure

. Compile the UDFs on the Db2 server host, by using the `bldrtn` 
C-compiler:
+
[source,shell]
----
...
. Start the database if it is not already running. Replace `DB_NAME` with the name of your database. 
+
[source,shell]
----
...
. Ensure that the metadata catalog can be read by means of JDBC:
+
[source,shell]
----
cd $HOME/sqllib/bnd
...
. Ensure that the database was recently backed-up. The ASN agents must have a recent starting point to read from. If you need to perform a backup, run the following commands. This prunes the data such that only the most recent version is available. 
...
----
+
Iif you do not need to retain the older versions of the data, specify `dev/null` for the back-up location. 
...
----
+
A prerequisite was to log in as the `db2instl` user so the `db2` tool should be installed on the `db2inst1` user.

. Copy the tool and set permissions: 
+
[source,shell]
----
...
. Enable the UDF for starting and stopping the ASN capture agent:
+
[source,shell]
----
...
. Enable the UDF for adding tables to capture mode and removing tables from capture mode: 
+
[source,shell]
----
...
----
+
If the ASN agent ever needs to be stopped, use this statement: 
+
[source,sql]
----
...
----
+
To check the status of the ASN agent:
+
[source,sql]
----
...
. Put tables into capture mode. For each table, use the following statement. Replace `MYSCHEMA`  with the name of the schema that contains the table you want to put into capture mode. Likewise, replace `MYTABLE` with the name of the table to put into capture mode: 
+
[source,sql]
----
...
----
+
To remove a table from capture mode, use the following statement: 
+
[source,sql]
----
...
[id="db2-reinitialize-asn-service"]
. After you change the tables that are in capture mode by adding or removing one or more tables, reinitialize the ASN service:
+
[source,sql]
----
...
. {link-prefix}:{link-db2-connector}#db2-reinitialize-asn-service[Reinitialize the ASN capture service.
. Remove the source table with the old schema from ASN.
. Add the source table with the new schema to ASN.
