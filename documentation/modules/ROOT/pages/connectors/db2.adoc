...
|`initial`
|Specifies what the connector should read when it takes snapshot. +
 +
`initial` - For tables that are in capture mode, the connector takes a snapshot of the schema for each table as well as the data in each table. This is useful for populating Kafka topics with a complete representation of the data for tables in capture mode. +
 +
 `schema_only` - For tables in capture mode, the connector takes a snapshot of only the schema for each table. This is useful when only the changes that are happening from now onwards need to be emitted to Kafka topics. After the snapshot is complete, the connector continues reading change events from the database's redo logs.
