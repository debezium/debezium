...
Consult withÂ your legal department to verify whether the licensing in your environment complies with the terms of use.
====
...
However, for Kafka to remove all messages that have that same key, the message value must be `null`. To make this possible, after {prodname}â€™s Db2 connector emits a _delete_ event, the connector emits a special tombstone event that has the same key but a `null` value.
