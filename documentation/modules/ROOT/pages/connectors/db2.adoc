...
`fail` will propagate the exception (indicating the problematic event and its offset), causing the connector to stop. +
`warn` will cause the problematic event to be skipped and the problematic event and its offset to be logged. +
