...
Db2 connector stores the historic schema structure of database tables in a database history topic.
...
Debezium emits a new message to this topic whenever a new table is streamed from or when the structure of the table is altered (link:#db2-schema-evolution[schema evolution procedure must be followed]).
...
* `ddl` contains the DDL responsible for the schema change (not available for Db2 connector)
...
All data change events produced by the Db2 connector have a key and a value, although the structure of the key and value depend on the table from which the change events originated (see link:#db2-topic-names[Topic names]).
...
[NOTE]
====
When the columns for a row's primary/unique key are updated, the value of the row's key has changed so Debezium will output _three_ events: a `DELETE` event and a link:#db2-tombstone-events[tombstone event] with the old key for the row, followed by an `INSERT` event with the new key for the row.
====
...
. create a link:#db2-example-configuration[configuration file for the Db2 Connector] and use the {link-kafka-docs}/#connect_rest[Kafka Connect REST API] to add that connector to your Kafka Connect cluster.
...
See the link:#db2-connector-properties[complete list of connector properties] that can be specified in these configurations.
...
|`10000`
|An integer value that specifies the maximum amount of time (in milliseconds) to wait to obtain table locks when performing a snapshot. If table locks cannot be acquired in this time interval, the snapshot will fail (also see link:#db2-snapshots[snapshots]). +
...
See link:#db2-transaction-metadata[Transaction Metadata] for additional details.
|=======================
