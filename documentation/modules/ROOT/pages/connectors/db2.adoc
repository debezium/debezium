...
This connector was added in Debezium 1.0.x.
...
Note that by default all tables in the database are snapshoted, note just those that are in capture mode.
...
The database administrator normally enables _CDC_ in the mid-life of a table.
...
these are only for convenience and manually performing the same commands using
db2 control commands would have the same effect. 
...
For example, consider a Db2 installation with an `inventory` database that contains four tables: `products`, `products_on_hand`, `customers`, and `orders` in schema `myschema`. If the connector monitoring this database were given a logical server name of `fulfillment`, then the connector would produce events on these four Kafka topics:

* `fulfillment.myschema.products`
* `fulfillment.myschema.products_on_hand`
* `fulfillment.myschema.customers`
* `fulfillment.myschema.orders`
...
5.  Mark the tables as inactive on the ASN register table
6.  Reinit the ASN capture service (see the UDFs)
7. Remove the old structure table from ASN
8. Add the new structure table to ASN
9.  Reinit the ASN capture service (see the UDFs)
10. Resume the application
11. Start Debezium connector
...
This is used to define the timezone of the transaction timestamp (ts_ms) retrieved from the server (which is actually not zoned). Default value is unset. Should only be specified when running on Db2 2014 or older and using different timezones for the database server and the JVM running the Debezium connector. +
When unset, default behavior is to use the timezone of the VM running the Debezium connector. In this case, when running on on Db2 2014 or older and using different timezones on server and the connector, incorrect ts_ms values may be produced. +
