...
To make this possible, by default, the {prodname} YashanDB connector always follows a _delete_ event with a special _tombstone_ event that has the same key but `null` value.
...
|`false`
|Specifies whether large object (CLOB or BLOB, etc.) column values are emitted in change events.
...
|The name of the class used to determine topic names for data changes, schema changes, transactions, heartbeat events, etc.
