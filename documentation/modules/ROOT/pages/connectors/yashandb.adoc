...
{prodname}'s YashanDB connector captures and records row-level changes that occur in databases on a YashanDB server, including tables that are added while the connector is running.
...
For more information about YashanDB and YStream, see the https://www.yashandb.com/[YashanDB official website].

ifdef::product[]

Information and procedures for using a {prodname} YashanDB connector are organized as follows:

  * xref:how-debezium-yashandb-connectors-work[]
  * xref:descriptions-of-debezium-yashandb-connector-data-change-events[]
  * xref:how-debezium-yashandb-connectors-map-data-types[]
  * xref:setting-up-yashandb-to-work-with-debezium[]
  * xref:deployment-of-debezium-yashandb-connectors[]
  * xref:descriptions-of-debezium-yashandb-connector-configuration-properties[]
  * xref:monitoring-debezium-yashandb-connector-performance[]
  * xref:debezium-yashandb-connector-frequently-asked-questions[]
endif::product[]

// Type: assembly
// ModuleID: how-debezium-yashandb-connectors-work
// Title: How {prodname} YashanDB connectors work
[[how-the-yashandb-connector-works]]
== How the YashanDB connector works
...
The YashanDB connector uses YashanDB's native YStream feature to capture changes from the database transaction logs.
YStream is YashanDB's Change Data Capture (CDC) mechanism.
...
// Type: concept
// ModuleID: default-names-of-kafka-topics-that-receive-debezium-yashandb-change-event-records
// Title: Default names of Kafka topics that receive {prodname} YashanDB change event records
...
// Type: concept
// ModuleID: how-debezium-yashandb-connectors-handle-database-schema-changes
// Title: How {prodname} YashanDB connectors handle database schema changes
...
// Type: concept
// ModuleID: how-debezium-yashandb-connectors-expose-database-schema-changes
// Title: How {prodname} YashanDB connectors expose database schema changes
...
In messages that the connector sends to the schema change topic
...
// Type: assembly
// ModuleID: descriptions-of-debezium-yashandb-connector-data-change-events
// Title: Descriptions of {prodname} YashanDB connector data change events
[[yashandb-events]]
== Data change events
...
* xref:about-keys-in-debezium-yashandb-connector-change-events[]
* xref:about-values-in-debezium-yashandb-connector-change-events[]
endif::product[]

// Type: concept
// ModuleID: about-keys-in-debezium-yashandb-connector-change-events
// Title: About keys in {prodname} YashanDB connector change events
...
For example, a `customers` table that is defined in the `inventory` database schema:
...
// Type: concept
// ModuleID: about-values-in-debezium-yashandb-connector-change-events
// Title: About values in {prodname} YashanDB connector change events
...
`commit_scn`:: The SCN at which the transaction was committed.
...
* xref:yashandb-create-events[_create_ events]
* xref:yashandb-update-events[_update_ events]
* xref:yashandb-delete-events[_delete_ events]
endif::product[]
...
`commit_scn`::: System change number at which the transaction was committed.
`lcr_position`::: Position in the logical change record.
`position_scn`::: SCN position in the log.
`group_lsn`::: Group log sequence number.
`group_offset`::: Group offset.
`instance_id`::: Instance identifier.
`rs_id`::: Redo sequence identifier.
...
* The structure of the `source` field includes many of the keys that exist in _create_ or _update_ events, but the values in the `ts_ms`, `scn`, and `commit_scn` fields are different.
...
To make this possible, by default, {prodname}'s YashanDB connector always follows a _delete_ event with a special _tombstone_ event that has the same key but `null` value.
...
If you do not want the connector to capture _truncate_ events, use the xref:yashandb-property-skipped-operations[`skipped.operations`] option to filter them out.

// Type: assembly
// ModuleID: how-debezium-yashandb-connectors-map-data-types
// Title: How {prodname} YashanDB connectors map data types
[[yashandb-data-types]]
== Data type mapping
...
Describe how values are represented literally, using one of the following Kafka Connect schema types: `INT8`, `INT16`, `INT32`, `INT64`, `FLOAT32`, `FLOAT64`, `BOOLEAN`, `STRING`, `BYTES`, `ARRAY`, `MAP`, `STRUCT`.
...
// Type: assembly
// ModuleID: setting-up-yashandb-to-work-with-debezium
// Title: Setting up YashanDB to work with {prodname}
[[setting-up-yashandb]]
== Setting up YashanDB

Before deploying and running the {prodname} YashanDB connector, you need to perform some configuration on the YashanDB database.
...
[CAUTION]
====
Failure to configure this parameter may cause the task to fail.
This parameter is a global parameter.
Please refer to the YashanDB official documentation for further assistance.
====
...
When you need to monitor all objects in the database (including new objects), you can enable database-level supplemental logging:
...
When you only need to monitor certain tables, you can enable table-level supplemental logging:
...
[CAUTION]
====
It is important to note that failing to enable supplemental logging, or enabling it incorrectly, can result in data loss or even task failure.
====
...
[NOTE]
====
The table names and schemas added here must be the same as those that the connector is configured to capture.
====
...
The corresponding service must be in a state that allows the current operation to be executed (the service status can be obtained by querying the `V$YSTREAM_SERVER` view).
...
// Type: assembly
// ModuleID: deployment-of-debezium-yashandb-connectors
// Title: Deployment of {prodname} YashanDB connectors
...
// Type: reference
// ModuleID: descriptions-of-debezium-yashandb-connector-configuration-properties
// Title: Descriptions of {prodname} YashanDB connector configuration properties
[[yashandb-properties]]
...
|No default
|The name of the Java class for the connector. Always use a value of `io.debezium.connector.yashandb.YashanDBConnector` for the YashanDB connector.
...
|No default
|An optional comma-separated list of regular expressions that match the names of schemas for which you want to capture changes. Any schema names not included in `schema.include.list` are excluded from having their changes captured. By default, changes for all non-system schemas are captured. To match schema names, Debezium applies the regular expressions you specify as anchored regular expressions.

|[[yashandb-property-schema-exclude-list]]<<yashandb-property-schema-exclude-list, `+schema.exclude.list+`>>
|No default
|An optional comma-separated list of regular expressions that match the names of schemas for which you do not want to capture changes. Any schema names not included in `schema.exclude.list` have their changes captured, except for system schemas.
...
Each table identifier uses the following format: `<schema_name>.<table_name>`. By default, the connector monitors every non-system table in each captured database schema.
...
|`1`
|Specifies the number of threads the connector uses when performing the initial snapshot. To enable parallel initial snapshots, set the property to a value greater than 1. In a parallel initial snapshot, the connector processes multiple tables simultaneously. This feature is in incubation.

|[[yashandb-property-snapshot-locking-mode]]<<yashandb-property-snapshot-locking-mode, `+snapshot.locking.mode+`>>
|No default
|Locking mode to prevent DDL changes before synchronizing snapshot data.
...
* `precise` (default): Use `java.math.BigDecimal` to represent values precisely. Represented in binary form in change events.
...
|The name of the class used to determine topic names for data changes, schema changes, transactions, heartbeat events, etc. Defaults to `SchemaTopicNamingStrategy`.
...
|`false`
|After incremental DDL parsing fails, fully read the source table structure for analysis when processing DML events. This takes effect when both `schema.history.internal.skip.unparseable.ddl` and `ddl.parse.fail.retry.read.table` are set to `true`.
...
|`false`
|Controls whether the connector records the DDL statements for all tables in the database, or only for tables that are captured. Set to `true` to store DDL only for captured tables.

|===

[[yashandb-properties-database-history]]
...
The following table describes the `schema.history.internal` properties for configuring the {prodname} YashanDB connector.

.Connector database schema history configuration properties
[cols="33%a,17%a,50%a",options="header"]
|===
|Property
|Default
|Description

|[[yashandb-property-database-history-kafka-topic]]<<yashandb-property-database-history-kafka-topic, `+schema.history.internal.kafka.topic+`>>
|No default
|The full name of the Kafka topic where the connector stores the database schema history.

|[[yashandb-property-database-history-kafka-bootstrap-servers]]<<yashandb-property-database-history-kafka-bootstrap-servers, `+schema.history.internal.kafka.bootstrap.servers+`>>
|No default
...
|[[yashandb-property-database-history-kafka-recovery-poll-interval-ms]]<<yashandb-property-database-history-kafka-recovery-poll-interval-ms, `+schema.history.internal.kafka.recovery.poll.interval.ms+`>>
|`100`
...
|[[yashandb-property-database-history-kafka-query-timeout-ms]]<<yashandb-property-database-history-kafka-query-timeout-ms, `+schema.history.internal.kafka.query.timeout.ms+`>>
|`3000`
...
|[[yashandb-property-database-history-kafka-create-timeout-ms]]<<yashandb-property-database-history-kafka-create-timeout-ms, `+schema.history.internal.kafka.create.timeout.ms+`>>
|`30000`
|An integer value that specifies the maximum number of milliseconds the connector should wait while creating the Kafka history topic using Kafka admin client.

|[[yashandb-property-database-history-kafka-recovery-attempts]]<<yashandb-property-database-history-kafka-recovery-attempts, `+schema.history.internal.kafka.recovery.attempts+`>>
|`100`
...
|[[yashandb-property-database-history-skip-unparseable-ddl]]<<yashandb-property-database-history-skip-unparseable-ddl, `+schema.history.internal.skip.unparseable.ddl+`>>
|`false`
|A Boolean value that specifies whether the connector should ignore malformed or unknown database statements or stop processing so a human can fix the issue. The safe default is `false`. Skipping should be used only with care as it can lead to data loss or mangling when the log is being processed.

|[[yashandb-property-database-history-store-only-captured-tables-ddl]]<<yashandb-property-database-history-store-only-captured-tables-ddl, `+schema.history.internal.store.only.captured.tables.ddl+`>>
|`false`
...
`false` (default):: During a database snapshot, the connector records the schema data for all non-system tables in the database, including tables that are not designated for capture. It's best to retain the default setting.
...
|[[yashandb-property-database-history-store-only-captured-databases-ddl]]<<yashandb-property-database-history-store-only-captured-databases-ddl, `+schema.history.internal.store.only.captured.databases.ddl+`>>
|`false`
...
[[yashandb-connector-pass-through-properties]]
...
[[yashandb-pass-through-database-history-properties-for-configuring-producer-and-consumer-clients]]
...
[[yashandb-connector-pass-through-kafka-signals-configuration-properties]]
...
[[yashandb-connector-pass-through-signals-kafka-consumer-client-configuration-properties]]
...
[[yashandb-connector-pass-through-kafka-sink-notification-configuration-properties]]
...
[[yashandb-connector-pass-through-database-driver-configuration-properties]]
...
// Type: reference
// ModuleID: monitoring-debezium-yashandb-connector-performance
// Title: Monitoring {prodname} YashanDB connector performance
...
// Type: concept
// ModuleID: monitoring-debezium-yashandb-connectors-customized-mbean-names
// Title: Customized names for YashanDB connector snapshot and streaming MBean objects
...
// Type: reference
// ModuleID: debezium-yashandb-connector-snapshot-metrics
// Title: {prodname} YashanDB connector snapshot metrics
...
// Type: reference
// ModuleID: debezium-yashandb-connector-streaming-metrics
// Title: {prodname} YashanDB connector streaming metrics
...
// Type: reference
// ModuleID: debezium-yashandb-connector-schema-history-metrics
// Title: {prodname} YashanDB connector schema history metrics
...
// Type: assembly
// ModuleID: debezium-yashandb-connector-frequently-asked-questions
// Title: {prodname} YashanDB connector frequently asked questions
[[yashandb-faq]]
...
+
The YStream server corresponding to the `database.ystream.server.name` parameter does not exist in the YashanDB database. Create the relevant YStream server as described in xref:yashandb-ystream-service[Configure the YStream service].
...
+
Debezium performs special handling for Decimals with negative scale. It is recommended to use the parameter `decimal.handling.mode=string` to work around this issue.
...
+
Debezium's default behavior is to map temporal types to `INT64`. If you need to map them to a fixed format string, please refer to the <<yashandb-custom-converters,Data type conversion>> section to configure custom converters.
...
+
Debezium captures the structure of all tables in the database by default. You can set `schema.history.internal.store.only.captured.tables.ddl=true` in the task configuration to capture only the structure of configured tables.

[[yashandb-faq-schema-not-captured-after-recreate]]
*After deleting and recreating a task, the table metadata structure is not captured upon restart (e.g., "Capturing structure of table" does not appear in the logs). What causes this?*::
+
...
+
This type of DDL cannot be parsed to obtain the table's metadata, causing subsequent DML data parsing to fail. After research, the Debezium Oracle Connector has the same issue. If this occurs, please restart a new synchronization task.
...
+
DDL statements created through stored procedure packages are not identifiable as specific DDL statements by the connector. This may cause metadata mismatches during data synchronization and result in synchronization errors. To avoid this issue, execute DDL statements directly rather than through stored procedure packages.
