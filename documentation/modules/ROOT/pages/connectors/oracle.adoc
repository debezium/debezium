...
|[[oracle-property-log-mining-buffer-infinispan-cache-transactions]]<<oracle-property-log-mining-buffer-infinispan-cache-transactions, `+log.mining.buffer.infinispan.cache.global+`>>
|No default
...
Once a change has been ingested by the OpenLogReplicator adapter, this {prodname} connector transforms the events into xref:#oracle-events[Data change events] just like any other adapter.

From a network topology perspective, the {prodname} Oracle connector relies on a network connection to both Oracle and OpenLogReplicator.
Similarly, OpenLogReplicator requires a network connection to the Oracle database along with direct access to the raw redo and archive logs.
...
Archive logs are only generated when the database is placed into `ARCHIVELOG` mode and this mode requires that specific configuration properties by set to identify where these archive log files will be stored.

The following illustrates the steps to enable archive log mode.
...
In addition, supplemental logging is required for Oracle to write the necessarily pieces of information about what has changed to the redo logs so that {prodname} can generate change events with before and after state.
Supplemental logging can be configured a number of different ways, but {prodname} for Oracle only requires minimal logging to work.

The database must be configured with minimal supplemental logging.
Minimal supplemental logging writes the least amount of information needed to be able to create change events.
The following illustrates how to enable minimal supplemental logging.

.Configure database supplemental logging
[source,sql,indent=0]
----
ALTER DATABASE ADD SUPPLEMENTAL LOG DATA
----
...
[NOTE]
====
{prodname} for Oracle can work with a higher degree of supplemental logging if that is already configured for other purposes.
If you are replacing a prior solution that required a higher fidelity of logging with {prodname}, you can chose to reduce the logging levels accordingly as minimum is all that is required at the database level.
====

Finally, each captured table must be explicitly configured with a higher fidelity of supplemental logging called `(ALL) COLUMNS`.
This logging level guarantees that Oracle captures the state of a column regardless of whether the column changed when a redo entry is written to the redo log.
This allows {prodname} for Oracle to generate change events safely with both _before_ and _after_ state.

The following illustrates how to enable supplemental logging for each captured table.
...
[IMPORTANT]
====
As new tables are added to the {prodname} Oracle's connector configuration, be sure to configure the table's supplemental logging.
If a table is configured but does not have the proper supplemental logging, the connector will report a warning when streaming starts.
====
...
The following briefly describes the user configurations using a multi-tenant database model.
...
You can adjust the connector configuration to enable the connector to use OpenLogReplicator.

The following configuration example adds the property `database.connection.adapter`, `openlogreplicator.source`, `openlogreplicator.host`, and `openlogreplicator.port` to enable the connector to use the OpenLogReplicator adapter.
...
It is written in C++; however, the author provides a container image to build the source for a variety of operating systems such as Debian, Centos, Fedora, etc.
This https://github.com/bersler/OpenLogReplicator-docker[repository] provides details on how to build the tool using a container.

[id="obtaining-oracle-jdbc-driver-openlogreplicator"]
=== Obtaining the Oracle JDBC driver for OpenLogReplicator

The {prodname} Oracle connector requires the Oracle JDBC driver to connect to Oracle databases.
Please refer to the link:#obtaining-the-oracle-jdbc-driver[Obtaining the Oracle JDBC driver] as OpenLogReplicator has the exact same requirements as Oracle LogMiner.
...
OpenLogReplicator is configured by using a JSON-based configuration in a file called `scripts/OpenLogReplicator.json`.
The format of this JSON configuration can be found https://github.com/bersler/OpenLogReplicator/blob/master/documentation/reference-manual/reference-manual.adoc#openlogreplicator-json-file-format[here].
...
<.> List of file path pairs `[before1,after1,befor2,after2,...]` where if a log file path matches one of the `beforeX` prefixes, the prefix is replaced with the `afterX` path.  This is useful when OpenLogReplicator is running on a different host than the source database and the path to the redo and archive logs different between the database and the OpenLogReplicator process.
...
<.> This specifies the payload format ingested by {prodname} Oracle connector. Use these values as specified, as these are the only format options that we require beyond the defaults.
<.> This must specify `network`, as {prodname} Oracle connector communicates with OpenLogReplicator via a network connection.
<.> This specifies the bind host and port that OpenLogReplicator listens for connections on.  This should be accessible by {prodname} Oracle connector.

[[oracle-openlogreplicator-connector-properties]]
=== OpenLogReplicator connector properties

The following configuration properties are _required_ when using OpenLogReplicator unless a default value is available.
...
|No default
|The host or IP of the OpenLogReplicator network service.
