...
The {prodname} Oracle connector supports capturing changes from an Oracle database mining database.
This capability is useful in scenarios where you want to offload change data capture processing from the primary database to a downstream instance, reducing the load on your production system.
...
Before configuring the connector to capture changes from a downstream mining databases, ensure the following requirements are met:

1. An Oracle downstream mining database is configured and synchronized with the primary instance.
2. The primary instance generates an external dictionary file that is accessible to the downstream mining database.
3. Configure the connector using the following key properties.
`log.mining.strategy`:: This must be set to `dictionary_from_file` for this mode to be used.
`internal.log.mining.read.only`:: Set this property to `true` to enable capturing changes from a downstream mining database.
When this property is not set to `true`, the {prodname} connector assumes changes are being captured from the primary instance.
`log.mining.read.only.hostname`:: Specifies the hostname of the downstream mining database that the {prodname} Oracle connector connects to for capturing and reading transaction logs.
Given the connector uses this downstream mining instance to capture changes, changes must be replicated from the primary to the downstream instance before change events can be emitted.
`log.mining.path.dictionary`:: Specifies the absolute file path on the downstream mining database server where Oracle LogMiner can access the data dictionary export file generated from the primary Oracle instance.
This file is essential for LogMiner to interpret schema changes and other database metadata.
The specified path must be accessible from the Oracle downstream mining LogMiner process to work.
...
1. The dictionary file must be regularly updated on the primary instance to ensure schema changes are properly reflected.
2. Network connectivity and file access permissions must be properly configured between the primary and downstream mining databases.
3. The `dictionary_from_file` strategy is mandatory when using downstream mining mode; other mining strategies are not supported.
4. Monitor the dictionary file synchronization to prevent issues with schema change detection.
endif::community[]
...
|`null`
|Specifies read-only hostname for log mining strategy `dictionary_from_file`.
endif::community[]
...
ifdef::community[]
`dictionary_from_file`:: Uses dictionary from file located in a Oracle directory object.
