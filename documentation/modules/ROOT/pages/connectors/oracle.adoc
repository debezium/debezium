...
Each destination can be set with various parameters for different activities, including log shipping for physical standbys or external storage allowing for longer retention of the logs.
All log destination details are found in the `V$ARCHIVED_DEST_STATUS` table in the Oracle database.

The {prodname} Oracle connector only uses destinations that have a status of `VALID` and a type of `LOCAL`.
If multiple destinations are configured in your Oracle environment that satisfy that criteria, then please consult your Oracle administrator to determine which archive log destination should be used.
...
As an example, if both the `LOG_ARCHIVE_DEST_2` and `LOG_ARCHIVE_DEST_3` destinations satisfy the criteria and the administrator has confirmed that destination three should be used, you would provide this configuration using the `log.mining.archive.destination.name` property as follows:
...
[WARNING]
====
If multiple valid paths are configured as `LOCAL` and the destination name is not provided in the connector configuration, the {prodname} Oracle connector will use a random path.
This can lead to missing logfile problems sporadically, especially if the retention policy for one path that is picked is less than the others.
====
