...
Oracle stores several data types by using internal large object (LOB) mechanisms in the redo logs.
To capture changes for any of these types, you must set the xref:oracle-property-lob-enabled[`lob.enabled`] connector configuration property to `true`.

The following data types require `lob.enabled` to be set to `true`:
...
* Extended strings - `VARCHAR2`, `NVARCHAR2`, and `RAW` columns that exceed the legacy maximum lengths when the database parameter `max_string_size` is set to `EXTENDED` (see xref:support-for-extended-max-string-size[Extended max string size])

When `lob.enabled` is `false` (the default), snapshot events include the current LOB column values, but streamed change events represent LOB columns with the xref:oracle-property-unavailable-value-placeholder[`unavailable.value.placeholder`] constant.
No LOB-specific redo log entries are mined, which reduces the volume of data that LogMiner processes.
...
Oracle only supplies LOB column values when the column value is explicitly set or changed in a SQL statement.
If a LOB column is not modified by a given DML statement, the change event contains the unavailable value placeholder rather than the column's current value.

==== INTERNAL events and ROLLBACK TO SAVEPOINT handling

Oracle uses `INTERNAL` events (operation code 0) in the redo logs to associate `ROW_ID` values with out-of-line LOB column operations.
...
Without `ROW_ID` information from `INTERNAL` events, the connector cannot always determine which specific row a rolled-back LOB operation belongs to.
In these scenarios, the connector may emit change events with stale LOB field values.

To address this, the connector provides the xref:oracle-property-log-mining-include-internal-events[`log.mining.include.internal.events`] configuration property.

* When set to `false` (the default), `INTERNAL` events are excluded from LogMiner queries.
This keeps the volume of mined redo log data lower, but partial rollbacks in transactions that modify out-of-line LOB columns may produce change events with stale values for the affected LOB fields.
* When set to `true`, the connector includes `INTERNAL` events in LogMiner queries.
This enables the connector to accurately handle `ROLLBACK TO SAVEPOINT` for tables with out-of-line LOB columns, at the cost of increased redo log query volume.
...
As an alternative, you can set `lob.enabled` to `false` and use the xref:post-processors/reselect-columns.adoc[Reselect columns post processor] to re-query LOB column values directly from the database.
This approach avoids the overhead of mining both LOB-specific redo log entries and `INTERNAL` events entirely, while still ensuring that LOB fields contain correct values, since the post processor fetches the current column value from the source table.
...
|`false`
|Specifies whether the connector supports mining `INTERNAL` events.
When set to `false`, the default, `INTERNAL` events are not captured.
When `lob.enabled` is `true` and all LOB columns of an operation are stored out-of-line, `INTERNAL` events provide the `ROW_ID` values necessary for accurate `ROLLBACK TO SAVEPOINT` handling.
Enabling this option might significantly increase the volume of events returned by LogMiner.
...
<7> The sum of the OpenLogReplicator flags that control what the stream includes. See <<oracle-openlogreplicator-flags>>.
<8> This must specify `network`, as {prodname} Oracle connector communicates with OpenLogReplicator via a network connection.
<9> This specifies the bind host and port that OpenLogReplicator listens for connections on.  This should be accessible by {prodname} Oracle connector.
====
