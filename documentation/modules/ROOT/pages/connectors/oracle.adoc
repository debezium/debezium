...
To use the {prodname} Oracle connector with an Autonomous Database, configure the following property:

[source,json]
----
{
    "database.autonomous.mode": "true"
}
----

When `database.autonomous.mode` is set to `true`, the connector automatically:
...
* Operates in archive-log-only mode, excluding online redo logs from mining.

NOTE: The `log.mining.archive.log.only.mode` property is automatically set to `true` when `database.autonomous.mode` is enabled.
...
----
{
    ...
    "config": {
        ...
        "database.autonomous.mode": "true",  // <5>
        "log.mining.archive.log.only.mode": "true",  // <6>
        "driver.oracle.net.wallet_location": "(SOURCE=(METHOD=file)(METHOD_DATA=(DIRECTORY=_<wallet_location>_)))",  // <7>
        "schema.history.internal.kafka.bootstrap.servers" : "kafka:9092",
        "schema.history.internal.kafka.topic": "schema-changes.inventory"
    }
}
...
<5> Enables Autonomous Database mode, which adapts the connector's behavior to work within ADB restrictions.
<6> Explicitly enables archive-log-only mode. This property is automatically set when `database.autonomous.mode` is `true`, but including it makes the configuration more explicit.
<7> The Oracle Wallet location configuration. Download the wallet from the Oracle Cloud Console and specify the directory path where the wallet files are stored. For more information about Oracle Wallet configuration, see xref:using-mtls-secure-connections-oracle-wallet[].
...
ifdef::community[]
|[[oracle-property-database-autonomous-mode]]<<oracle-property-database-autonomous-mode, `+database.autonomous.mode+`>>
|`false`
|Specifies whether the connector is capturing changes from an Oracle Autonomous Database (ADB). +
When set to `true`, the connector adapts its behavior to work within ADB's restrictions:
...
For more information, see xref:using-debezium-oracle-connector-with-autonomous-database[Using the connector with Oracle Autonomous Database].
endif::community[]
