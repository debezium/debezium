...
When the {prodname} Oracle connector restarts, you may find that the connector reports the following error:
...
This error occurs when the connector examines the available redo and archive logs and was unable to locate a record that contains the SCN in the connector offsets.
Since the connector automatically starts where it left off and if that SCN (position) is no longer available, then a re-snapshot will be necessary.

It's worth noting that you may find a record in the `V$ARCHIVED_LOG` table that has a SCN range that matches the referenced SCN; however, that alone doesn't mean the record is available to be mined.
The record must also have a filename in the `NAME` column, a value of `NO` in the `DELETED` column, and a value of `A` (available) in the `STATUS` column.
If any of these criteria do not match, then the record is considered incomplete and cannot be mined.

Archive logs must be retained up-to if not slightly longer than your longest downtime window of the connector.

[NOTE]
====
When a record has no value in the `NAME` column, this means the log has been removed the file system.
In this situation, the record is accompanied with `DELETED` being set to `YES` and the `STATUS` column being set to `D` to represent the log has been deleted.
====
