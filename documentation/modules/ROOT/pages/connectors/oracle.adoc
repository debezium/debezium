...
Oracle records `INTERNAL` events in the redo log to associate `ROW_ID` values with out-of-line LOB operations.
Capturing these events helps the connector avoid emitting stale LOB values after savepoint rollbacks.
