...
* Set the connector property link:#oracle-property-database-history-store-only-captured-tables-ddl[`database.history.store.only.captured.tables.ddl`] to `true`.
====
...
If a table matches any of these rules, the table will need to be renamed for the connector to capture its changes.
