...
.Prerequisites
* The connector is configured to use one of the available link:{debezium-signaling}#sending-signals-to-a-debezium-connector[{prodname} signaling channels].
...
The values should be ordered by preference as the {prodname} connector will use the first destination that is `LOCAL` and `VALID`.
...
This will prioritize destination #2 and it will be used on the primary and destination #5 will be used on the standby.

If the destination for CDC is the same for both the primary and standby instances, using a single value is sufficient.
