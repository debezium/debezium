...
In the event your primary and standby archive destination configurations are significantly different, you can use the hostname for the instance as an additional qualifier.
To do so, use the format `+<hostname>:<destination_name>+` separated by commas.
The hostname check is a case-insensitive match against the `HOST_NAME` column value in the `V$INSTANCE` performance view.
====
