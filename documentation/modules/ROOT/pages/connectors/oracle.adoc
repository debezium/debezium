...
The data dictionary is exported once, so it does not describe schema changes that are applied to the primary afterwards.
The connector resolves the affected tables and columns from its own schema model, and rebuilding the dictionary file restores resolution on the LogMiner side.
For details of what happens when an object cannot be resolved by either, see the xref:oracle-property-log-mining-strategy[`log.mining.strategy`] property.
...
The file is a point-in-time copy, so LogMiner cannot name an object that is created or altered after the file is built, and reports it as `OBJ#` followed by the object identifier.
The connector resolves such objects from its own schema model, and reconstructs the column names that LogMiner reports as `COL 1`, `COL 2`, and so forth.
An object that the connector has no schema for, such as a table created after the initial snapshot ran, cannot be resolved this way.
The connector then applies the xref:oracle-property-event-processing-failure-handling-mode[`event.processing.failure.handling.mode`] property, which stops the connector by default.
Rebuild the dictionary file to have LogMiner resolve these objects again.
endif::community[]
