...
The {prodname} Oracle connector can only capture changes when all required database transaction logs are available and contain no sequence gaps.
When using the LogMiner adapter, users may notice when this requirement cannot be met, log entries appear reporting that an Oracle redo thread is inconsistent:
+
```
Redo Thread X is inconsistent; an archive log with sequence 12345 is not available
```
An Oracle redo thread can be inconsistent for a variety of reasons, including but not limited to:
. While an Oracle RAC node is in the process of shutting down, when the redo thread transitions from OPEN to CLOSED.
. An online redo log has reached maximum capacity and is queued for archival, but the archival process has not completed.
. The resume read position the connector is not available.
...
Given that {prodname} does not have direct filesystem access, the log must be registered in the performance view before Debezium can continue. +
 +
In some instances, redo thread inconsistency may result in an error that reports the SCN is not available.
It is important to review the log entries prior to this exception to determine if the reason was because the maximum number of retries were exhausted. +
 +
When the Oracle ARC process takes more time to complete the archival than the {prodname} Oracle connector allows, the internal connector configuration property `internal.log.mining.query.max.retries` can be adjusted to increase the number of retry attempts.
By default, the connector retries up to `5` attempts before throwing an exception.
