...
To ensure reliable log mining and schema change detection, ensure that the mining database can access dictionary and redo log files on the primary, per the following requirements:

* Network and file access settings permit the mining database to access the dictionary file and redo logs on the primary database.
* The dictionary file on the primary instance is current and it properly reflects schema changes.
* The dictionary file on the mining database is synchronized with the file on the primary.
Synchronization gaps can lead to problems in detecting schema changes.
endif::community[]
////
