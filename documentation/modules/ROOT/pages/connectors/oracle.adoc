...
Debezium emits a new message to this topic whenever a new table is streamed from or when the structure of the table is altered ({link-prefix}:{link-oracle-connector}#oracle-schema-evolution[schema evolution procedure must be followed]).
...
[NOTE]
====
When the columns for a row's primary/unique key are updated, the value of the row's key has changed so Debezium will output _three_ events: a `DELETE` event and a {link-prefix}:{link-oracle-connector}#oracle-tombstone-events[tombstone event] with the old key for the row, followed by an `INSERT` event with the new key for the row.
====
...
|[[oracle-property-table-whitelist]]<<oracle-property-table-whitelist, `table.whitelist`>>
|_empty string_
...
|[[oracle-property-table-blacklist]]<<oracle-property-table-blacklist, `table.blacklist`>>
|_empty string_
...
|[[oracle-property-provide-transaction-metadata]]<<oracle-property-provide-transaction-metadata, `provide.transaction.metadata` (Incubating)>>
|`false`
