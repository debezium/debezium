...
To permit {prodname} to capture changes to LOB data, you must set the xref:oracle-property-lob-enabled[`lob.enabled`] connector configuration property to `true`.

Set `lob.enabled` to `true` if you want the connector to capture the following data types:
...
For more information about the connector supports extended strings, see xref:support-for-extended-max-string-size[Extended max string size].
...
Oracle writes `INTERNAL` events  (operation code 0) to the redo log to associate `ROW_ID` values with out-of-line LOB operations.
Configure `log.mining.include.internal.events` to specify whether the connector captures these events to enable accurate handling of `ROLLBACK TO SAVEPOINT` in transactions that modify LOB columns.
...
To prevent {prodname} from emitting change events that contain stale LOB values, you can set the xref:oracle-property-log-mining-include-internal-events[`log.mining.include.internal.events`] configuration property.
...
This setting increases the volume of mined redo log data, but enables the connector to reliably handle `ROLLBACK TO SAVEPOINT` operations for tables with out-of-line LOB columns.
