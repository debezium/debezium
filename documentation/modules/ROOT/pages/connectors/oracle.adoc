...
`database.standby.hostname`:: Specifies the hostname of the downstream mining database from which you want the {prodname} Oracle connector to read and capture transaction logs.
...
`database.standby.port`:: Specifies the port of the downstream mining database, if it differs from the primary instance port.
`database.standby.url`:: Specifies the full JDBC connection string to the downstream mining database. Use this as an alternative to `database.standby.hostname` and `database.standby.port` when the connection string requires additional parameters.
...
database.standby.hostname=standby-host
```
+
Alternatively, you can use `database.standby.url` to specify the full JDBC connection string to the standby:
+
```
database.standby.url=jdbc:oracle:thin:@standby-host:1521/ORCLPDB1
```
+
If the standby uses a different port than the primary, you can also set `database.standby.port`.
...
2. Connects to the standby database (configured via `database.standby.hostname` or `database.standby.url`) in read-only mode for LogMiner streaming operations.
...
|[[oracle-property-database-standby-hostname]]<<oracle-property-database-standby-hostname, `+database.standby.hostname+`>>
|No default
|Specifies the hostname of the physical standby database from which the connector streams changes.
When set, the connector uses the primary database (configured via `database.hostname`) for snapshots and metadata queries, and the standby for LogMiner streaming.

|[[oracle-property-database-standby-port]]<<oracle-property-database-standby-port, `+database.standby.port+`>>
|Same as `database.port`
|Specifies the port of the standby database, if it differs from the primary instance port.

|[[oracle-property-database-standby-url]]<<oracle-property-database-standby-url, `+database.standby.url+`>>
|No default
|Specifies the full JDBC connection string to the standby database.
Use this as an alternative to `database.standby.hostname` and `database.standby.port` when the connection string requires additional parameters.
endif::community[]
