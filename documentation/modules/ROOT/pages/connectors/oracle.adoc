...
There are two ways to establish mTLS connections:

* xref:#using-mtls-secure-connections-java-key-trust-stores[]
* xref:#using-mtls-secure-connections-oracle-wallet[]

[[using-mtls-secure-connections-java-key-trust-stores]]
==== Java key/trust stores

.Prerequisites
* Verify connector can access key/trust store files directly.
* Verify database TNS listener supports `tcps` secure connections.

.Procedure
* Configure the {prodname} Oracle connector, as illustrated below in the example.
...
[NOTE]
====
It is important for TLS encryption to use `database.url` rather than `database.host`.
The URL provides a TNS connection string which is what explicitly controls the secure connection requirement by setting the protocol to `tcps`.
====
