...
=== Connector fails to connect due to ORA-01882 with Oracle 11g

When using `ojdbc8.jar` with Oracle 11g, the connector may raise the following error on start up:
...
In order to use `ojdbc8.jar` with Oracle 11g, the connector configuration must include `database.oracle.jdbc.timezoneAsRegion=false`.
This allows the driver to be able to successfully connect to Oracle 11g.