...
*What is the cause for ORA-01013 and how to handle it?*::
The {prodname} Oracle Connector may report the following exception when a query execution takes too much time:
+
```
ORA-01013: user requested cancel of current operation
```
+
The {prodname} Oracle connector provides a configuration option, xref:oracle-property-database-query-timeout-ms[`database.query.timeout.ms`], which controls how long a query may execute before being canceled.
This prevents resources being allocated for long queries, which can potentially cause database performance degradation.
...
+
The configuration option can be increased providing a higher threshold before queries are canceled, particularly in cases where the exception is raised frequently.
The configuration option can also be set to `0`, disabling query timeouts, and causes all queries to execute until they return results.
