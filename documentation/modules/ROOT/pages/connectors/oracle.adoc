...
=== Built-in excluded schemas

The {prodname} Oracle connector automatically excludes capturing changes from the following schemas:
`appqossys`, `audsys`, `ctxsys`, `dvsys`, `dbsfwuser`, `dbsnmp`, `qsmadmin_internal`, `lbacsys`, `mdsys`, `ojvmsys`, `olapsys`, `orddata`, `ordsys`, `outln`, `sys`, `system`, `wmsys`, and `xdb`.
In order to guarantee that a table is captured by the connector, please be sure to any other schema.
