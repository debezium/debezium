...
{prodname} ingests change events from Oracle using the https://docs.oracle.com/database/121/XSTRM/xstrm_intro.htm#XSTRM72647[XStream API] or directly by Log Miner.
...
[NOTE]
====
The Log Miner implementation requires the database to be configured with a minimum number of redo log groups and files per group.
...
Create Log Miner user (used by the {prodname} connector to connect to Oracle):
...
[NOTE]
====
If you're using the Log Miner implementation, this step is not necessary.
====
...
`logminer` uses the native Oracle Log Miner API.
