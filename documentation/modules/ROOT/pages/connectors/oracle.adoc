...
However, you can use a specific destination can be used by providing the destination name, for example, `LOG_ARCHIVE_DEST_5`. +
 +
Please see xref:#_archive_log_destinations[] for more information if your primary and standby instances use different names.
...
|[[oracle-property-guardrail-tables-max]]<<oracle-property-guardrail-tables-max, `+guardrail.tables.max+`>>
|`0`
|Specifies the maximum number of tables that can be captured by the connector. When this limit is exceeded, the action specified by `guardrail.tables.limit.action` will be taken.
A value of `0` disables this guardrail.

|[[oracle-property-guardrail-tables-limit-action]]<<oracle-property-guardrail-tables-limit-action, `+guardrail.tables.limit.action+`>>
|`warn`
|Specifies the action to take when a guardrail tables limit is exceeded.
Set the property to one of the following values:

`fail`::
The connector fails if the total number of tables being captured exceed the maximum limit specified by the `guardrail.tables.max` property.

`warn`::
The connector logs a warning if the total number of tables being captured exceed the maximum limit specified by the `guardrail.tables.max` property.
