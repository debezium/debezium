...
The {prodname} Oracle connector supports multiple _adapters_ for capturing changes from the Oracle database’s transaction logs.
You can configure the adapter by setting the xref:oracle-property-database-connection-adapter[`database.connection.adapter`] configuration property.
...
LogMiner can operate in two distinct modes, both supported by {prodname}.
...
As this is the connector’s default mode, ensure that the connector runtime is allocated sufficient memory to handle your largest transactions and highest concurrency levels across all captured tables.

This mode minimizes load on the source database by delegating most processing to the connector.
...
The {prodname} Oracle connector can be configured to use OpenLogReplicator, an open-source application that reads Oracle changes directly from redo and archive logs with minimal impact on the database.
...
For more details, see the xref:oracle-xstreams-support[] section.
