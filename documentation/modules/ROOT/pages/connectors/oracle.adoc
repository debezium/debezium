...
Oracle allows defining custom user types when the built-in types are insufficient.
There are a several user-defined types such as Object types, REF data types, Varrays, and Nested Tables.
At this time, none of these supplied user-defined types are supported by the {prodname} Oracle connector.

[id="oracle-suppplied-types"]
=== Oracle supplied types

Oracle provides SQL-based interfaces to define new types when the built-in or ANSI-supported types are insufficient.
Oracle offers several commonly used data types to serve a broad array of purposes such as *Any*, *XML*, or *Spatial* types.
At this time, none of these are supported by the {prodname} Oracle connector.
...
The {prodname} Oracle connector is compatible with both installations.
...
An Oracle database may have either a physical or a logical standby environment configured in the case of a production failure.
...
In the case of a physical standby, the standby will be an exact copy of production, which means the SCN values are identical.
...
In the case of a logical standby, the standby is not an exact copy of production and therefore the SCN offsets will be different between production and the standby.
When using a logical standby, it is recommended that, once the database is open, a new connector be used and to perform a new snapshot to guarantee that no change events are lost.
