...
|[[oracle-property-log-mining-buffer-memory-transaction-events-threshold]]<<oracle-property-log-mining-buffer-memory-transaction-events-threshold, `+log.mining.buffer.memory.transaction.events.threshold+`>>
|`0`
| The transaction size threshold Debezium is able to handle when `log.mining.buffer.type` = memory. Transaction with event count exceeds this threshold will be abandoned. Value '0' means no limitation. +
...
|`long`
|The number of oversized transactions in the transaction buffer.
