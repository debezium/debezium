...
There are also several xref:#oracle-openlogreplicator-known-issues[known issues] with the current implementation.
Please let us know if you encounter any other problems beyond those known.
====
...
[[oracle-openlogreplicator-known-issues]]
=== Known issues

The following describe known problems with the OpenLogReplicator ingestion adapter.

*Connector restarts may miss or re-send old events*::
The current checkpoint confirmation logic does not correctly account for situations where transactions may interweave.
As a result, connector restarts can cause already sent events to be re-delivered or in-progress transactions that interweave with other transactions that commit after a connector restarts to be omitted from the event stream.
Unfortunately this is due to how OpenLogReplicator confirms redo blocks and they're actively working on a fix we hope to integrate in a future build.
