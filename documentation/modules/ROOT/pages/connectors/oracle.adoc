...
[WARNING]
====
While database changes performed by the connector user account will be captured by the connector,
changes made by the `SYS` and `SYSTEM` user accounts will not.
====
...
|`memory`
|The buffer type controls how the connector will manage buffering transaction data. +
 +
`memory` - Uses the JVM process' heap to buffer all transaction data.
This is the ideal choice if you don't expect a lot of long-running or large transactions. +
The buffer state is not persisted across restarts and will need to be recreated from the current offset's SCN value. +
 +
`infinispan` - This uses an embedded Infinispan cache to buffer all transaction data and persist it to disk.
This is the ideal choice if you expect long-running or large transactions.
Since the buffer state is persisted across restarts, the buffer does not need to be recreated at restart.
Using this mode requires the use of `log.mining.buffer.location` to designate where the cache files are to be written.

|[[oracle-property-log-mining-buffer-location]]<<oracle-property-log-mining-buffer-location, `+log.mining.buffer.location+`>>
|
|Specifies a directory location to where the connector can read and write the buffer cache to disk.
This directory should be shared and available across all nodes in a Kafka Cluster and should be unique for each connector deployment.
...
|`false`
|Whether or not to delete the buffer state when the connector stops in a graceful, expected way. +
 +
