...
* *Archive-log-only mode*: You must explicitly set `log.mining.archive.log.only.mode=true` in the connector configuration.
ADB does not provide access to online redo logs.
If this property is not enabled, the connector will fail during startup validation.
...
When connecting to ADB, the connector must enable autonomous database mode and archive-log-only mode.
...
----
{
    ...
    "config": {
        ...
        "log.mining.archive.log.only.mode": "true",  // <5>
        "driver.oracle.net.wallet_location": "(SOURCE=(METHOD=file)(METHOD_DATA=(DIRECTORY=_<wallet_location>_)))",  // <6>
        "schema.history.internal.kafka.bootstrap.servers" : "kafka:9092",
        "schema.history.internal.kafka.topic": "schema-changes.inventory"
    }
}
...
<5> Required for Autonomous Database. ADB does not provide access to online redo logs, so archive-log-only mode must be enabled.
<6> The Oracle Wallet location configuration. Download the wallet from the Oracle Cloud Console and specify the directory path where the wallet files are stored. For more information about Oracle Wallet configuration, see xref:using-mtls-secure-connections-oracle-wallet[].
