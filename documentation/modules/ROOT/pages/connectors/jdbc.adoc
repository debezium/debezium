...
|`long`
|The total number of upsert (insert or update) operations that the connector applied to the target database. +
The connector cannot distinguish an insert from an update because flattened records carry no operation field and the connector applies both as an upsert.
