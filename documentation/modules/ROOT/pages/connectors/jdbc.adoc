...
Defines the name for the Containerfile.
...
...
  config:
    connection.url: jdbc:<_{context}_>://<_database-host_>:5432/<_database-name_>
...
`metadata.labels` (`strimzi.io/cluster`)::
Identifies the KafkaConnect cluster resource that hosts the connector.
{StreamsName} uses this label associate the KafkaConnector with a specific KafkaConnect deployment.
