...
By contrast, to represent more complex data, such as `Struct` types, which group together fixed sets of named fields each with its own data type, Kafka uses logical schema names.
Logical types take into account the semantic meaning of the data and provide a more opinionated interpretation about how to serialize underlying primitive types.
Logical types are also useful in representing values that have a specific encoding, such as numbers that represent the time since the epoch.
