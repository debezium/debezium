...
[IMPORTANT]
====
When a logical type maps to more than one type, the {prodname} JDBC connector defaults to the first more generalized data type.

When consuming changes from topics populated by a {prodname} source connector, enable either `column.propagate.source.type` or `datatype.propagate.source.type` in the source connector configuration to pass column metadata that the JDBC connector will use to construct columns with a stricter concrete data type, shown as the second type.
====
