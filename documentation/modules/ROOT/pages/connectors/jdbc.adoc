...
This is because deletes are executed based on the primary key mapping, and therefore if a destination table has no primary key mapping, rows cannot be deleted by the connector.
...
No, that is actual one of the differentiating factors of the {prodname} JDBC connector from other implementations.
