...
|Specifies the fully-qualified class name of a `ColumnNamingStrategy` implementation that the connector uses to resolve column names from event field names. +
 +
The `i.d.c.j.n.UnicodeColumnNamingStrategy` will revert column names including unicodes from the `avro_unicode` of debezium's `field.name.adjustment.mode` back to normal characters. +
 +
...
|Specifies the fully-qualified class name of a `TableNamingStrategy` implementation that the connector uses to resolve table names from incoming event topic names. +
 +
The `i.d.c.j.n.UnicodeTableNamingStrategy` will revert the topic name including unicodes from debezium's `DefaultUnicodeTopicNamingStrategy` or `SchemaUnicodeTopicNamingStrategy` back to normal characters. +
 +
