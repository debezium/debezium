...
|5
|Specifies the maximum number of attempts to flush changes to the target database on specific database errors.
If the number of retries is exhausted, the sink connector will enter a _FAILED_ state.

|[[jdbc-property-flush-retry-delay-ms]]<<jdbc-property-flush-retry-delay-ms, `+flush.retry.delay.ms+`>>
|1000
|Specifies the number of milliseconds to wait in between flush attempts when a flush operation fails and is scheduled to be retried.
