...
When `schema.evolution` is set to `none-validated`, startup validation only checks whether the destination table exists.
...
The connector can perform startup validation only for table names that it can resolve from static connector configuration.
For example, it can validate a literal xref:jdbc-property-collection-name-format[`collection.name.format`] value, or a `collection.name.format` value that uses `+${topic}+` together with a statically configured `topics` list and the default collection naming strategy.
The connector cannot perform startup validation when `topics.regex` is used with `+${topic}+`, when `collection.name.format` uses `+${source.*}+` placeholders, or when a non-default collection naming strategy is configured.
