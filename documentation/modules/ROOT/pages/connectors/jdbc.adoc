...
When `schema.evolution` is set to `none-validated`, validation only checks whether the destination table exists.
...
`none-validated`:: Specifies that the connector does not evolve the destination schema.
