...
[IMPORTANT]
====
This connector is not meant to process `schema change` records. If your source connector is configured to capture schema changes then configure `topics` or `topics.regex` JDBC connector properties to exclude `schema change` topic.
====
