...
+
You can use the enhanced column naming capabilities by setting <<jdbc-property-column-naming-style-enabled, `+column.naming.style.enabled+`>> to `true`.

|[[jdbc-property-column-naming-style-enabled]]<<jdbc-property-column-naming-style-enabled, `+column.naming.style.enabled+`>>
|`false`
|When set to `true`, enables enhanced column naming with support for naming styles, prefixes, and suffixes. When enabled, this activates the custom naming strategy which provides more flexible column name transformations.

|[[jdbc-property-column-naming-style]]<<jdbc-property-column-naming-style, `+column.naming.style+`>>
|`default`
|The naming style to use for column names. Only used when <<jdbc-property-column-naming-style-enabled, `+column.naming.style.enabled+`>> is `true`. +
Valid options are: +
...
* `upper_case` - Transforms to UPPERCASE
* `lower_case` - Transforms to lowercase

|[[jdbc-property-column-naming-prefix]]<<jdbc-property-column-naming-prefix, `+column.naming.prefix+`>>
|_empty string_
|Optional prefix to add to column names. Only used when <<jdbc-property-column-naming-style-enabled, `+column.naming.style.enabled+`>> is `true`.

|[[jdbc-property-column-naming-suffix]]<<jdbc-property-column-naming-suffix, `+column.naming.suffix+`>>
|_empty string_
|Optional suffix to add to column names. Only used when <<jdbc-property-column-naming-style-enabled, `+column.naming.style.enabled+`>> is `true`.
