...
|`log_bin_compress`
|Specifies whether the binary log can be compressed. The {prodname} {connector-name} connector does not support compressed binary log entries, so for compatibility, you must set `log_bin_compress` to `0` (the default) to disable compression.
