...
|`log_bin_compress`
|Whether or not the binary log can be compressed. The {prodname} {connector-name} connector does not support compressed binary log entries, so `log_bin_compress` must be set to `0` (the default), which means no compression.
