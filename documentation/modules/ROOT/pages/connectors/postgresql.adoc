...
* Tables with `REPLICA IDENTITY FULL` - unchanged TOAST column values are part of the `before` block, so connector is able to ingest them as if they were provided in `after` block.
