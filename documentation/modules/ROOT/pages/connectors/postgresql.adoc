...
It is possible to use {prodname} with link:https://cloud.google.com/sql/postgresql/[Cloud SQL for PostgreSQL], which supports logical decoding for PostgreSQL.

To use Cloud SQL with {prodname}:

* Enable the `cloudsql.logical_decoding` flag so Cloud SQL configures `wal_level=logical`.
* Configure the replication user with privileges required for logical decoding.
* Set the {prodname} `plugin.name` parameter to `pgoutput`.
* For Cloud SQL-specific steps to configure logical replication, publications, and replication slots, see the link:https://cloud.google.com/sql/docs/postgres/replication/configure-logical-replication[Google Cloud documentation].

.Setting the `cloudsql.logical_decoding` flag
In Cloud SQL, set the `cloudsql.logical_decoding` flag to `on` by using either the Google Cloud console or the `gcloud` CLI.
After you apply the change, verify that the server reports logical decoding enabled:

----
SHOW wal_level;
----

.Configuring a replication user
To use logical decoding, create a PostgreSQL user with the `REPLICATION` attribute (or grant it to an existing user).
Run the commands as `postgres` or as a member of the `cloudsqlsuperuser` group:
...
In the connector configuration, set `plugin.name` to `pgoutput`:
