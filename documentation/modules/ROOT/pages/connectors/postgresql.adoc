...
|`io.debezium.time.StructuredTimestamp` +
 +
Contains `year`, `month`, `day`, `hour`, `minute`, `second`, and `nanos` fields. PostgreSQL `infinity` and `-infinity` values are represented as `specialValue`.

|`TIMETZ`
|`STRUCT`
|`io.debezium.time.StructuredZonedTime` +
 +
Contains clock component fields and an `offsetSeconds` field.

|`TIMESTAMPTZ`
|`STRUCT`
|`io.debezium.time.StructuredZonedTimestamp` +
 +
Contains calendar and clock component fields, `offsetSeconds`, and optional `zoneId`. PostgreSQL `infinity` and `-infinity` values are represented as `specialValue`.
