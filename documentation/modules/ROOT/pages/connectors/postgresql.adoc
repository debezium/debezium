...
For users AWS RDS with Postgres a similar situation to the third cause may occur on an idle environment, since AWS RDS makes writes to its own system tables not visible to the useres on a frequent basis (5 minutes). Again
regularly emitting events will solve the problem. AWS DMS also made a similar approach. See release notes for 2.3.0 [here](https://docs.aws.amazon.com/dms/latest/userguide/CHAP_ReleaseNotes.html)
...
Kafka, Zookeeper, and Kafka Connect all have built-in support for JMX metrics. The PostgreSQL connector also publishes a number of metrics about the connector's activities that can be monitored through JMX. The connector has two types of metrics. Snapshot metrics help you monitor the snapshot activity and are available when the connector is performing a snapshot. Streaming metrics help you monitor the progress and activity while the connector processes the logical replication stream.

[[monitoring-snapshots]]
[[snapshot-metrics]]
==== Snapshot Metrics

===== *MBean: debezium.postgres:type=connector-metrics,context=snapshot,server=_<database.server.name>_*
...
===== *MBean: debezium.postres:type=connector-metrics,context=streaming,server=_<database.server.name>_*
