...
As a result, for topics with multiple partitions, there are no ordering guarantees for the change events (_create_, _update_, and so forth), or _truncate_ events that pertain to a table.
For example, when a consumer reads events for a table from multiple partitions, it might receive an _update_ event for a table only after it receives a _truncate_ event that deletes all of the data in the table.
====
