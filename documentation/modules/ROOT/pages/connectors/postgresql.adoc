...
For more advanced uses, you can fine control the snapshot implementing the following interfaces:

* `io.debezium.snapshot.spi.Snapshotter`: allows control whether to take a snapshot
* `io.debezium.snapshot.spi.SnapshotQuery`: allows control on how data is queried
* `io.debezium.snapshot.spi.SnapshotLock`: allows control whether to take locks
...
`exported` -  deprecated +
 +
ifdef::community[]
`custom` - The connector performs a snapshot according to the implementation provided via `snapshot.mode.custom.name` property, which is a custom implementation of the `io.debezium.spi.snapshot.Snapshotter` interface. +
endif::community[]
 +
...
|No default
| When `snapshot.mode` is set as `custom`, this setting must be set to specify the name of the custom implementation provided in the `name()` method defined by the 'io.debezium.spi.snapshot.Snapshotter' interface.
The provided implementation is called on each app boot to determine whether to do a snapshot. See xref:postgresql-custom-snapshot[custom snapshotter SPI].
endif::community[]
...
|Controls how the connector holds locks on tables while performing the schema snapshot: +
 +
`shared` -  The connector will hold a table lock that prevents exclusive table access for just the initial portion of the snapshot while the database schemas and other metadata are being read. The remaining work in a snapshot involves selecting all rows from each table, and this is done using a flashback query that requires no locks. +
 +
`none` -  The connector avoids locks entirely. This mode is only safe to use if no schema changes are happening while the snapshot is taken. +
 +
ifdef::community[]
`custom` - The connector performs a snapshot according to the implementation provided via `snapshot.locking.mode.custom.name` property, which is a custom implementation of the `io.debezium.spi.snapshot.SnapshotLock` interface. +
endif::community[]
...
|No default
| When `snapshot.locking.mode` is set as `custom`, this setting must be set to specify the name of the custom implementation provided in the `name()` method defined by the 'io.debezium.spi.snapshot.SnapshotLock' interface.
See xref:postgresql-custom-snapshot[custom snapshotter SPI].
endif::community[]

|[[postgresql-property-snapshot-query-mode]]<<postgresql-property-snapshot-query-mode, `+snapshot.query.mode+`>>
|`select_all`
|Controls how the connector queries data while performing the snapshot: +
 +

`select_all` - The connector performs a select all query. +
 +
ifdef::community[]
`custom` - The connector performs a snapshot query according to the implementation provided via `snapshot.query.mode.custom.name` property, which is a custom implementation of the `io.debezium.spi.snapshot.SnapshotQuery` interface. +
endif::community[]
 +
This is a more flexible alternative to xref:{context}-property-snapshot-select-statement-overrides[`snapshot.select.statement.overrides`]
...
|No default
| When `snapshot.query.mode` is set as `custom`, this setting must be set to specify the name of the custom implementation provided in the `name()` method defined by the 'io.debezium.spi.snapshot.SnapshotQuery' interface.
See xref:postgresql-custom-snapshot[custom snapshotter SPI].
endif::community[]
