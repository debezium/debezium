...
In certain cases, it is possible that PostgreSQL disk space consumed by WAL file either experiences spikes or increases out of usual proportions.
Three reasons that explain the situation

 * Debezium regularly confirms LSN of processed events to the database.
This is visible as `confirmed_flush_lsn` in `pg_replication_slots` slots table.
 ...
 * There are many updates in a monitored database but only minuscule amount relates to the monitored tables and/or schemas.
 ...
 * The PostgreSQL instance contains multiple databases where one of them is a high-traffic database.
 ...
 Debezium then cannot confirm the LSN as replication slot works per-database and Debezium is not invoked.
 ...
 * enable periodic heartbeat record generation using `heartbeat.interval.ms` configuration option
 * regularly emit change events by Debezium monitored database
