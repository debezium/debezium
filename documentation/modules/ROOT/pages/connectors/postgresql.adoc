...
`commit_lsn`:: The log sequence number (LSN) of the transaction's commit record, taken from the pgoutput `BEGIN` message.
Unlike the per-event `source.lsn`, this value is identical for every event in the transaction and increases in commit order, so it can be used as a single-scalar watermark for ordering or deduplication.
The field is optional and is omitted when the connector cannot determine the commit LSN (for example, for snapshot records or when transaction metadata is not available).
...
`commit_lsn`:: The log sequence number (LSN) of the transaction's commit record, identical to the `commit_lsn` reported on the transaction's `BEGIN`/`END` events.
The field is optional and is omitted when the connector cannot determine the commit LSN.
...
// TODO: Review this introductory sentence for accuracy after callout removal (DBZ-9674).
