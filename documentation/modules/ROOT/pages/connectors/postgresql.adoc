...
* `id` - string representation of unique transaction identifier
* `total_order` - the total order of event generated by the transaction
* `data_collection_order` - the per-data collection order of event emitted by the transaction
...
See link:#transaction-metadata[section] with additional details.
