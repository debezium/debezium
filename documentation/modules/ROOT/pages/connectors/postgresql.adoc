...
* Initiate logical replication from an AWS account that has the `rds_replication` role.
  ...
  For example, `grant rds_replication to _<my_user>_`. Superuser access is required to grant the role.
  To enable the non-master account to perform an initial snapshot, the account also requires `SELECT` permission on the schema of any tables to be captured.
