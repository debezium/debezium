...
// Type: concept
[id="debezium-postgresql-chunk-based-parallel-snapshots"]
...
When a PostgreSQL connector uses a read-only connection, you can trigger incremental snapshots without using a signal data collection table.
Instead, you can use any {link-prefix}:{link-signalling}#sending-signals-to-a-debezium-connector[available signaling channel].
endif::community[]
...
`name` (`io.debezium.connector.postgresql.Source`)::
`io.debezium.connector.postgresql.Source` is the schema for the payload's `source` field.
...
[NOTE]
====
Updates to the columns that represent the primary or unique key for a row changes the value of the row's key.
...
For more information, see xref:postgresql-primary-key-updates[Primary key updates].
====
...
* Database and table that contains the new row
* Schema name
* Whether the event was part of a snapshot (always `false` for _delete_ events)
* ID of the transaction in which the operation was performed
* Offset of the operation in the database log
* Timestamp for when the change was made in the database
...
A *message* event captures a generic logical decoding message that is written directly to the WAL by using the `pg_logical_emit_message` function.
The following examples show the event structure for transactional and non-transactional messages, and describe the fields in each.

The message key is a `Struct` with a single field named `prefix`, carrying the prefix specified when inserting the message.
The following JSON shows an example of a message value for a transactional message:
...
The following list describes select fields in the preceding _message_ event value examples:
...
The time is based on the system clock in the JVM running the Kafka Connect task.
+
For transactional _message_ events, the `ts_ms` attribute of the `source` object indicates the time that the change was made in the database.
By comparing the value for `payload.source.ts_ms` with the value for `payload.ts_ms`, you can determine the lag between the source database update and {prodname}.
+
For non-transactional _message_ events, the `source` object's `ts_ms` indicates the time at which the connector encounters the _message_ event, while the `payload.ts_ms` indicates the time at which the connector processed the event.
...
* xref:postgresql-time-precision-mode-structured[`time.precision.mode=structured`]
...
[IMPORTANT]
====
Because nanoseconds since the Unix epoch are stored in a signed `INT64`, the representable range of `time.precision.mode=nanoseconds` is approximately *`1677-09-21T00:12:43Z` to `2262-04-11T23:47:16Z`*.
Values outside this range — for example the common `9999-12-31` end-of-time sentinel — cannot be represented and cause the connector to throw an `IllegalArgumentException` from the value converter.
...
If the source schema may contain values outside the representable range, use `time.precision.mode=microseconds` (covers approximately ±290000 years) or `time.precision.mode=isostring` instead.
====


[[postgresql-time-precision-mode-structured]]
...
[NOTE]
====
To preserve finite PostgreSQL `TIMESTAMP` values that exceed the range of an epoch-microsecond integer, use the `pgoutput` logical decoding plug-in.
The `pgoutput` plug-in passes the PostgreSQL text representation to the connector, which enables values such as `294276-12-31 23:59:59.999999` to be represented as structured calendar and clock components.
When you use the `decoderbufs` plug-in, these values can reach the connector as already-converted epoch-based instants.
If that conversion overflows before structured conversion, the connector cannot recover the original calendar value.
====
...
|`io.debezium.time.StructuredDate` +
 +
Contains `year`, `month`, and `day` fields. PostgreSQL `infinity` and `-infinity` values are represented as `specialValue`.
...
Before running the {prodname} PostgreSQL connector, configure PostgreSQL to support logical replication, set up the required user permissions, and allow network access for the connector host.

ifdef::community[]
Decide which logical decoding plug-in you intend to use.
...
ifdef::product[]
The {prodname} PostgreSQL connector requires the `pgoutput` logical replication plug-in.
When the connector starts, it automatically creates the configured replication slot if the slot does not already exist.
...
When you run PostgreSQL on a managed cloud platform such as Amazon RDS, Azure Database for PostgreSQL, Google Cloud SQL, or CrunchyBridge, additional steps are required to enable logical replication for {prodname}.
...
Install a logical decoding output plug-in on the PostgreSQL server so that the {prodname} connector can receive change events.
The `pgoutput` plug-in is built into PostgreSQL and requires no installation; other plug-ins, such as `decoderbufs`, must be compiled and installed separately.
...
Set the `plugin.name` connector property to specify which logical decoding plug-in the connector uses to read change events from the PostgreSQL replication stream.
...
Configure the PostgreSQL server to support logical decoding and to provide the replication parameters that the {prodname} connector requires to capture change events reliably.
...
When you use the `pgoutput` logical decoding plug-in, {prodname} streams change events from PostgreSQL publications.
Grant the {prodname} database user the required privileges, and configure shared table ownership, so that the connector can create and manage publications for the tables it captures.
...
----
+
The following list describes the purpose of select lines in the preceding `pg_hba.conf` example:
...
ifdef::product[]
To deploy a {prodname} PostgreSQL connector, use {StreamsName} to automatically create a Kafka Connect image that includes the connector plug-in.
...
`database.user`::
The name of the PostgreSQL user that has the xref:postgresql-permissions[required privileges].

`database.password`::
The password for the PostgreSQL user that has the xref:postgresql-permissions[required privileges].

`database.dbname`::
The name of the PostgreSQL database to connect to.

`topic.prefix`::
The topic prefix for the PostgreSQL server/cluster, which forms a namespace and is used in all the names of the Kafka topics to which the connector writes, the Kafka Connect schema names, and the namespaces of the corresponding Avro schema when the Avro converter is used.

`table.include.list`::
A list of all tables hosted by this server that this connector will monitor.
...
If you use xref:postgresql-time-precision-mode-structured[`time.precision.mode=structured`] to preserve finite `TIMESTAMP` values beyond the epoch-microsecond range, set `plugin.name` to `pgoutput`.
endif::community[]
ifdef::product[]
...
If the slot is ahead of the offset, PostgreSQL returns an error because the requested LSN is no longer available in the WAL (Postgres 14 and older) or will start from `confirmed_flush_lsn` (Postgres 15 and newer).
...
`isostring` represents time, date, and timestamp values as ISO-8601 formatted strings at the UTC time zone. +
 +
`microseconds` represents time, date, and timestamp values using microsecond precision. +
 +
`nanoseconds` represents time, date, and timestamp values using nanosecond precision. +
 +
`structured` represents time, date, timestamp, and interval values as Kafka Connect structs with separate calendar and clock components.
...
|[[postgresql-property-poll-dispatch-interval-ms]]<<postgresql-property-poll-dispatch-interval-ms, `+poll.dispatch.interval.ms+`>>
|`0`
...
For more information, see xref:postgresql-streaming-metrics[Streaming metrics].
...
|[[postgresql-property-incremental-snapshot-pool-release-delay-ms]]<<postgresql-property-incremental-snapshot-pool-release-delay-ms, `+incremental.snapshot.pool.release.delay.ms+`>>
|`60000` (1 minute)
...
|[[postgresql-property-incremental-snapshot-retry-max-attempts]]<<postgresql-property-incremental-snapshot-retry-max-attempts, `+incremental.snapshot.retry.max.attempts+`>>
|`5`
...
|[[postgresql-property-incremental-snapshot-retry-initial-delay-ms]]<<postgresql-property-incremental-snapshot-retry-initial-delay-ms, `+incremental.snapshot.retry.initial.delay.ms+`>>
|`1000`
...
|[[postgresql-property-incremental-snapshot-retry-max-delay-ms]]<<postgresql-property-incremental-snapshot-retry-max-delay-ms, `+incremental.snapshot.retry.max.delay.ms+`>>
|`60000` (1 minute)
...
|[[postgresql-property-incremental-snapshot-retry-backoff-multiplier]]<<postgresql-property-incremental-snapshot-retry-backoff-multiplier, `+incremental.snapshot.retry.backoff.multiplier+`>>
|`2.0`
...
|[[postgresql-property-snapshot-max-threads-multiplier]]<<postgresql-property-snapshot-max-threads-multiplier, `+snapshot.max.threads.multiplier+`>>
|`1`
...
|[[postgresql-property-legacy-snapshot-max-threads]]<<postgresql-property-legacy-snapshot-max-threads, `+legacy.snapshot.max.threads+`>>
|`false`
...
The behavior of the {prodname} PostgreSQL connector after a cluster failure depends on the PostgreSQL version.
PostgreSQL 17 and later support failover replication slots for automatic recovery; earlier versions require manual intervention to resume event capture.
