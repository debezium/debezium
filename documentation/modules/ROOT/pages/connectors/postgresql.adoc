...
If you are using one of the supported link:#postgresql-output-plugin[logical decoding plug-ins] (i.e. not pgoutput) and it has been installed,
...
Each `record` produced by the PostgreSQL connector has, in addition to the link:#postgresql-events[_database event_], some meta-information about where the event occurred on the server, the name of the source partition and the name of the Kafka topic and partition where the event should be placed:
...
[WARNING]
====
Whether or not this field is available is highly dependent on the link:#postgresql-replica-identity[_REPLICA IDENTITY_] setting for each table
====
...
* `source` is a mandatory field that contains a structure describing the source metadata for the event, which in the case of PostgreSQL contains several fields: the {prodname} version, the connector name, the name of the affected database, schema and table, whether the event is part of an ongoing snapshot or not and the same fields from the record's link:#postgresql-meta-information[_meta information_] section
...
* The `before` field now has the state of the row with the values before the database commit, but only for the primary key column `id`. This is because the  link:#postgresql-replica-identity[_REPLICA IDENTITY_] which is by default `DEFAULT`.
...
[NOTE]
====
When the columns for a row's primary/unique key are updated, the value of the row's key has changed so {prodname} will output _three_ events: a `DELETE` event and link:#postgresql-tombstone-events[tombstone event] with the old key for the row, followed by an `INSERT` event with the new key for the row.
====
...
* The `before` field now has the state of the row that was deleted with the database commit. Again this only contains the primary key column due to the link:#postgresql-replica-identity[_REPLICA IDENTITY_] setting
...
. Install the link:#postgresql-output-plugin[logical decoding plugin]
. Configure the link:#postgresql-server-configuration[PostgreSQL server] to support logical replication
...
See the link:#postgresql-connector-properties[complete list of connector properties] that can be specified in these configurations.
...
|`decoderbufs`
|The name of the Postgres link:#postgresql-output-plugin[logical decoding plugin] installed on the server.
ifdef::product[]
...
ifdef::community[]
Finally, if set to *custom* then the user must also set `snapshot.custom.class` which is a custom implementation of the `io.debezium.connector.postgresql.spi.Snapshotter` interface. See link:#postgresql-snapshots[snapshots].
...
|`10000`
|Positive integer value that specifies the maximum amount of time (in milliseconds) to wait to obtain table locks when performing a snapshot. If table locks cannot be acquired in this time interval, the snapshot will fail. See link:#postgresql-snapshots[snapshots]
...
See link:#postgresql-toasted-values[Toasted Values] for additional details.

|[[postgresql-property-provide-transaction-metadata]]<<postgresql-property-provide-transaction-metadata, `provide.transaction.metadata` (Incubating)>>
|`false`
...
See link:#postgresql-transaction-metadata[Transaction Metadata] for additional details.
...
If the primary node goes down, only after a new primary has been promoted (with the link:#postgresql-output-plugin[logical decoding plugin] installed) and a replication slot has been created there, the connector can be restarted and pointed to the new server.
