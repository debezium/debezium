...
This property replaces the deprecated `slot.seek.to.known.offset.on.start` boolean property.
...
When using a persistent offset store, ensure slot position is preserved during primary-replica failover, such as by using PostgreSQL 17 or later with xref:postgresql-property-slot-failover[`slot.failover=true`].
...
The same slot reliability requirements as `trust_slot` apply when using a persistent offset store.
+
...
For backward compatibility, the deprecated `slot.seek.to.known.offset.on.start` property is automatically mapped:

* `slot.seek.to.known.offset.on.start=false` maps to `offset.mismatch.strategy=no_validation`
* `slot.seek.to.known.offset.on.start=true` maps to `offset.mismatch.strategy=trust_offset`
...
|No default
|The name of the ConnectionFactory. For example, can be used to handle AWS IAM Authentication (see xref:postgresql-on-amazon-rds[here]).
...
+
NOTE: When using `connector_and_driver` mode, the JDBC driver can flush LSN positions for database activity that {prodname} has not yet processed into change events.
Because the replication slot advances ahead of the stored offset, this can cause startup issues if the connector restarts.
