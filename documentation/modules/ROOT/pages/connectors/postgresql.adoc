...
and add the debezium user to the group
...
Alternatively if we don't want to do this we can create the publication outside of debezium and then set

`"publication.autocreate.mode": "filtered".` or `disabled`

to stop {prodname} from attempting to add the table to the publication.

Finally, In order to be able to copy the initial table data, the role used for the replication connection must have the SELECT privilege on a published table. This is fairly trivial and nothing is required if the debezium user is in the group that owns the table.
