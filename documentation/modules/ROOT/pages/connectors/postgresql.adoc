...
[[postgresql-adhoc-snapshot]]
==== Ad-hoc snapshot

When the initial snapshot is completed then it is not executed again as further data are acquired via streaming.

Sometimes it would be really useful to re-execute the snapshot, either full or for only a subset of tables.
Examples for such use cases contain

* a new table was added into the list of captured tables
* some of the topics were deleted and their content needs to be rebuild
* the data were corrupted due to a configuration error

Debezium can be requested via mechanism of {link-prefix}:{link-signalling}[signalling table] to execute the new snapshot of defined table list.
The signal is named `execute-snapshot` and accepts message in format
...
|`data-collections`
|_N/A_
| Array of qualified table names, to be snapshotted like `schema1.table`

|===

An example of the signal to be triggered is
...
[[postgresql-incremental-snapshot]]
==== Incremental snapshot

The initial snapshot is a great mechanism to obtain a consistent set of all data stored in database upfront.
Its strengths are outweighed by some negatives

* For larger datasets it can take very long to complete; hours or even days.
* The snapshot must be completed before the start of the streaming.
* Snapshot is not resumable.
If it is not completed it must be restarted from scratch.
* New tables that are added among the captured ones later are not snapshotted.

To mitigate the negatives a new mechanism calle incremental snapshot has been introduced.

[NOTE]
====
Incremental snapshot can be started current only as an ad-hoc snapshot.
====

[NOTE]
====
Configuration option {link-prefix}:{link-postgresql-connector}#postgresql-property-signal-data-collection[signal.data.collection] must be set.
====

Incremental snapshot is base on link:https://github.com/debezium/debezium-design-documents/blob/main/DDD-3.md[DDD-3] design document.
...
The incremental snapshot events should be understand in a different way then events for the initial snapshot.
The `read` events shoudl be understand like a materialization of data in a certain period of time.
This means that contrary to initial snapshot semantics in case of incremental snapshots
...
|
| Fully-qualified name of data collection that is use to send {link-prefix}:{link-signalling}[signals] to the connector.
