...
However, as part of the PostgreSQL upgrade process, the database removes all replication slots, including those that store the {prodname} connector offsets.
After the upgrade, the database does not restore the original slots.
For the connector to start and to continue capturing change events from the database, replication slots must be present.
You can create the new slot manually, or you can rely on {prodname}'s automatic slot creation process.

But the presence of a new replication slot is not sufficient to guarantee against data loss.
When you create a new slot, the log sequence numbers (LSNs) on the new slot do not match the numbers from the original slot, which were based on the offsets that existed before the upgrade.
So when the connector restarts and it reads the WAL files on the new slot, it cannot find the last known position from which to resume streaming.
Because the connector cannot find the known offset, it skips any existing change events in the log and resumes streaming only from the most recent position.
Silent data loss occurs, because the connector does not emit records for the skipped events, and the connector does not report an error.
To ensure that {prodname} captures does not miss any events, it might be necessary to remove offsets from from the offset storage topic.
See the following procedure for guidance about how to perform a PostgreSQL database upgrade to enable {prodname} to continue to capture events, and to minimize the risk of data loss.
...
4. Verify that any changes that occurred in the database before you blocked write operations are saved to the WAL on the replication slot.
