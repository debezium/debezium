...
ifdef::community[]
For the `decoderbufs` plug-in, valid parameters include `include-xacts`, `write-in-chunks`, and `chunk-size`.
For example,

 include-xacts=true;write-in-chunks=true;chunk-size=65536

These options can help to prevent memory-related problems when the plugin handles large transactions.
endif::community[]

Valid parameters for use with the `pgoutput` plug-in, include `publication_names`, `messages`, `binary`, `streaming`, `two_phase`, and `origin`.
For example,

 publicationnames=dbzpublication;messages=true;binary=true;two_phase=true;origin=any

Use these options to customize streaming behavior related to transaction boundaries, multi-phase commits, and replication filtering.


[NOTE]
====
The parameters that are available for use with the `pgoutput` plug-in depend on the `proto_version`, which defines the version of the logical streaming replication protocol, and which is specified when you create the replication slot.
Currently, if you configure {prodname} to create replication slots automatically, the value of `proto_version` is hardcoded to `1`.
This version of the protocol does not support streaming of large in-progress transactions, or the use of two-phase commits.
To take advantage of those features, you must create slots manually and set the `proto_version` to a higher value.
For more information, see the https://www.postgresql.org/docs/18/protocol-logical-replication.html[PostgreSQL documentation].
====
...
Specify `false` if you do not want the connector to commit the LSN of processed records.

NOTE: If you set the value of this property to `false`, {prodname} does not acknowledge the LSN.
Failure to acknowledge the LSN can lead to uncontrolled growth of the WAL logs, which stresses storage capacity, and could result degraded performance, and even data loss.
To maintain normal service, if you set this property to `false`, you must configure some other mechanism to commit the LSN.
...
[IMPORTANT]
====
Exactly-once delivery of PostgreSQL change event records is a Technology Preview feature only.
