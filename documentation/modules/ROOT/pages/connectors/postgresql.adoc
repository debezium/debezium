...
There are some really important caveats to failovers, and you should pause {ProductName} until you can verify that you have a replication slot intact which has not lost data.  After a failover, you will lose data unless your administration of failovers includes a process to recreate the {ProductName} replication slot before the application is allowed to write to the *new* primary. You also may need to verify in a failover situation that {ProductName} was able to read all changes in the slot **before the old primary failed**.
