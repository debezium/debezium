...
[id="{context}-ad-hoc-snapshots"]
==== Ad hoc snapshots
include::{partialsdir}/modules/all-connectors/ref-connector-ad-hoc-snapshots.adoc[leveloffset=+3]
...
For more information, see xref:postgresql-incremental-snapshots[Incremental snapshots].
====
endif::[product]
...
For more information, see xref:sending-signals-to-a-debezium-connector[Sending signals to a {prodname} connector].
====
endif::[product]
...
Incremental snapshots are not affected by the `snapshot.include.collection.list`.
