...
All data change events produced by the Cassandra connector have a key and a value, although the structure of the key and value depends on the table from which the change events originated (see link:#cassandra-topic-names[Topic Names]).
...
The Cassandra connector should be deployed each Cassandra node in a Cassandra cluster. The Cassandra connector Jar file takes in a cdc configuration (.properties) file. See link:#cassandra-example-configuration[see example configurations] for reference.
