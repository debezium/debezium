...
When tracing is enabled, the connector creates two nested spans for each change event that it emits:
...
|`false`
|Enables OpenTelemetry tracing for change events emitted by the connector, if the OpenTelemetry API JAR is available on the classpath.
