...
Schema changes in Cassandra will be detected less than 1s by the implemented SchemaChangeListener, which will then update the Schema Instance loaded from Cassandra as well as the Kafka key value schemas cached upon starting.
...
Please note that with this schema evolution approach, Cassandra Connector won't be able provide accurate data change information in the following schema change situations:
