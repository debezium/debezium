...
* xref:debezium-mongodb-sink-connector-data-types-mapping[]
...
// Type: concept
// ModuleID: debezium-mongodb-sink-connector-data-format-that-the-connector-supports
// Title: Data formats that the {prodname} MongoDB sink connector supports
[[mongodb-sink-supported-data-formats]]
== Supported data formats

The {prodname} MongoDB sink connector supports the following data formats:

* Kafka Messages with Schema (Struct)
* CloudEvents
* JSON (as a Map)
* JSON (as a String)
* Raw binary BSON byte array


// Type: concept
// ModuleID: debezium-mongodb-sink-connector-how-the-connector-maps-data-types
// Title: How the {prodname} MongoDB sink connector maps data types
[[mongodb-sink-data-types-mapping]]
== Data type mappings

Before the {prodname} MongoDB sink connector sends data to a MongoDB instance, it converts the data types in the source records to corresponding types in MongoDB.
Appropriate data type mapping ensures that the original data is accurately represented in the destination database.

The {prodname} MongoDB sink connector resolves a column's data type by using a logical or primitive type-mapping system.
...
For a JSON string, JSON map, or a raw BSON byte array, the connector uses the default data type mappings from MongoDB to store the document.
...
For example, when a {prodname} source connector emits a change event, it assigns similar logical types to event fields to represent such data types as timestamps, dates, and even JSON data.


The {prodname} MongoDB sink connector uses these primitive and logical types to resolve a column's type to a MongoDB BSON document field type.
The following tables illustrate the primitive and logical mappings between Kafka Connect and MongoDB BSON types, and between {prodname} and MongoDB BSON types.

. xref:mongodb-sink-kafka-connect-primitive-mappings[]
. xref:mongodb-sink-kafka-connect-logical-mappings[]


// Type: reference
// Title: Mappings between Kafka Connect primitive data types and MongoDB field types
// ModuleID: debezium-mongodb-sink-connector-mappings-between-kc-primitive-data-types-and-mongodb-field-types
[[mongodb-sink-kafka-connect-primitive-mappings]]

.Mappings between Kafka Connect primitive data types and MongoDB field data types
|===
|Primitive Type |MongoDB BSON Type
...
// Type: reference
// Title: Mappings between Kafka Connect logical data types and MongoDB field types
// ModuleID: debezium-mongodb-connector-mappings-betw-kc-logical-data-types-and-mongodb-field-types
[[mongodb-sink-kafka-connect-logical-mappings]]

.Mappings between Kafka Connect logical data types and column data types
|===
|Logical Type |MongoDB BSON type
...
[NOTE]
====
Support for other data types, such as {prodname} logical types, is not currently available.
====
...
|No default value
|Specifies a https://www.mongodb.com/docs/manual/reference/connection-string/[MongoDB connection string (URI)] that the connector uses to connect to a MongoDB instance.
