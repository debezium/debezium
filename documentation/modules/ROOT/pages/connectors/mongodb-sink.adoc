...
// Type: assembly
// ModuleID: debezium-mongodb-sink-connector-supported-data-format
// Title: Which data formats the {prodname} MongoDB sink connector supports
...
// Type: assembly
// ModuleID: debezium-mongodb-sink-connector-data-types-mapping
// Title: How the {prodname} MongoDB sink connector maps data types
[[mongodb-sink-data-types-mapping]]
== Data type mappings

Before the {prodname} MongoDB sink connector sends stores data into a MongoDB instance, it converts data types from the original source record to a corresponding type in MongoDB.
...
For a JSON string, a JSON map, or a raw BSON byte array the document is stored using the default data type mappings from MongoDB.
...
[NOTE]
====
Other data types, for example Debezium logical types, are not yet supported.
====
...
|No default value
| A MongoDB connection string (URI) that the sink uses to connect to MongoDB. Specifies a https://www.mongodb.com/docs/manual/reference/connection-string/[connection string] that the connector uses to connect to a MongoDB instance.
