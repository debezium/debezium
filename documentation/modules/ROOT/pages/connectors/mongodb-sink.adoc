...
The {prodname} MongoDB sink connector mirrors the state of tables in a relational source database to collections in a MongoDB database by consuming {prodname} change events from Kafka topics and applying them as document-level operations.
...
The following example demonstrates a connector configuration that reads change events from specific topics in a relational database and writes the resulting documents to a MongoDB sink database.
The example configures the connector to capture change events from the `dbserver1.inventory.customers`, `dbserver1.inventory.orders`, and `dbserver1.inventory.products` topics and writes the data to documents in a `debezium` collection in the MongoDB sink database.
