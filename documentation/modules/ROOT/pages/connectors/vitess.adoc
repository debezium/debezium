...
When you make changes to the connector configuration that affect transaction ordering semantics, you must increment the connector generation to ensure that downstream consumers can correctly determine event order.
The connector generation is a configuration property that, when increased, triggers an automatic increment of the `transaction_epoch` for all shards.

.When to Increment Connector Generation

Increment the connector generation when you enable or disable features that change how the `transaction_rank` field is computed.
For example, when enabling transaction chunking, the `transaction_rank` is computed differently because the VGTID is not available until the final chunk of a transaction.
Without incrementing the connector generation, downstream consumers might incorrectly determine event order when comparing events produced before and after the configuration change.

.How Connector Generation Works

When the connector starts and detects that the configured generation is higher than the generation stored in the offset, it increments the `transaction_epoch` for all shards by one.
This ensures that events produced after the configuration change have a higher epoch than events produced before, allowing consumers to correctly apply the ordering logic described in xref:vitess-ordered-transaction-metadata[Ordered Transaction Metadata].

The connector also increments epochs if the configured generation is lower than the stored generation (a downgrade scenario), as a safety measure.

To configure the connector generation, set the xref:vitess-property-connector-generation[`vitess.connector.generation`] property.
...
|`0`
|Generation number for transaction ordering semantics.
Increment this value when making configuration changes that affect how transaction ordering metadata is computed, such as enabling transaction chunking.
When the connector detects that the configured generation differs from the generation stored in the offset, it increments the `transaction_epoch` for all shards.
This setting only takes effect when `transaction.metadata.factory` is set to `io.debezium.connector.vitess.pipeline.txmetadata.VitessOrderedTransactionMetadataFactory`.
