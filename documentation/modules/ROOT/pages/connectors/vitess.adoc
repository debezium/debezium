...
* VStream 8.0.0 doesn't provide additional metadata of permitted values for `ENUM` columns. Therefore, the Connector does not support the `ENUM` column type. The index number (1-based) will to emitted instead of the enumeration value. E.g. `"3"` will be emitted as the value instead of `"L"` if the `ENUM` definition is `enum('S','M','L')`.
