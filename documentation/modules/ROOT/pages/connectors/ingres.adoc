...
The {prodname} Ingres connector captures row-level changes in the tables of an Actian Analytics database.
...
This connector is strongly inspired by the {prodname} connector implementation for IBM Db2.
The connector use the Ingres Change Streams Java client to capture transactional data from the database.

The Change Data Capture API captures data from databases that have logging enabled and captures transactions from the current logical log. 
The API processes all transactions sequentially.

The first time that a {prodname} Ingres connector connects to an Ingres database, the connector reads a consistent snapshot of the tables for which the connector is configured to capture changes. 
...
Applications and services can then consume change event records from the resulting topics.
...
The {prodname} Ingres connector is based on the link:https://docs.actian.com/actianingres/12.1/index.html#page/DatabaseAdmin/Change_Data_Capture.htm[Ingres Change Data Capture API] that enables Change Data Capture in Ingres. 
The API was introduced in Ingres 12.1, and is available in all subsequent versions of the database.

The database administrator must enable journaling on the Ingres database to capture changes. 

To enable CDC on the database, you must have `DB_ADMIN` privileges or be the database or instance owner. 
For more information about how to configure CDC on Ingres, see link:https://docs.actian.com/actianingres/12.1/index.html#page/DatabaseAdmin/Prerequisites_for_Server_Configuration.htm[Prerequisites for server configuration].
...
If the connector stops for any reason, including communication failures, network problems, or crashes, after restarting it continues reading the change stream where it left off.
...
4. Generate a single transaction on a generated database table.
  Ingres does not support querying the latest transaction record position.
  Therefore the {prodname} connector creates a table in the database, inserts one row then drops the table to generate a "latest" log record that is captured and stored. 
  This is the log position where data streaming will start from after the snapshot is complete. 
...
[CAUTION]
====
The current version of the Ingres connector does not implement table locking.
If the connector runs a snapshot on a table while other client operations are active in the database, snapshot consistency is not guaranteed.
====
...
[id="ingres-capturing-data-from-tables-not-captured-by-the-initial-snapshot"]
...
For information about capturing data from a new table that has undergone structural changes, see xref:ingres-capturing-data-from-new-tables-with-schema-changes[].
...
6. (Optional) If the connector performed a `no_data` snapshot, after the snapshot completes, initiate an xref:debezium-ingres-incremental-snapshots[incremental snapshot] to capture data from the tables that you added.
...
[id="ingres-capturing-data-from-new-tables-with-schema-changes"]
...
3. Initiate an xref:debezium-ingres-incremental-snapshots[incremental snapshot] if you want to capture existing data from the newly added tables.
...
7. To ensure that no data is lost, initiate an xref:debezium-ingres-incremental-snapshots[incremental snapshot].
...
7. (Optional) To capture any data that changed while the connector was off-line, initiate an xref:debezium-ingres-incremental-snapshots[incremental snapshot].

// Type: concept
[id="debezium-ingres-chunk-based-parallel-snapshots"]
=== Chunk-based parallel snapshots

include::{partialsdir}/modules/all-connectors/con-connector-chunk-based-parallel-snapshots.adoc[leveloffset=+1]

[id="ingres-ad-hoc-snapshots"]
=== Ad hoc snapshots

include::{partialsdir}/modules/all-connectors/con-connector-ad-hoc-snapshots.adoc[leveloffset=+1]


[id="debezium-ingres-incremental-snapshots"]
...
[WARNING]
====
The {prodname} connector for Ingres does not support schema changes while an incremental snapshot is running.
====

[id="ingres-triggering-an-incremental-snapshot"]
==== Triggering an incremental snapshot

include::{partialsdir}/modules/all-connectors/proc-triggering-an-incremental-snapshot-sql.adoc[leveloffset=+1]

[id="ingres-incremental-snapshots-additional-conditions"]
...
// Type: procedure
[id="ingres-triggering-an-incremental-snapshot-kafka"]
...
// Type: procedure
[id="ingres-stopping-an-incremental-snapshot"]
...
// Type: procedure
[id="ingres-stopping-an-incremental-snapshot-kafka"]
...
The {prodname} Ingres connector generates a data change event record for each row-level `INSERT`, `UPDATE`, `DELETE`, and `TRUNCATE` operation. 
Each event contains a key and a value. 
The event structure depends on the table that was changed.

[[ingres-data-types]]
== Data type mappings

The Ingres connector represents changes to rows with events that are structured like the table. 
The event contains a field for each column value.
How that value is represented in the event depends on the Ingres data type of the column.
For information about how the connector maps Ingres data types in event fields to literal and semantic types, see xref:ingres-basic-types[Basic types].
...
literal type:: Describes how the value is literally represented using the following Kafka Connect schema types: `INT8`, `INT16`, `INT32`, `INT64`, `FLOAT32`, `FLOAT64`, `BOOLEAN`, `STRING`, `BYTES`, `ARRAY`, `MAP`, and `STRUCT`.

semantic type:: Describes how the Kafka Connect schema captures the meaning of the source field by assigning the name of the corresponding Kafka Connect schema to the field.
...
|`INGRESDATE`
|`STRING`
|INGRESDATE can represent a date/time or an interval.
Users must handle the conversion of the string to a date/time or interval value in their application.
...
To prepare an Ingres database to work with {prodname}, ensure that the user account that the connector uses to connect to the database has the necessary permissions, and enable journaling on the database.

[[ingres-permissions]]
=== Setting up permissions

Users connecting to the server to utilize CDC must have `DB_ADMIN` privileges or be the database or instance owner.
For more information about setting up for CDC with Ingres, see link:https://docs.actian.com/actianingres/12.1/index.html#page/DatabaseAdmin/Prerequisites_for_Server_Configuration.htm[Prerequisites for server configuration].


[[ingres-journaling]]
=== Setting up journaling

Each database that the connector captures data from must have journaling enabled to capture changes.
To enable journaling on an Ingres database, use the `ckpdb` utility to set the `journaling` option to `on` for the database.
For example, to enable journaling for a database, run the following command:

[source,shell,subs="+quotes"]
----
> ckpdb +j <database name>
----
...
This name forms a namespace that is used in the names of all Kafka topics to which the connector writes, the Kafka Connect schema names, and the namespaces of the corresponding Avro schema when the {link-prefix}:{link-avro-serialization}#avro-serialization[Avro Connector] is used.
...
|`cdc_header_marker`
| The name of the table that the connector creates to store metadata about the current log position.
This is used to start data streaming after a snapshot is taken.
...
|[[ingres-property-snapshot-max-threads-multiplier]]<<ingres-property-snapshot-max-threads-multiplier, `+snapshot.max.threads.multiplier+`>>
|`1`
...
Smaller chunks keep threads more evenly loaded.
...
|[[ingres-property-legacy-snapshot-max-threads]]<<ingres-property-legacy-snapshot-max-threads, `+legacy.snapshot.max.threads+`>>
|`false`
