...
To enables CDC on the database, you must have `DB_ADMIN` privileges or be the database or instance owner. 
...
5. 
Therefore the Debezium connector creates a table in the database, inserts one row then drops the table to generate a "latest" log record that is captured and stored. 
...
The event contains a field for each column value. How that value is represented in the event depends on the Ingres data type of the column. 
