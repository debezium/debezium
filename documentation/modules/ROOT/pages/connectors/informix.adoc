...
`0` - Return immediately if no data is available. +
 +
`>=1` - The number of seconds to wait for data before timing out.

|[[informix-property-cdc-batchsize]]<<informix-property-cdc-batchsize, `+cdc.batchsize+`>>
|`0x100000`
|Positive integer value that specifies the maximum size of each batch of records that the Ifx Change Stream Client processes.
