...
|`true`
|Specifies whether the connector collects statistical metrics for streaming metrics, such as averages and quantiles.
 +
...
The statistics are computed using a probabilistic data structure (DDSketch) that provides approximate quantile values with 1% relative accuracy.
 +
When set to `false`, the connector does not collect these statistics, and the corresponding JMX metrics return `null` values.
Disabling statistics collection reduces memory overhead slightly.
 +
