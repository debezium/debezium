...
Use this property to specify a custom regular expression that identifies configuration keys that contain sensitive information and should be masked.

[IMPORTANT]
====
Setting `custom.sanitize.pattern` replaces Debezium's default pattern entirely; it does not extend or augment the default.
====

For example, to mask keys containing `api.key` or `token` anywhere in the name:

----
"custom.sanitize.pattern": ".*api\\.key.*\\|.*token.*"
----

Configuration keys whose values are masked are replaced with `pass:[********]` when configuration properties are displayed, logged, or returned by APIs.
