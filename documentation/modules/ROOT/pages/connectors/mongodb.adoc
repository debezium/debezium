...
|[[mongodb-property-query-fetch-size]]<<mongodb-property-query-fetch-size, `+query.fetch.size+`>>
|`0`
|Specifies the maximum number of change stream documents that MongoDB returns in each cursor response batch.
A positive value sets the batch size for both the initial change stream response and subsequent `getMore` responses.
The default value (`0`) leaves the batch size unspecified and uses the server default.
Negative values are not permitted.

MongoDB can return fewer documents than the configured value, including an empty batch, and its response size limits still apply.
When xref:mongodb-property-cursor-oversize-handling-mode[`cursor.oversize.handling.mode`] is `split`, each fragment counts as a separate change stream document.

This property applies to change streams for all capture scopes.
It is independent of xref:mongodb-property-snapshot-fetch-size[`snapshot.fetch.size`], which controls collection reads during a snapshot, and xref:mongodb-property-max-batch-size[`max.batch.size`], which controls the connector's record processing batches.
