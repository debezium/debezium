...
This class configures credentials for MongoDB clients.
Each connector task owns an instance, shared by its snapshot and streaming operations.
Connection validation uses a separate instance.
Implementations that allocate resources, such as authentication clients or HTTP connection pools, should override `close()` to release them.
The connector calls `close()` after the clients using the provider are closed, and also if initialization fails.
Implementations must therefore support cleanup after a partially completed `init()`.
The default `close()` implementation does nothing, so existing providers do not need to implement it.
