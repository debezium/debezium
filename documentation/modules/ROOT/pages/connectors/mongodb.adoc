...
After the discovery process completes, when the connector attempts to establish a direct connection to a primary replica set member, the connector then returns to using the standard connection properties. +
 +
When establishing a connection, the connector generally ignores this connection string.
However, it's possible that information that is required to establish a connection is not specified in the standard connection properties.
For example, the `mongodb.user` property might not be set, leaving the MongoDB username undefined.
When connection information is undefined in other connector properties, but is available in the connection string, the connector uses the information in the string to establish the connection.
====
