...
`sharded`:: The connector establishes a single connection to the mongos router instance that is specified in the xref:mongodb-property-mongodb-connection-string[`mongodb.connection.string`] property.
...
|[[mongodb-property-cursor-inactivity-pause-ms]]<<mongodb-property-cursor-inactivity-pause-ms, `+cursor.inactivity.pause.ms+`>>
|`500`
|Specifies the number of milliseconds the connector will pause for when no additional event is available.

In some cases (e.g. heavy use of bulk writes by data producers), lowering this value can lead to a significant connector throughput increase.
