...
The {prodname} MongoDB connector works by obtaining changes from https://www.mongodb.com/docs/manual/core/replica-set-oplog/[Oplog] using https://www.mongodb.com/docs/manual/changeStreams/[change streams]. As oplog is a https://www.mongodb.com/docs/manual/reference/glossary/#std-term-capped-collection/[capped collection], older entries are overwritten once maximum configured size is reached. For MongoDB connector to be able to deal with failures it is important that the oplog retention config is such that the resume point is available upon restart or retries. If the last oplog position is lost, the connector can not start again and a new connector has to be created afresh MongoDB is installed - See xref:debezium-mongodb-connector-is-stopped-for-a-long-interval[Connector fails after it is stopped for a long interval if snapshot.mode is set to initial].

To avoid this, you can provide a higher https://www.mongodb.com/docs/manual/core/replica-set-oplog/[Oplog size] or preferably a https://www.mongodb.com/docs/manual/core/replica-set-oplog/#std-label-replica-set-minimum-oplog-size/[Minimum Oplog Window] config available since MongoDB 4.4 (i.e. time-based, entries of the last X-hours are guaranteed to be available even if oplog size cap is reached). The later is preferable but may not always be the option as it can have impact if the cluster is reaching its capacity and there is sudden load of data. Thus you can instead choose to provide a higher Oplog size based on you workloads, ideally should be more than peak Oplog/Hour.

E.g. If we have 3 GB oplog per hour and only 1 GB oplog size limit. Oplog entries will get cleared thrice an hour, which would be bad if Debezium fails during this time. Thus, if your average reaction time to Debezium failure is 2 hours then the oplog size should be at least 6 GB.

Monitoring Oplog is easy on a Atlas MongoDB Cluster, See https://www.mongodb.com/basics/how-to-monitor-mongodb-and-what-metrics-to-monitor#oplog-gbhour/[Monitoring Oplog] about Replication Oplog Window (if debezium is disabled and lagging more than this, then it is bound to fail) and Oplog GB/Hour (explained above).
...
[WARNING]
====
If you set the value of this property to `database`, to use {prodname} signaling, you the signaling collection that you create must reside in the database that is specified by the xref:mongodb-property-capture-target[`capture.target`] property.
====
