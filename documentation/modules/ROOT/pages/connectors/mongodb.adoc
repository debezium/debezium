...
If the connector restarts during an incremental snapshot, it re-reads the snapshot window that was in progress to avoid skipping documents that were not emitted before shutdown.
Records already emitted from that window can be emitted again.
