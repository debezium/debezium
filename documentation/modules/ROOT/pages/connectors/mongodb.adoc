...
This setting is time-based, such that entries in the last _n_ hours are guaranteed to be available even if oplog size cap is reached. 
...
----
This indicates that the entry which corresponds to the connector's resume token no longer exists in the Oplog. At this point the connector cannot resume streaming, because the oplog no longer contains the previous offset that the connector has processed.
