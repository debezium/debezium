...
If an offset can be found and it still exists in the oplog, then the task immediately proceeds with link:#mongodb-streaming-changes[streaming changes], starting at the recorded offset position.
...
The name of the Kafka topics always takes the form _logicalName_._databaseName_._collectionName_, where _logicalName_ is the link:#mongodb-logical-connector-name[logical name] of the connector as specified with the `mongodb.name` configuration property, _databaseName_ is the name of the database where the operation occurred, and _collectionName_ is the name of the MongoDB collection in which the affected document existed.
