...
Thus it is not possible to switch from one mode to another.
To prevent inadverent capture mode change the connector has a built-in safety check.

When the connector is started it checks the stored offsets and will use the capture mode that was used last time when the connector was stopped regardless of the configuration and just emits a warning into a log about that.
