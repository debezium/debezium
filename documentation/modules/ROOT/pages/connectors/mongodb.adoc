...
[NOTE]
====
Under most circumstances, the connector uses this connection string during the initial replica set discovery process only.
During this discovery process, the connector ignores the values specified in the standard connection properties (`mongodb.hosts`, `mongodb.user`, `mongodb.password`, `mongodb.authsource`, SSL configuration properties, and so forth). +
After the discovery process completes, and the connector attempts to establish a direct connection to a primary replica set member, it does the opposite.
That is, the connector returns to using the standard connection properties, and generally ignores this connection string.

However, in some situations, some of the information that is required to establish a connection might not be specified in the standard connection properties.
For example, the `mongodb.user` property might not be set, leaving the MongoDB username undefined.
If the missing information is available in the connection string, the connector uses the information in the string to establish the connection.
====
