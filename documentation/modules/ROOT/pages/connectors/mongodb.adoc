...
You cannot use this property together with the `mongodb.hosts` property. +
 +
[NOTE]
====
The connector uses this connection string during the initial replica set discovery process only.
During the discovery process, the connector ignores the values of other connection properties (`mongodb.hosts`, `mongodb.user`,`mongodb.password`, `mongodb.authsource`, SSL configuration properties, and so forth).
After the discovery process, when the connector attempts to establish a direct connection to a primary replica set member, it returns to using the standard connection properties,
unless `mongodb.connection.string` specifies credential information that is otherwise undefined in the previously mentioned connection properties (for example, `mongodb.user`).
