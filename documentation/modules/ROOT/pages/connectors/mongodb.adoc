...
By default, the connector streams changes from the replica set's primary node, but you can configure it to stream from a secondary by setting the read preference in the connection string (see xref:read-preference[]).
If the member that the connector is reading from becomes unavailable -- for example, because the replica set elects a new primary -- the connector stops streaming and automatically reconnects to another suitable member according to the configured read preference.
Similarly, if the connector is unable to communicate with the replica set, it attempts to reconnect (using exponential backoff so as to not overwhelm the network or replica set).
...
You control which member the connector reads from by setting the standard MongoDB link:https://www.mongodb.com/docs/manual/core/read-preference/[`readPreference`] parameter in the xref:mongodb-property-mongodb-connection-string[`mongodb.connection.string`].
...
Be aware, however, that change streams emit only majority-committed events, and a secondary can lag behind the primary by its replication delay, so events captured from a secondary can arrive later than they would from the primary.
...
The task then opens a MongoDB change stream -- against the deployment, a database, or a collection, depending on the xref:mongodb-property-capture-scope[`capture.scope`] setting -- and starts streaming changes from that position.
The change stream is served by the replica set member that matches the configured read preference (the primary, by default); see xref:read-preference[].
...
As described in xref:read-preference[], the connector tasks stream changes from the replica set member that matches the configured read preference.
By default this is the primary, which lets the connector capture operations with the lowest latency; you can configure the connector to read from a secondary instead, at the cost of additional latency from replication lag.
...
Once the connector is running, if the replica set member that the connector reads from (the primary, by default) becomes unavailable or unreachable, the connector repeatedly attempts to reconnect to a suitable member, using exponential backoff to prevent saturating the network or servers. If no suitable member is available after the configurable number of connection attempts, the connector fails.
