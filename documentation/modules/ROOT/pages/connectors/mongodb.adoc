...
MongoDB connector supports two distinct modes of capturing the changes controlled by {link-prefix}:{link-mongodb-connector}#mongodb-property-capture-mode[`capture.mode`] option:
...
The MongoDB connector uses this same replication mechanism as described above, though it does not actually become a member of the replica set.
...
The disadvantage of this mode is that Debezium does not provide updated value in case of the changed value but only the operation that was used to modify the record which can be either the full knew value or an update instruction.
...
The MongoDB connector uses similar replication mechanism to the described above, though it does not actually become a member of the replica set.
The main difference is that the connector does not read oplog directly but delegates capturing and decoding the oplog to https://docs.mongodb.com/manual/changeStreams/[Chnage Streams] features.
When used then the MongoDB server exposes the changes to collections as an event stream.
The connector watches the stream and delivers the changes downstream.
And, when the connector sees a replica set for the first time, it looks at the oplog to get the last recorded transaction and then performs a snapshot of the primary's databases and collections.
When all the data is copied, the connector then creates a change stream from the position it read earlier from the oplog.

This is recommended mode starting with MongoDB 4.x.

[WARNING]
====
Both capture modes use different values stored in offsets that allow them to resume streaming from the last position seen.
Thus it is not possible to switch from change streams to oplog.
To prevent inadverent capture mode change the connector has a built-in safety check.

When the connector is started it checks the stored offsets.
If the original capture mode was oplog based then it will try to migrate to change streams.
If the original capture mode was change streams based then it will switch to change streams regardless of the configuration and just emits a warning into a log about that.
If the capture mode should be switched then it is necessary to remove stored offsets and run the connector from scratch.
====
...
When the MongoDB connector stops, it records the last oplog/stream position that it processed, so that upon restart it simply begins streaming from that position.
...
The task then (depending on the configuration) either connects to the replica set's primary node or connects to a replica-set-wide change stream and start streaming changes from that position.
...
The value of a change event for an update in the sample `customers` collection has the same schema as a _create_ event for that collection. Likewise, the event value's payload has the same structure. However, the event value payload contains different values in an _update_ event. An _update_ event does have an `after` value only if `capture.mode` is set to `change_streams_update_full`. There is a new structured field `updateDescription` with a few additional fields:
...
|Contains the JSON string representation of the actual MongoDB document.
 +
An _update_ event value does not contain an `after` field if capture mode is not set to `change_streams_update_full`
...
|`initial`
|Specifies the criteria for running a snapshot upon startup of the connector. The default is *initial*, and specifies the connector reads a snapshot when either no offset is found or if the oplog/change stream no longer contains the previous offset. The *never* option specifies that the connector should never use snapshots, instead the connector should proceed to tail the log.

|[[mongodb-property-capture-mode]]<<mongodb-property-capture-mode, `+capture.mode+`>>
|`change_streams_update_full`
|Specifies the method used to capture changes from MongoDB server. The default is *change_streams_update_full*, and specifies the connector captures changes via MongoDB Change Streams mechanism, update message contains a full message. The *change_streams* specifies to use the same method but not to provide the full update message. +
Option `oplog` specifies to access MongoDB directly; this is the legacy method.
