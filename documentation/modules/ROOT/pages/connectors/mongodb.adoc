...
[WARNING]
====
To support {link-prefix}:{link-signalling}#[{prodname} signaling], if you set `capture.scope` is set to `database`, the xref:mongodb-property-signal-data-collection[signaling data collection] must reside in a database that is specified by the xref:mongodb-property-capture-target[`capture.target`] property.
====
