...
For each collection that you specify, also specify another configuration property: `snapshot{zwsp}.collection{zwsp}.filter{zwsp}.overrides{zwsp}._DB_NAME_._COLLECTION_NAME_`. For example, the name of the other configuration property might be:  `snapshot{zwsp}.collection{zwsp}.filter{zwsp}.overrides{zwsp}.customers{zwsp}.orders`. Set this property to a valid filter expression that retrieves only the items that you want in the snapshot. When the connector performs a snapshot, it retrieves only the items that matches the filter expression.
