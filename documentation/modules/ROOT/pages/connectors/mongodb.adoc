...
[WARNING]
====
When `capture.scope` is set to `collection`, the connector does not support {link-prefix}:{link-signalling}#[{prodname} signaling] via the `source` channel which is enabled by default. Due to this limitation the connector also does not support incremental snapshots.
====
