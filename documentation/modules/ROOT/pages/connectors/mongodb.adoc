...
The {prodname} connector for MongoDB can be used with https://www.mongodb.com/atlas/database[MongoDB Atlas].
Please make sure to have one of the xref:mongodb-property-capture-mode[`capture modes`] based on change streams (rather than oplog) enabled when connecting to MongoDB Atlas.
endif::community[]
