...
=== Required user permissions
Depending on the value of xref:mongodb-property-capture-scope[`capture.scope`] property the user used by the connector needs either the permissions to read any database (when xref:mongodb-property-capture-scope[`capture.scope`] is set to `deployment`) or the permissions to read the database specified by xref:mongodb-property-capture-target[`capture.target`] (when xref:mongodb-property-capture-scope[`capture.scope`] is set to `database`).

In any case the user should also have the permission to execute the https://www.mongodb.com/docs/manual/reference/command/ping/[ping] command.

Additionally when capturing changes from a sharded MongoDB cluster and the xref:mongodb-property-mongodb-connection-mode[`mongodb.connection.mode`] property is set to `replica_set` the user used by the connector also needs the permission to read the `config.shards` system collection.
...
|`deployment`
|Specifies the https://www.mongodb.com/docs/manual/changeStreams/#watch-a-collection--database--or-deployment[scope of change streamed] opened by the connector.
Set this property to one of the following values:
...
`database`:: opens a change stream cursor for a single database to watch for changes to all its non-system collections.
+
[WARNING]
====
To use signaling when `database` is specified as the value the signaling collection needs to be located in the database specified by the xref:mongodb-property-capture-target[`capture.target`] property.
====

|[[mongodb-property-capture-target]]<<mongodb-property-capture-target, `+capture.target+`>>
|
| Specifies the database to watch for changes when xref:mongodb-property-capture-scope[`capture.scope`] is set to `database`
