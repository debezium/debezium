...
+
To use the MongoDB connector with a replica set, provide the https://www.mongodb.com/docs/manual/reference/connection-string/[replica set connection string] through the connector's `mongodb.connection.string` property. The connector will start a task to connect to the replica set and capture the changes from change stream.

[WARNING]
====
Previous version of Debezium allowed the connector to forgo membership discovery by setting the  `mongodb.members.auto.discover` property to false. However, this option always had limitations, and it was only possible due to how older versions of the connector managed database connection. For this reason current versions of the connector no longer support this property.
====
...
+
To use the MongoDB connector with a sharded cluster, provide the https://www.mongodb.com/docs/manual/reference/connection-string/[sharded cluster connection string] through the connector's `mongodb.connection.string` property.

[WARNING]
====
Previous versions of MongoDB connector required the connector to be configured with the host addresses of the _configuration server_ replica set via the `mongodb.hosts` property. Connection string used by the newer connector versions should include the addresses of Mongo routes (also called `mongos`).
====

[NOTE]
====
When the connector connects to sharded cluster, it discovers the information about each replica set used as a shard in the cluster, and will then start up a separate task to capture the changes from each shard. If new shards are added to the cluster or existing shards removed, the connector will automatically adjust its tasks accordingly.
====
...
This property replaces  `mongodb.hosts` property available in previous versions of MongoDB connector. +
 +
[NOTE]
====
In case of sharded MongoDB cluster, the connector uses this connection string only during the initial shard discovery process. After the initial discovery process finishes a connection string is gennerated for each shard individually.
====
