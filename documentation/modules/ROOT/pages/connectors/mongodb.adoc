...
[IMPORTANT]
====
The use of the {prodname} `collection` option for the `capture.scope` property is a Technology Preview feature.
...
|
|Specifies the https://www.mongodb.com/docs/manual/changeStreams/#ref-start-time-id1[startAtOperationTime] change stream parameter. The value ought to be a long representation of Bson timestamp.
...
|[[mongodb-property-guardrail-tables-max]]<<mongodb-property-guardrail-tables-max, `+guardrail.tables.max+`>>
|`0`
...
|[[mongodb-property-guardrail-tables-limit-action]]<<mongodb-property-guardrail-tables-limit-action, `+guardrail.tables.limit.action+`>>
|`warn`
