...
|`never`
|The connector starts to stream event records for subsequent database changes. Evaluating deprecation in favor of `no_data`.
...
|`when_needed`
|The connector performs a snapshot upon startup whenever it deems it necessary. That is, when no offsets are available, or when a previously recorded offset specifies a log position that is not available in the server.
...
`never`:: The connector starts to stream event records for subsequent database changes. Evaluating deprecation in favor of `no_data`.
...
`select_all`:: `select_all`:: The connector performs a `select all` query by default, optionally adjusting the columns selected based on the column include and exclude list configurations.
