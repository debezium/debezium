...
// Type: concept
[id="debezium-mysql-chunk-based-parallel-snapshots"]
...
include::{partialsdir}/modules/all-connectors/shared-mariadb-mysql.adoc[leveloffset=+1,tags=set-binlog-position-intro]

// Type: concept
// ModuleID: debezium-mysql-connector-format-of-a-set-binlog-position-signal
// Title: Format of a set-binlog-position signal
==== Format of a `set-binlog-position` signal

include::{partialsdir}/modules/all-connectors/shared-mariadb-mysql.adoc[leveloffset=+1,tags=set-binlog-position-format]
...
:binlog-filename-example: mysql-bin.000003
:gtid-label: UUID-based GTID set
:gtid-value: 3E11FA47-71CA-11E1-9E33-C80AA9429562:1-100
include::{partialsdir}/modules/all-connectors/shared-mariadb-mysql.adoc[leveloffset=+1,tags=set-binlog-position-procedure]
...
|`gtid_mode`
a|Specifies whether GTID mode of the MySQL server is enabled.

* `ON` = enabled
* `OFF` = disabled
* `OFF_PERMISSIVE`, `ON_PERMISSIVE` = intermediate values, required while switching between `OFF` and `ON`
...
[NOTE]
====
The {prodname} MySQL connector uses GTIDs only when `gtid_mode` is `ON`.
In the intermediate modes, the connector streams by binlog file and position, and validates that position when it restarts.
If the offset contains a GTID, the connector logs a warning and ignores the GTID.
====
