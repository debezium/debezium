...
GTID set format::
When GTID mode is enabled, specify the GTID set in the signal, as in the following example:
+
[source,json]
----
...
The signal is subject to the following rules:

* You cannot specify both binlog file/position and GTID set in the same signal.
* When using binlog file and position format:
