...
|0
|Controls the order in which tables are processed in the initial snapshot. A `negative` value will order the tables by row count descending. A `positive` value will order the tables by row count ascending. A value of `0` will disable ordering by row count.
