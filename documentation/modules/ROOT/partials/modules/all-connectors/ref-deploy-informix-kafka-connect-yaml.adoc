    ...
    plugins:
      - name: debezium-connector-{connector-file}
        artifacts:
          ...
          - type: jar
            url: https://repo1.maven.org/maven2/com/ibm/informix/jdbc/{informix-jdbc-version}/jdbc-{informix-jdbc-version}.jar
...
`jdbc-informix`:::
Specifies the Maven Central location of the IBM {connector-name} JDBC driver.
