...
tag::boolean-values[]
{connector-name} treats `BOOL` and `BOOLEAN` as aliases for the `TINYINT(1)` data type.
During snapshots, {prodname} executes `SHOW CREATE TABLE` to obtain table definitions, and the database returns `TINYINT(1)` for columns that were declared as `BOOL`, `BOOLEAN`, or `TINYINT(1)`.
During streaming, {prodname} normalizes `BOOL` and `BOOLEAN` declarations in `CREATE TABLE` and `ALTER TABLE` statements to the same `TINYINT(1)` representation.
As a result, the connector emits these columns as `INT8` in both phases and preserves values across the full signed `TINYINT` range.
...
By default, the {prodname} {connector-name} connector maps `BOOL`, `BOOLEAN`, and `TINYINT(1)` columns to `INT8` fields during both snapshots and streaming.
To emit selected columns as `BOOLEAN` fields instead, you can apply the custom `TinyIntOneToBooleanConverter`, as shown in the following configuration example.
