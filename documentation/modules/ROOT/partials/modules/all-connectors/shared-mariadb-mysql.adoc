...
Contains signed `years`, `months`, `days`, `hours`, `minutes`, `seconds`, and `nanos` fields.

|`DATETIME[(M)]`
|`STRUCT`
a|`io.debezium.time.StructuredTimestamp` +
Contains `year`, `month`, `day`, `hour`, `minute`, `second`, and `nanos` fields.

|`TIMESTAMP[(M)]`
|`STRUCT`
a|`io.debezium.time.StructuredZonedTimestamp` +
Contains calendar and clock component fields, `offsetSeconds`, and optional `zoneId`.
