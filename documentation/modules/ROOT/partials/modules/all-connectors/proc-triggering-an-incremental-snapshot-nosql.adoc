...
`_id`::
An optional parameter that assigna an arbitrary string to be the identifier for the signal request.
...
You can specify one or more data collections along with an optional filter parameter for each collection.
Specify a filter to instruct the snapshot to capture  only fields that contain a specified value.
In this case, the parameter instructs the signal to capture only documents in the `schema1.table1` data collection in which the field `color` is present and contains the value `'blue'`.
