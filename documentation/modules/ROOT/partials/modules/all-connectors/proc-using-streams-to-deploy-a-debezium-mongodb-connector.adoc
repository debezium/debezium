You can deploy a {prodname} connector on OpenShift by using {StreamsName} to automatically build a Kafka Connect container image that includes the connector plug-in, without having to manually build the image first.
...
=====================================================================
+
The preceding connector configuration includes the following properties:
