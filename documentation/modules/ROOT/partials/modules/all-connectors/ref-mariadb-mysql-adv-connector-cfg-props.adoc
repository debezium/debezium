...
After the initial read fails, the connector retries the chunk up to five times in subsequent watermark windows.
If all five retries fail with a column mismatch, the connector reports an error through its task error handling instead of skipping the table.
Windows spent verifying the schema do not consume retries.
The retry count resets after a successful chunk read or when a new table or snapshot begins.
The connector does not report the snapshot as complete while the table is waiting for its schema to catch up.
If transaction cleanup or watermark-window closure fails during recovery, the connector reports the failure through its task error handling.
When this property is `false`, a column mismatch continues to skip the affected table after transaction cleanup is attempted.
+
