The following list describes the advanced configuration properties for the {connector-name} connector that let you fine-tune behavior for specific environments, including schema history handling, signal channels, event filtering, and custom transformations.
...
[id="{context}-property-binlog-net-read-timeout"]
xref:{context}-property-binlog-net-read-timeout[`binlog.net.read.timeout`]::

Default value::: `0`

Description:::
The number of seconds to wait for a read from the binlog connection to complete before the server times out.
A value of `0` means that the connector uses the MySQL server default.
+
In high-latency network environments, the default server value for `net_read_timeout` might be too low, causing the server to close the binlog streaming connection prematurely with an `EOFException`.
Increase this value to prevent unexpected disconnects during binlog streaming.
+
NOTE: This value is set at the session level on the binlog streaming connection using `SET net_read_timeout=<value>`.
It does not affect the global server setting.


[id="{context}-property-binlog-net-write-timeout"]
xref:{context}-property-binlog-net-write-timeout[`binlog.net.write.timeout`]::

Default value::: `0`

Description:::
The number of seconds to wait for a write to the binlog connection to complete before the server times out.
A value of `0` means that the connector uses the MySQL server default.
+
When the server transfers large data volumes through the binlog, the default server value for `net_write_timeout` might be too low, causing the server to close the connection with an `EOFException`.
Increase this value to prevent unexpected disconnects during streaming of large transactions.
+
NOTE: This value is set at the session level on the binlog streaming connection using `SET net_write_timeout=<value>`.
It does not affect the global server setting.
...
[id="{context}-property-custom-sanitize-pattern"]
xref:{context}-property-custom-sanitize-pattern[`custom.sanitize.pattern`]::

Default value::: `.*secret$|.*password$|.*sasl\.jaas\.config$|.*basic\.auth\.user\.info|.*registry\.auth\.client-secret`
...
`verify-full`:::: Use SSL/TLS for encryption, certificate verification, and hostname verification. This is the standard TLS behavior.
endif::MARIADB[]
...
[id="{context}-property-ddl-parser-type"]
xref:{context}-property-ddl-parser-type[`ddl.parser.type`]::
...
`default`:::: (Default) Uses the Oracle MySQL grammar, which is actively maintained and supports MySQL 8.0+ features.
This grammar is based on the official MySQL grammar specification and is recommended for production use.

`legacy`:::: Uses the Positive Technologies (PT) MySQL grammar.
This option is provided for backward compatibility with existing deployments.
The PT grammar may not support all MySQL 8.0+ features.
+
In most cases, you should use the `default` grammar.
Use the `legacy` grammar only if you encounter compatibility issues with existing DDL statements during migration.
...
[id="{context}-property-incremental-snapshot-pool-release-delay-ms"]
xref:{context}-property-incremental-snapshot-pool-release-delay-ms[`incremental.snapshot.pool.release.delay.ms`]::
...
[id="{context}-property-incremental-snapshot-retry-max-attempts"]
xref:{context}-property-incremental-snapshot-retry-max-attempts[`incremental.snapshot.retry.max.attempts`]::
...
[id="{context}-property-incremental-snapshot-retry-initial-delay-ms"]
xref:{context}-property-incremental-snapshot-retry-initial-delay-ms[`incremental.snapshot.retry.initial.delay.ms`]::
...
[id="{context}-property-incremental-snapshot-retry-max-delay-ms"]
xref:{context}-property-incremental-snapshot-retry-max-delay-ms[`incremental.snapshot.retry.max.delay.ms`]::
...
[id="{context}-property-incremental-snapshot-retry-backoff-multiplier"]
xref:{context}-property-incremental-snapshot-retry-backoff-multiplier[`incremental.snapshot.retry.backoff.multiplier`]::
...
[id="{context}-property-poll-dispatch-interval-ms"]
xref:{context}-property-poll-dispatch-interval-ms[`poll.dispatch.interval.ms`]::
...
+
This property takes effect only if the connector's xref:{context}-property-snapshot-mode[`snapshot.mode`] property is set to a value other than `no_data`.
...
+
NOTE: Tables without a primary key, and tables that use snapshot select overrides, are processed as a single chunk and fall back to single-threaded snapshotting.

[id="{context}-property-snapshot-max-threads-multiplier"]
xref:{context}-property-snapshot-max-threads-multiplier[`snapshot.max.threads.multiplier`]::
...
[id="{context}-property-legacy-snapshot-max-threads"]
xref:{context}-property-legacy-snapshot-max-threads[`legacy.snapshot.max.threads`]::
...
+
ifdef::community[]
NOTE: The legacy table-per-thread behavior is deprecated and will be removed in a future release.
...
Main property::::
A comma-separated list of fully-qualified table names in the format `_<databaseName>.<tableName>_`.
...
For each table listed in the main property, you must define a corresponding `snapshot.select.statement.overrides._<databaseName>_._<tableName>_` property that specifies the custom `SELECT` statement to run during the snapshot.
+
The `SELECT` statement determines which rows from the table to include in the snapshot.
+
For example, to specify the `SELECT` statement for the `customers.orders` table:, add the following property:
+
...
[id="{context}-property-statistics-metrics-enabled"]
xref:{context}-property-statistics-metrics-enabled[`statistics.metrics.enabled`]::
...
+
For more information, see xref:{context}-streaming-metrics[Streaming metrics].
...
[id="{context}-property-topic-heartbeat-name"]
xref:{context}-property-topic-heartbeat-name[`topic.heartbeat.name`]::
...
[id="{context}-property-guardrail-collections-max"]
xref:{context}-property-guardrail-collections-max[`guardrail.collections.max`]::
...
[id="{context}-property-guardrail-collections-limit-action"]
xref:{context}-property-guardrail-collections-limit-action[`guardrail.collections.limit.action`]::
