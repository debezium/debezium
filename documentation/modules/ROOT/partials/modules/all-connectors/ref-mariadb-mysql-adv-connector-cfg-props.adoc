...
Use this property to specify a custom regular expression that identifies the configuration keys that contains sensitive information that should me masked.
+
[IMPORTANT]
====
...
`custom.sanitize.pattern=pass:['.*api\.key*|.*token.*']`
+
