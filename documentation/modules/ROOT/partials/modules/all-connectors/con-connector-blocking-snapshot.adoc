A blocking snapshot lets you capture a complete, consistent snapshot of a {data-collection} on demand while the connector is running, temporarily pausing streaming until the snapshot completes.
