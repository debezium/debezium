...
|`long`
|The number of update events skipped by the connector since its last start or metrics reset because no monitored columns changed.
ifdef::MONGODB[]
Note that for the MongoDB connector, this metric is not supported and will return `-1`.
endif::MONGODB[]
