...
The following list describes the purpose of select lines in the preceding Kafka Connect configuration:
...
[IMPORTANT]
====
For each of scripting language component, `artifacts.url` must specify the location of a JAR file, and the value of `artifacts.type` must be set to `jar`.
...
`ojdbc11`:::
Specifies the Maven Central location of the {connector-name} JDBC driver.
