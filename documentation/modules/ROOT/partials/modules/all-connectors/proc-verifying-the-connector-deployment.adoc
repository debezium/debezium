...
After deploying a {prodname} connector to {StreamsName} on OpenShift, verify that the connector is running correctly by checking the connector status, confirming that Kafka topics were created, and reviewing the initial snapshot events.
