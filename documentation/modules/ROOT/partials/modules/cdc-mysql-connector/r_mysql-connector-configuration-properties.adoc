...
|raw
|Specifies how binary (`blob`, `binary`, `varbinary`, etc.) columns should be represented in change events, including: `raw` represents binary data as native Java binary type (default), `base64` represents binary data as base64-encoded, `hex` represents binary data as hex-encoded (base16)
