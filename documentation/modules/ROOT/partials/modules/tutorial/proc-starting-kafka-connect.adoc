...
Start the Kafka Connect service, which exposes a REST API that you use to manage the {prodname} MySQL connector and monitor its status.

.Prerequisites

* Kafka and MySQL are running.
* You have connected to the `inventory` database with the MySQL command line client.
